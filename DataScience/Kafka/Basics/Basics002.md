...
* 또한, 같은 디렉토리의 `kafka-topics.sh` 명령어와 --list 옵션을 병용하여 임의의 브로커 서버와 관련된 토픽의 목록을 조회할 수 있다.
  * 이 때, 브로커 서버를 처음 실행한 경우라면 아무런 토픽이 없을 것이므로 빈 결과가 출력된다.
...
### 카프카 토픽을 생성하는 방법
* 카프카에 의해 관리되는 토픽을 생성하는 방법은 크게 다음과 같은 두 가지 방식으로 분류된다.
  1. 컨슈머 또는 프로듀서를 활용하여 아직 카프카 브로커에 생성된 적 없는 토픽의 데이터를 요청하기
  2. 또는 kafka-topics.sh 명령을 활용하여 명시적으로 토픽을 생성하기
* 첫 번째 방식의 경우, 파티션 개수 등 토픽의 여러 옵션들은 카프카 브로커의 설정 파일에 명시된 값을 따르게 된다.
  * 이는 여러 옵션을 명시하지 않은 경우 `server.properties`의 값을 따르던 CLI 방식과 유사하다.
* 반면, **토픽 역시 지속적으로 유지보수되어야 할 자산이므로 가능한 한 명시적인 생성 방식을 활용하는 것이 권장**된다.
  * 이는 무엇보다도 **토픽마다 처리하는 데이터의 특성이 다르기에 토픽을 명시적으로 생성할 경우 여러 옵션을 다르게 설정할 수 있기 때문**이다.
  * 예를 들어 동시 데이터 처리량이 많아야 하는 토픽의 경우 파티션의 개수를 높이거나, 단기간 데이터 처리만 필요한 경우 retention 옵션을 낮출 수 있다.
* 또한, **암시적인 방식을 통해 브로커의 기본 설정값에 따라 토픽이 생성된 경우에는 파티션의 개수를 다시 줄일 수 없다는 단점이 크게 작용하는 경우가 많다**.
* 상술한 이유에서, **토픽을 명시적으로 생성하는 경우에는 토픽에 적재될 데이터의 양과 병렬 처리 수준을 종합적으로 고려한 옵션을 설정하는 것이 바람직**하다.

## 2023-03-07 Tue
### 카프카 브로커와 CLI 툴 버전의 중요성
* **카프카 브로커에 CLI 툴을 통해 어떠한 작업을 요청하는 경우, 기본적으로는 둘 간의 버전을 일치시키는 것이 권장**된다.
* 브로커 버전이 높아지는 경우 이에 따라 CLI 명령의 상세한 옵션이 달라질 수 있으므로, 버전 차이로 인해 실행되던 명령이 실행되지 않는 경우가 존재할 수 있다.
  * 때문에 **임의의 카프카 버전을 사용하는 경우, 해당 버전에 함께 배포되는 CLI 툴을 사용하는 것이 바람직**하다. 