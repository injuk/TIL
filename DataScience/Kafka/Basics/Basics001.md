...
## 2023-02-13 Mon
### 임의의 브로커에 파티션이 쏠리는 경우
* 간혹 브로커 서버 하나에 파티션의 개수가 집중되는 경우가 발생할 수 있으며, 이는 모든 리더 파티션이 단일 브로커 서버에 배치됨을 의미한다.
  * 이 경우 모든 프로듀서 또는 컨슈머 등의 카프카 클라이언트 애플리케이션은 하나의 브로커 서버와만 통신하게 된다.
  * 이로 인해 통신이 집중되는 브로커는 CPU 및 메모리 등의 HW 자원을 많이 점유하게 될 수 밖에 없다.
* **상술한 이유로 인해 카프카 클러스터를 운영하는 경우, 브로커 하나에만 파티션이 집중되는 상황을 최대한 방지하고 주의를 기울이는 것이 바람직**하다.
* 또한, **특정한 브로커에만 파티션이 집중된 경우에는 `kafka-reassign-partitions.sh` 명령어를 활용하여 파티션을 재분배**할 수 있다.

### 파티션의 개수와 컨슈머 개수에 따른 처리량
* **파티션은 카프카 병렬 처리의 핵심이며, 그룹화된 컨슈머들이 레코드를 병렬로 처리할 수 있도록 매칭**된다.
  * 컨슈머와 파티션은 기본적으로 1:1 관계로 매칭되며, 파티션은 컨슈머 그룹 마다 최대 하나의 컨슈머만을 가질 수 있다.
  * 때문에 동일한 컨슈머 그룹에 컨슈머 애플리케이션을 추가로 배치하더라도, 파티션의 수가 고정되었다면 해당 파티션은 추가된 컨슈머와 매칭될 수 없다.
* **컨슈머의 처리량이 한정된 경우, 많은 레코드를 병렬로 처리하는 가장 좋은 방법은 컨슈머의 수를 늘리는 것**이다.
  * 그러나, 상술한 바와 같이 컨슈머가 추가되더라도 파티션의 개수가 고정되어 있다면 파티션이 매칭되지 않아 처리량은 늘어나지 않는다.
  * 따라서 **컨슈머를 스케일 아웃하는 동시에 파티션의 수도 늘려주어야만 처리량이 높아지는 효과**를 볼 수 있다.
* 이 때, **각 컨슈머는 여러 파티션과 매칭될 수 있으므로 컨슈머에 장애가 발생했더라도 다른 컨슈머가 이를 재할당받아 지속적으로 운영**해나갈 수 있다.
* 컨슈머를 늘림과 동시에 파티션을 늘리는 것으로 처리량을 증대시키는 것은 굉장히 중요하며, 실무에서도 병렬 처리량을 늘려야만 하는 경우가 많을 수 있다.
  * 이는 프로듀서의 관점에서 데이터 적재량을 예상할 수 없기 때문이며, 데이터 적재량이 높아지는 경우 컨슈머에는 지연이 발생할 수 있다.
  * 이러한 컨슈머의 데이터 소비 지연 현상을 컨슈머 랙이라고 지칭하며, 컨슈머는 제 때 데이터를 처리할 수 없게 된다.
  * 반면, **컨슈머와 파티션의 개수를 늘리는 것으로 처리량을 늘릴 수 있으므로 이러한 컨슈머 랙 현상에 대응할 수 있게 된다**.
* 운영 시에는 파티션과 컨슈머의 개수를 넉넉하게 할당하는 경우가 많으며, 예를 들어 초당 데이터 수가 10으로 예상된다면 파티션과 컨슈머는 각 20을 배정한다.
  * **이로 인해 다가올 컨슈머 랙을 어느 정도는 미연에 방지할 수 있으므로, 파티션과 컨슈머는 여럿 배포하여 운영하는 것이 일반적**이다.

### 파티션의 감소
```
> 한 번 늘린 파티션의 개수를 줄이는 것은 불가능하다! 
```
* **카프카의 경우 파티션의 수를 줄이는 것은 지원하지 않으므로, 파티션을 증설과 그 수량을 결정하는 작업은 반드시 신중히 결정**되어야 한다.
  * 카프카의 경우 파티션의 데이터를 세그먼트 형태로 저장하며, 이러한 기능을 지원하려면 여러 브로커의 데이터를 취합하고 정렬하는 복잡한 과정이 필요해진다.
  * 그러나 이러한 작업은 카프카 클러스터에 큰 영향을 줄 수 밖에 없으므로, 해당 기능은 논의된 적은 있으나 실제로 개발에 이르지는 못했다.
* **극단적으로, 한 번 늘린 파티션은 반드시 토픽의 삭제 및 재생성을 통해서만 없앨 수 있다**.

### 레코드의 구성 요소
* 레코드는 크게 다음과 같은 요소로 구성된다.
  1. 타임스탬프
  2. 헤더
  3. 메시지 키
  4. 메시지 값 
  5. 오프셋
* 프로듀서가 생성한 레코드를 브로커로 전송할 경우, 해당 레코드에는 오프셋과 타임스탬프가 지정되어 저장된다.
  * 실제로는 타임스탬프는 관련된 설정이 활성화되어 있는 경우에만 저장된다.
  * 반면, 오프셋은 프로듀서가 갖는 정보는 아니지만 브로커에 전송되어 저장되는 시점에 지정된다.
* 또한 앞서 다룬 바와 같이, **한 번 저장된 레코드는 수정이 불가하며 설정된 로그 리텐션 기간 또는 용량 설정에 의해서만 삭제**될 수 있다.
  * 나아가 이러한 삭제 과정은 레코드 개 별로 진행되는 것이 아닌, 레코드가 모인 세그먼트 파일 자체에 대해 진행된다.

### 레코드 타임스탬프란?
* 타임스탬프는 스트림 처리에서 활용하기 위한 시간 정보를 저장하는 용도로 사용된다.
  * 타임스탬프는 카프카 0.10.0.0 버전 이후부터 추가되었으며, Unix timestamp를 사용한다.
* **프로듀서 차원에서 관련된 설정을 하지 않았을 경우 기본적으로는 `ProducerRecord`의 생성 시간인 `CreateTime`이 지정**된다.
  * 또는 브로커에 적재되는 시간인 `LogAppendTime`으로 설정하는 것 역시 가능하다.
  * 이러한 옵션은 `message.timestamp.type` 설정을 통해 토픽 단위로도 지정이 가능하다.

### 레코드 오프셋이란?
```
> 오프셋은 프로듀서가 레코드를 생성하는 시점에는 존재하지 않는다.
```
* **오프셋은 프로듀서가 전송한 레코드가 브로커에 적재되는 시점에 지정되며, 0부터 시작하여 1씩 증가**한다.
* **각 컨슈머는 오프셋을 기반으로 처리가 완료된 데이터와, 처리가 되어야 할 데이터를 구분**할 수 있다.
  * 각각의 메시지는 파티션 별로 고유한 오프셋을 가지므로 컨슈머에서의 중복 처리를 방지하기 위한 목적으로도 사용될 수 있다.
* **컨슈머는 자신이 처리한 메시지 정보를 커밋을 통해 명시하며, 이러한 정보는 인터널 토픽에 저장**된다.

### 레코드 헤더란?
* 레코드 헤더는 카프카 0.11 버전부터 추가된 구성 요소이며, 키 - 값 형태의 데이터를 활용한다.
* 헤더는 예를 들어 레코드의 스키마 버전이나 포맷 등, 데이터 처리에 참고할 만한 메타데이터를 저장하는 용도로 사용될 수 있다.

## 2023-02-14 Tue
### 레코드 메시지 키란?
* 메시지 키는 메시지의 값을 분류하기 위해 사용되며, 이러한 분류는 파티셔닝이라는 용어로 지칭된다.
  * 이 때 **프로듀서에 설정된 Partitioner에 따라 메시지 키가 처리되며, 결과 어떤 토픽의 어떤 파티션 번호에 저장할지 결정**된다.
* 메시지 키는 필수 값은 아니며, 지정하지 않을 경우에는 기본적으로 null로 설정된다.
* 메시지 키가 null인 레코드는 round-robin 방식에 따라 특정 토픽의 파티션에 전달된다.
  * 반면, **null이 아닌 메시지 키를 갖는 경우 해싱에 의해 특정 파티션에 매핑되어 전달되므로 항상 같은 파티션에 매핑되는 것이 보장**될 수 있다.
  * 또한, **파티션은 FIFO 구조를 갖기에 메시지 키를 갖는 모든 레코드 역시 순차 처리가 될 것임을 보장**할 수 있다.
  * 때문에 데이터 별 분류 처리 또는 순차 처리를 위해서는 메시지 키를 활용할 수 있는 반면, 이러한 처리가 필요 없는 경우에는 null을 사용할 수 있다.

### 레코드 메시지 값이란?
* **메시지 값은 실제로 처리될 데이터가 포함되는 공간을 의미하며, 포맷은 제네릭 형태이므로 사용자에 의해 결정**된다.
* Float이나 Byte[] 또는 String 등 다양한 형태를 지정할 수 있으며, 필요한 경우 사용자 정의 포맷으로 직렬화 또는 역직렬화 클래스를 지정할 수 있다.
  * 그러나 **이렇게 브로커에 저장된 레코드의 메시지 값은 어떻게 직렬화되었는지 알 수 없으므로, 컨슈머는 반드시 미리 역직렬화 포맷을 알고 있어야 한다**.
* 일반적으로는 String으로 직렬화하는 것이 간편하나, 이러한 방법은 Int 값도 모두 문자열 처리하므로 공간 낭비가 있을 수 있다.
  * 때문에 필요한 경우, 사용자의 용도에 맞는 직렬화 및 역직렬화 클래스를 정의하는 것이 바람직하다.
  * 예를 들어 StringSerializer 클래스를 활용하여 JSON 형태의 데이터를 직렬화하거나, StringDeserializer 를 통해 역직렬화할 수 있다.

### 토픽 명명규칙
* 토픽의 이름은 다음과 같은 제약을 갖는다.
  1. 빈 문자열로 된 토픽 이름을 설정할 수 없다.
  2. 토픽 이름은 온점 하나 또는 두 개로 생성될 수 없다.
  3. 토픽 이름은 249자 미만이어야 한다.
  4. **토픽 이름은 영문자와 숫자, 온점과 언더바 및 하이픈 조합으로만 생성**할 수 있다.
  5. 카프카의 내부 관리 목적으로 사용되는 토픽인 `__consumer_offsets, __transaction_state`와 동일한 이름의 토픽을 생설할 수는 없다.
  6. **카프카 내부에서 사용하는 로직으로 인해 토픽 이름에 온점과 언더바가 동시에 포함될 수는 없다**.
* 이 때, 온점과 언더바가 동시에 포함되는 토픽을 생성할 수는 있으나 사용 상에 이슈가 발생할 가능성이 있으므로 지양하는 것이 바람직하다.

## 2023-02-15 Wed
### 유지보수성이 좋은 토픽 명명하기
* **토픽의 이름을 모호하게 작성할 경우 유지보수가 어려워질 수 있으며, 토픽은 어떠한 용도로 사용하는지 이름에 드러나는 것이 바람직**하다.
* **중요한 것은 토픽 명명 규칙을 팀 내에서 사전에 정의하고 이를 준수**하는 데에 있다.
  * 이를 **따르지 않을 경우 이러한 토픽들은 모두 팀의 기술 부채가 될 것이며, 무엇보다도 카프카는 토픽을 재생성하지 않는 한 이름의 변경이 불가능**하다.
* 명명 규칙을 따르지 않는 토픽은 주기적으로 확인하여 사용 여부를 판단하는 것이 바람직하며, 용도에 따라 다음과 같이 처리할 수 있다.
  1. 단발성 토픽: 일회성으로 만들어진 토픽이므로 삭제한다.
  2. 사용 중인 토픽: **토픽 삭제 후 명확한 이름과 함께 재생성하는 것이 권장**된다.

### 토픽 명명 예시
* 토픽 명명시, 일반적으로 다음과 같은 명명법을 따를 수 있다.
  1. `env.team.app.message-type`: 예를 들어, `prod.dev-team.message-platform.json`과 같이 명명할 수 있다.
  2. `project.service.env.event`: 예를 들어, `dev.audit-log.qa.notification`과 같이 명명할 수 있다.
  3. `env.service.jira-issue.message-type`: 예를 들어, `dev.email-receiver.jira-1234.email-vo-custom`과 같이 명명할 수 있다.
  4. `kafka-cluster-name.env.service.message-type`: 예를 들어, `aws-kafka.dev.email-platform.email.json`과 같이 명명할 수 있다.
* 특히, **메시지 타입을 명시하는 토픽은 프로듀서에 의해 직렬화된 데이터를 컨슈머가 역직렬화하는 기능을 개발하는 상황에 힌트**가 될 수 있다.

### 카프카 클라이언트와 메타데이터
```
> 카프카 클라이언트는 카프카 클러스터로부터 주기적으로 메타데이터를 전달받고, 이를 토대로 리더 파티션이 위치한 브로커 서버와 통신한다.
```
* 카프카 클라이언트는 통신하고자 하는 리더 파티션의 위치를 알아내기 위해 데이터를 주거나 받기 전에 브로커로부터 메타데이터를 우선 전달받는다.
  * **카프카 클라이언트는 프로듀서와 컨슈머를 모두 포함하며, 데이터를 주는 경우는 프로듀서인 반면 데이터를 받는 경우는 컨슈머 애플리케이션을 의미**한다.
* 이러한 메타데이터는 크게 다음과 같은 옵션을 통해 새로고침될 수 있다.
  1. `metadata.max.age.ms`: 메타데이터를 강제로 새로고침하는 간격을 의미하며, 기본 값은 5분으로 설정된다.
  2. `metadata.max.idle.ms`: 프로듀서가 유휴상태인 경우 메타데이터를 캐시에 유지하는 기간을 의미하며, 기본 값은 5분으로 설정된다.
* 특히 캐시 설정의 경우, 예를 들어 프로듀서가 특정한 토픽으로 데이터를 전송한 후 일정 시간이 지난 경우에 메타데이터를 강제 새로고침하게 된다.

### 카프카 클라이언트 메타데이터와 관련된 이슈
* 카프카 클라이언트는 종류를 불문하고 반드시 리더 파티션과 통신해야 하지만, 메타데이터가 제 때 새로고침되지 않았다면 예외가 발생할 수 있다.
  * 이 경우, 카프카 클라이언트 애플리케이션은 잘못된 파티션에 정보를 요청하게 되므로 `LEADER_NOT_AVAILABLE` 예외가 발생하게 된다.
  * 이러한 예외는 클라이언트가 데이터를 요청한 브로커에 실제로는 리더 파티션이 존재하지 않는 경우에 발생한다.
* 이는 **전형적인 메타데이터 새로고침과 관련된 이슈이며, 이러한 예외 상황이 자주 발생한다면 반드시 설정된 메타데이터 새로고침 간격을 확인**해야 한다.
  * 즉, 반드시 카프카 클라이언트가 정상적인 메타데이터를 갖고 동작하는지 확인해야 한다.