...
### 리더 파티션과 팔로워 파티션
* 상술한 카프카 브로커의 복제 기능에서, 복제된 파티션은 리더와 팔로워로 구분된다.
  * 이 때, **프로듀서 또는 컨슈머와 직접 통신하는 파티션은 리더 파티션이며, 나머지 복제 데이터를 갖는 파티션은 팔로워**로 지칭한다.
* 팔로워 파티션의 경우, 리더 파티션의 오프셋을 확인하여 현재 자신이 갖는 오프셋과 차이가 있을 때 리더 파티션으로부터 조회하여 자신의 파티션에 저장한다.
  * 이러한 과정을 솝쉽게 표현하면 '복제'가 된다.