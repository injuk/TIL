...
## 2022-12-16 Fri
### 타입의 필요성
* 타입을 사용하지 않는 경우, 시간에 따라 변화무쌍해지는 객체의 상태로 인한 복잡성을 극복하기 어려울 수 밖에 없다.
  * 그러나 객체에 타입을 지정할 경우, 동적인 상태와는 무관하게 유일하게 변하지 않는 식별성을 부여할 수 있게 된다.
* 이렇듯 **타입은 시간에 따라 동적으로 변화하는 상태를 시간과 무관한 정적인 모습으로 다룰 수 있도록 한다**.
  * 즉, **타입은 객체로부터 시간이라는 요소를 제거함으로써 시간에 독립적인 정적인 관점에서 객체의 상태를 표현할 수 있도록 한다**.
* **상술한 관점에서 타입은 불필요한 세부 사항을 제거하여 중요한 것에 집중할 수 있도록 하는 추상화의 두 번쩨 관점을 따르는 추상화라고 이해**할 수 있다.
  * 즉, 어떠한 시점에 객체에 대해 생각할 때는 불필요한 시간 및 이에 따른 상태 변화라는 요소는 제거하고 철저히 정적인 관점에서 객체를 묘사할 수 있도록 한다.
* 따라서 **타입은 곧 추상화이며, 특히 타입을 활용하여 시간에 따른 상태 변경이라는 복잡한 객체의 동적인 특성을 추상화**할 수 있다.

### 동적 모델과 정적 모델
* 동적 모델과 정적 모델은 일반적으로 객체를 생각하는 과정에서 동시에 고려되며, 각각 다음과 같은 특징을 갖는다.
  1. 동적 모델: 객체가 특정 시점에 구체적으로 어떠한 상태를 갖는지를 의미하는 스냅샷을 고려하며, 객체 생명 주기 동안 상태와 행동을 포착한다.
  2. 정적 모델: 객체가 가질 수 있는 모든 상태와 행동을 시간에 독립적으로 표현하며, 타입 모델이라고도 지칭한다.
* **객체지향 애플리케이션을 설계하고 구현하는 과정에서는 기본적으로 객체 관점의 동적 모델과 타입 관점의 정적 모델을 자유로이 혼용**할 수 있어야 한다.
  * 기본적으로 개발자는 객체지향 언어를 통해 클래스를 정의하는 동안에는 정적인 관점에서 애플리케이션을 접근한다.
  * 반면, 애플리케이션을 실행하여 런타임에서 상태를 추적하고 디버깅하는 과정은 동적인 모델로부터 접근하는 것과 같다.
* 상술한 이유에서, **좋은 객체지향 개발자는 애플리케이션의 동적인 관점과 정적인 관점을 모두 다룰 수 있어야 한다**.

### 구현 방법 중 하나로서의 클래스
* 일반적으로 객체지향 패러다임을 주도하는 언어들은 클래스를 기반으로 하기에 많은 개발자들이 클래스와 타입을 동일한 개념으로 착각한다.
  * **실제로도 객체지향 언어에서 정적인 모델은 클래스로 구현되므로, 타입을 구현하는 가장 보편적인 방법은 클래스라고 볼 수도 있다**.
  * 그러나 **실제로는 두 개념이 다르며, 타입은 객체를 분류하기 위한 개념인 반면 클래스는 타입을 구현할 수 있는 구현 기법 중 하나에 지나지 않는다**.
  * 예를 들어, JS의 경우 프로토타입 기반의 객체지향 메커니즘을 제공하므로 클래스가 존재하지 않는다.
  * 또한, 클래스는 타입 구현 이외에도 코드 재사용을 위해 사용되기도 하므로 두 개념을 동일시하는 것은 많은 오해를 불러일으키기 쉽다.
* 클래스와 타입을 구분할 줄 아는 것은 유연한 설계를 유지하기 위한 밑바탕이 될 수 있다.

### 타입과 추상화 - 결론
* **객체를 분류하는 기준은 타입**이다.
* **타입을 나누는 기준은 객체가 수행하는 행동**이다.
* **클래스는 객체를 분류하기 위한 타입이 결정되었을 때 프로그래밍 언어를 통해 이러한 타입 개념을 구현하는 방법 중 하나에 불과**하다.
  * 즉, 클래스는 객체지향 패러다임만의 특별한 개념이 아닌 프로그래밍 언어 차원에서 제공하는 구현 메커니즘이다.
* **객체지향 패러다임에서 중요한 것은 동적으로 변화하는 객체의 상태와, 이러한 상태를 변경하는 행동**이다. 