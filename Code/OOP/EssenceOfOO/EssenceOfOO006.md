...
### 도메인 모델과 유즈케이스의 통합 
```
> 불안정한 기능을 안정적인 구조에 담는 것으로 변경이 영향을 주는 범위를 최소화하는 것은 좋은 객체지향 설계자가 되기 위한 기본적인 설계 능력이다.
```
* 도메인 모델은 안정적인 구조를 개념화하기 위해 사용되는 반면, 유즈케이스는 불안정한 기능을 서술하는 데에 가장 일반적으로 사용되는 기법이다.
  * 이 때, **유연한 소프트웨어를 만들기 위해서는 유즈케이스에 정리된 애플리케이션의 기능을 도메인 모델을 기반으로 한 객체들의 책임으로 분배**해야 한다.
* 또한, **사용자의 관점에서 애플리케이션은 자신이 전송한 메시지에 응답하기 위해 필요한 책임을 수행하는 일종의 객체**로 볼 수 있다.
  * 이 경우, 애플리케이션이라는 거대한 객체는 더 작은 여러 객체로 분할될 수 있다.
  * 즉, **애플리케이션이 수행해야 할 거대한 책임은 작은 크기를 갖는 객체들의 협력으로 구현**될 수 있다.
  * 뿐만 아니라 **협력의 출발을 알리는 첫 번째 메시지는 애플리케이션의 기능을 거대한 애플리케이션의 책임으로 바꾸어진 후에 얻어진 것**과 같다.
* **애플리케이션의 거대한 책임을 작은 규모의 객체들에게 할당하는 경우, 도메인 모델에 포함된 개념을 은유하는 소프트웨어 객체를 선택하는 것이 바람직**하다. 
  * 이를 통해 소프트웨어와 코드 상의 표현적 차이를 줄여나갈 수 있다.

### 애플리케이션 설계와 도메인 모델, 유즈케이스, 책임 주도 설계
* 애플리케이션을 설계하는 과정에서, 세 개념은 다음과 같은 역할을 수행한다.
  1. **유즈케이스는 사용자에게 제공할 기능을 애플리케이션의 책임으로 보게 하며, 객체 간의 안정적 구조에 책임을 분배하기 위한 출발점**이 되어준다.
  2. **도메인 모델은 기능을 수용하기 위해 은유할 수 있을 만한 안정적인 구조를 제공**한다.
  3. **책임 주도 설계는 유즈케이스로부터 첫 메시지와 사용자 목표를 얻고, 도메인 모델로부터 기능을 수용할 안정적인 구조를 제공받아 객체 공동체를 창조**한다.
* 이렇듯 **책임 주도 설계는 애플리케이션의 기능을 역할과 책임을 수행하는 객체들의 협력으로 바라보게 하며, 이를 통해 유즈케이스와 도메인 모델을 통합**한다.
* 나아가 견고한 객체지향 애플리케이션을 개발하기 위해서는 다음과 같은 점에 주의를 기울여야 한다.
  1. **사용자 관점에서 시스템 기능을 명시한 후, 사용자와 설계자가 공유하는 안정적인 구조를 기반**으로 삼는다.
  2. 그 이후에서야 기능을 책임으로 변환하는 체계적인 절차를 따른다.

### 객체의 재귀적 합성
* **유즈케이스로부터 시작하여 객체들의 협력을 구성하는 과정은 객체 안에 다른 객체가 포함되는 재귀적 합성이라는 객체지향의 기본 개념이 잘 드러난다**.
  * 객체지향은 모든 것을 객체로서 바라보며, 큰 객체가 작은 객체로 나눠질 수 있다는 점을 적극적으로 활용한다.
  * 이 때, **크기와 관계 없이 모든 객체는 메시지를 전송하거나 수신하고 메시지에 응답하기 위해 자율적으로 메소드를 선택**한다.
* 이러한 객체에 대한 재귀는 객체지향의 개념을 모든 추상화 수준에서 적용 가능케 하며, 동시에 객체지향 패러다임이 어디에나 일관되게 적용될 수 있도록 한다.

### 도메인 모델이 안정적인 이유
* **도메인 모델을 기반으로 객체 구조를 설계하는 것은 도메인 모델이 안정적이기 때문이며, 도메인 모델은 다음과 같은 특징으로 인해 안정성**을 갖는다.
  1. 도메인 모델을 구성하는 개개의 개념은 비즈니스가 없어지거나 완전히 변경되지 않는 한은 안정적으로 유지될 수 있다.
  2. 도메인 모델을 구성하는 개념 간의 관계 역시 비즈니스 규칙을 기반으로 하므로, 비즈니스 정책이 크게 변경되지 않는 한은 안정적으로 유지될 수 있다.
* 도메인 모델의 특징은 도메인 모델을 중심으로 객체 구조를 설계하고 유즈케이스 상의 기능을 객체의 책임으로 분배하는 기본적인 객체지향 설계의 유연함을 드러낸다.
  * 예를 들어, 비즈니스 정책 또는 규칙이 크게 변경되지 않는 이상은 시스템의 기능이 변경되더라도 객체 간의 관계가 일정하게 유지될 수 있다.
  * 반면, 기능적인 요구사항이 변경되더라도 책임과 객체 간의 대응 관계만이 수정되게 된다.
  * 즉, 변경에 대한 파급효과는 최소화되어 요구사항 변경에 유연하게 대응할 수 있는 애플리케이션을 구축할 수 있게 된다.
* 이렇듯 **도메인 모델을 기반으로 한 애플리케이션은 비즈니스의 핵심 규칙이 변경되지 않는 이상 전체 구조가 한 번에 뒤흔들리지 않는다**.
  * 이는 또한 객체지향이 변경에 대해 더 유연하게 대응할 수 있는 패러다임으로 지목되는 이유이기도 하다.

### 연결 완전성과 가역성
```
> 객체지향의 가장 큰 장점은 도메인을 모델링하는 기법과 도메인을 프로그래밍하기 위해 사용하는 기법이 동일하다는 데에 있다.
```
* 객체지향 패러다임에서는 도메인 모델링에 사용된 객체와 개념을 프로그래밍 설계 시 객체와 클래스로 쉽게 변환할 수 있으며, 이는 연결 완전성으로 지칭된다.
  * 반면, 객체지향의 강력함은 연결 완전성의 역방향 역시 성립된다는 점에서 명확하게 드러난다.
  * **객체지향의 세계에서는 도메인 모델과 코드 모두가 공일한 모델링 패러다임을 공유하므로, 코드의 수정은 곧 모델의 수정**과 같다.
* 이렇듯 모델에서 코드로의 매끄러운 흐름을 지칭하는 연결 완전성의 반대 흐름은 가역성이라는 표현으로 지칭된다.
  * 예를 들어, **애플리케이션을 구성하는 코드가 변경되는 경우 각 이해관계자들은 변경된 코드로부터 자연스럽게 변경된 도메인 모델의 구조를 유추할 수 있다**.
* 이러한 과정에서 **도메인 모델은 코드와 분리된 별개의 산출물이 아니며, 문서나 다이어그램은 더더욱 아니다**.
  * 대신 도메인 모델은 이해관계자의 머릿 속에 든 상태로 서로에게 공유되는 멘탈 모델이다.
  * 나아가 **이해관계자들이 동일한 용어와 개념을 통해 의사소통하고, 코드로부터 도메인 모델을 유추할 수 있도록 하는 것이 도메인 모델의 진정한 목표**가 된다.

### 결론
* **안정적인 도메인 모델을 기반으로 애플리케이션의 기능을 구현하는 것이 바람직**하다.
* 나아가 **도메인 모델과 애플리케이션의 코드가 서로 밀접하게 연관되도록 주의를 기울이는 것이 바람직**하다.
* 이러한 과정을 통해서 작성된 객체지향 애플리케이션은 유지보수성과 유연성이라는 두 마리 토끼를 모두 잡게 될 가능성이 높아진다.