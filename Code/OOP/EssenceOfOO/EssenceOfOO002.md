...
### 현실과 객체지향 세계의 차이
```
> 널리 퍼진 객체지향의 오해 중 하나는 현실 세계에 대한 모방이라는 점이지만, 객체지향 세계는 현실 세계를 단순히 모방하지 않는다. 
```
* 흔히 객체지향은 현실의 추상화라고 하며, 이는 객체지향 세계가 현실을 모방하여 단순화한다는 의미가 포함된다.
  * 이 때, **추상화는 실제 사물로부터 원하는 특성만 취하고 필요 없는 부분을 걸러내어 핵심만을 표현하는 행위를 의미**한다.
* 그러나 객체지향 세계는 현실 세계를 단순히 모방하지 않으며, 모방과 추상화로는 현실의 객체와 소프트웨어 상의 객체 간의 관계를 깔끔하게 설명할 수 없다.

### 현실의 객체와 소프트웨어 객체의 차이
* **현실의 객체와 소프트웨어 객체의 가장 큰 차이는 현실의 수동적인 객체가 소프트웨어 상에서는 능동적으로 변할 수 있다는 점**에 있다.
  * 소프트웨어 객체를 설계하는 과정에서, 개발자는 오히려 현실 세계의 객체를 모방하는 대신 현실과는 전혀 다른 특징을 부여하는 것이 일반적이다.
  * 때문에 **소프트웨어 객체는 현실 객체의 부분적인 특징을 모방하는 것이 아니라, 현실 객체가 갖지 못한 추가적인 능력을 보유**하게 된다.
  * 예를 들어 현실의 객체는 대부분의 경우 인간이라는 에이전트가 필요하지만, 소프트웨어 세계의 객체는 정해진 체계 안에서 능동적이고 자율적인 에이전트가 된다.
  * 즉, **현실로부터 가져온 객체지향 세계의 객체들은 현실에서는 불가능한 일도 수행할 수 있는 전능한 존재로 승격**된다.
* 상술한 바와 같이 **현실의 객체보다 더 많은 일을 수행할 수 있는 소프트웨어 객체의 특징을 의인화라고 지칭**할 수 있다.
  * 의인화의 관점에서 소프트웨어는 생물처럼 생각할 수 있으며, 모든 생물과 같이 소프트웨어는 태어나서 죽기까지의 생명 주기를 갖는다.
* 이렇듯 **소프트웨어 안에서 구축되는 객체지향 세계는 현실을 모방하는 대신, 조금만 참조하여 현실과는 다른 전혀 다른 새로운 세계로 창조**된다.
  * 나아가 객체지향 세계는 현실을 추상화하는 대신, 현실의 객체보다 더 많은 특징과 능력을 보유한 객체들로 구성된다.

### 모방 대신 은유하기
* **객체지향 세계와 현실 세계가 전혀 무관한 것은 아니며, 모방이나 추상화 대신 은유적인 관점에서 설명했을 때 두 세계의 관계를 더 명확히 설명**할 수 있다.
  * 이 때, 은유는 실제로는 적용되지 않는 하나의 개념을 통해 다른 개념을 서술하는 비유법을 의미한다.
  * 즉, **은유의 본질은 하나의 사물을 다른 사물의 관점으로부터 이해하고 경험하는 데에 있다**.
* 은유라는 관계는 객체지향 모델링에서 발결되는 현실과 소프트웨어 상의 객체 관계와 일치한다.
  * 예를 들어, **현실 객체의 의미의 일부는 소프트웨어 객체로 전달되므로 소프트웨어 객체는 현실 객체에 대한 은유**에 해당한다.
* 또한, **은유 관계에 있는 실제 객체의 이름을 소프트웨어 상의 객체 이름으로 명명하는 것으로 표현적인 차이를 줄여 구조를 쉽게 예측할 수 있다**.
  * 즉, **소프트웨어 객체에 대한 현실 객체의 은유를 통해 표현적인 차이를 줄여 이해하기 쉽고 유지보수가 쉬운 소프트웨어를 제작**할 수 있게 된다.
  * 이러한 이유에서 많은 객체지향 지침서는 현실 도메인에서 사용되는 이름을 객체에 명명할 것을 권장한다.

### 이상한 나라 창조하기
* 루이스 캐럴의 이상한 나라의 앨리스에서, 체셔 고양이나 트럼프 카드는 현실 세계의 객체와는 거리가 멀다.
  * 그러나 **이들은 현실 속의 객체인 고양이와 카드를 은유하므로, 울음 소리나 쉽게 뒤집힌다는 특징을 쉽게 예상하고 공감**할 수 있다.
  * 이는 전적으로 루이스 캐럴이 현실의 객체를 바탕으로 은유를 통해 이상한 나라의 객체를 묘사하기 때문이다.
* 이렇듯 **객체지향 설계자로서 개발자의 목적은 현실을 모방하는 대신 자신만의 이상한 나라를 창조하는 데에 있다**.
  * **현실을 닮아야한다는 어떠한 강박이나 제약도 없으며, 단지 자신이 만들어낸 객체의 특징을 상기시키기 위해 현실 속위 객체 이름을 활용**할 수 있다.
  * 그렇지 못하는 경우가 있더라도 당황하는 대신 **현실을 완전히 무시한 채 자신만의 새로운 세계를 창조해도 무방**하다.