...
### 책임을 바탕으로 협력을 설계하기 
```
> 올바른 객체를 설계하기 위해서는 우선 견고하고 깔끔한 협력을 설계할 필요가 있다.
```
* **협력을 설계하는 것은 곧 설계에 참여하는 객체들이 주고받을 요청과 응답의 흐름을 결정하는 것**과 같다.
  * 또한, 이렇게 **결정된 요청과 응답의 흐름은 객체가 협력에 참여하기 위해 수행할 책임**이 된다.
* 나아가 **객체에게 책임을 한 번 할당한 후에는 책임은 객체가 외부에 제공할 행동**이 된다.
* 이렇듯 **협력이라는 문맥에서 객체가 수행할 적절한 책임, 즉 행동을 결정한 후에야 해옹을 수행하기 위해 필요한 데이터를 고려**해야 한다.
  * 객체가 협력에 참여하기 위해 필요한 행동과 데이터가 정리된 후에야 클래스의 구현 방법을 결정하게 된다.
  * 즉, 클래스와 데이터는 객체의 협력과 책임 집합이 결정된 후에야 고려될 수 있다.

### 객체지향 패러다임과 협력의 설계
```
> 객체지향은 적절한 객체에 올바른 책임을 할당하는 것과 관련된 모든 것이다.
> 또한, 협력이라는 문맥 속에서 객체를 생각하는 것은 올바른 객체지향 애플리케이션을 구현하는 것과 관련된 모든 것이다.
```
* 일단 **협력이라는 문맥이 견고하게 갖춰진다면, 개발자의 초점은 자연스레 협력을 위해 필요한 책임의 흐름으로 옮겨진다**.
  * 나아가 **객체지향 품질은 협력에 필요한 책임을 결정하고, 객체에 책임을 얼마나 적절히 할당했는지에 따라 결정**된다.
* 이렇듯 **객체의 행위에 초점을 맞추려면 우선 협력이라는 문맥을 고려하여 책임을 분배할 필요**가 있다.
  * 다시 말해 **객체의 상태에 대해 고민하기 전에 우선 행위를 고민해야 하며, 행위를 고민하기 전에 객체가 참여할 문맥인 협력을 정의하는 것이 바람직**하다.
* **객체지향 애플리케이션에서 가장 중요한 것은 충분히 자율적이면서도 충분히 협력적인 객체를 창조하는 것**이다.
  * 이를 달성하는 가장 쉬운 방법은 우선 객체를 충분히 협력적으로 정의한 후, 협력이라는 문맥에서 객체를 충분히 자율적으로 만드는 것이다.

### 책임 주도 설계
* **책임 주도 설계 가이드라인에 따를 경우 시스템의 기능은 더 작고 많은 책임으로 분할되며, 각 책임은 적절한 객체에 할당**된다.
  * 이 과정에서 각 객체는 스스로 처리할 수 없는 정보나 기능을 다른 객체에게 요청하는 식으로 협력하게 된다.
  * 이 때, 요청된 작업을 수행하는 것은 작업을 위임받은 객체의 책임으로 변환된다.
* 또한, **책임을 여러 종류의 객체가 수행할 수 있다면 객체의 협력자는 또 다른 객체가 아닌 추상적인 역할로 대체**될 수 있다.
* 이러한 책임 주도 설계를 활용하여 협조적이고 자율적인 객체지향 애플리케이션을 설계해나가는 과정은 크게 다음과 같이 요약된다.
  1. 애플리케이션이 사용자에게 제공할 기능인 애플리케이션 책임을 우선 파악한다.
  2. 애플리케이션 책임을 더 작은 여러 개의 책임으로 분할한다.
  3. 이렇게 분할된 책임을 수행할 수 있는 적절한 객체, 또는 역할을 찾아 책임을 할당한다.
  4. 각 객체는 책임을 자신만의 방법으로 성실히 수행하되, 다른 객체의 도움이 필요한 경우에는 이를 책임질 적합한 객체 또는 역할을 찾는다.
  5. 이렇게 찾아낸 해당 객체 또는 역할에게 책임을 할당하는 것으로 두 객체가 협력하도록 유도한다.
* **중요한 것은 역할과 책임, 그리고 협력이며 이들은 유연하고 견고한 객체지향 애플리케이션을 정의하는 데에 있어서 가장 중요한 재료**가 된다.
  * 그 외의 장치는 이들을 보완하고, 애플리케이션의 복잡도를 낮추기 위한 보조 재료에 지나지 않는다.

### 디자인 패턴
```
> 책임 주도 설계가 객체의 역할과 책임, 그리고 협력을 고안하기 위한 방법과 절차는 제시하는 반면 디자인 패턴은 책임 주도 설계의 결과를 표현한다.
```
* **일반적으로 디자인 패턴은 모범이 되는 설계로서, 반복적으로 발생하는 문제와 이에 대한 해법의 쌍으로 정의**된다.
* 이렇듯 **디자인 패턴은 책임 주도 설계의 결과물인 동시에 지름길이며, 공통으로 사용할 수 있는 역할과 책임 및 협력의 템플릿으로서 기능**한다.
  * 여러 상황에 적용 가능한 디자인 패턴을 잘 알고 있다면 책임 주도 설계의 절차를 순서대로 따르지 않더라도 역할과 책임 및 협력 관계를 빠르게 포착할 수 있다.

### 테스트 주도 개발
* 테스트 주도 개발은 객체가 이미 존재한다고 가정하고 객체에 대해 어떤 메시지를 전송할지 생각하라고 조언하지만, 이는 초보자에게는 적절하지 않을 수 있다.
  * 예를 들어, 이러한 충고는 역할과 책임 및 협력의 관점에서 객체를 바라보지 않는다면 무의미하다.
* **테스트 주도 개발은 단순히 테스트를 작성하는 것이 아니며, 책임을 수행할 객체 또는 역할이 메시지를 수신할 때의 반응과 협력을 코드화하는 것**이다.
* 테스트 주도 개발 역시 책임 주도 설계의 기본 원칙을 따르지만, 기본적으로는 책임 주도 설계의 절차와 기법을 빠르고 감각적으로 수행할 역량이 전제되어야 한다.
  * 즉, **테스트 주도 개발은 다양한 설계 경험과 패턴에 대한 지식이 없는 경우에는 온전한 혜택을 누리기 어렵다**.
* 이렇듯 테스트 주도 개발은 책임 주도 설계의 기본 개념과 원칙을 종합적으로 이해하고, 좋은 설계에 대한 감각과 경험 등 객체지향의 깊이 있는 이해가 요구된다.
  * 즉, **역할과 책임 및 협력에 집중하고 객체지향의 여러 원칙을 적용하려는 깊이 있는 고민과 노력을 통해서만 테스트 주도 개발의 혜택을 누릴 수 있다**.