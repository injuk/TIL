...
## 2025-11-18 Tue
### 기본 자료형과 타입으로 생각하기
* 편의상 엘릭서가 제공하는 개념들을 다음과 같이 기본 자료형과 타입을 다음과 같이 구분할 수 있다고 가정하자.
    1. 기본 자료형: 언어 차원에서 제공되는 저수준의 데이터 구조로, 리스트나 맵 등 최소한의 기능만을 제공한다.
    2. 타입: 기본 자료형에 더해 풍부한 의미와 기능을 위한 추상화 및 연산을 모두 포함하는 새로운 계층이며, 예를 들어 `List` 모듈이 있다.
* 이렇듯 **임의의 데이터 구조가 갖는 세부 사항인 구현과 이를 활용하는 여러 방법을 기술하는 추상화 계층을 구분해서 이해**할 수 있다.
    * 다시 말해, 데**이터 구조인 리스트는 기본 자료형이지만 리스트에 더해 여러 동작을 가능케 하는 `List` 모듈을 기반으로 `List` 타입을 사용**할 수 있다.
* 반면, 키워드 리스트는 내부적으로 튜플의 리스트이기에 `Keyword` 모듈은 물론 `List` 모듈의 기능을 활용할 수도 있다.
    * 이는 동적 객체 지향 언어에서 말하는 덕 타이핑 개념과도 상통하는 관점에 해당한다.
    * 이렇듯 **엘릭서의 타입 시스템은 동일한 데이터 구조를 여러 방식으로 활용하는 식으로 유연성과 조합 가능성을 모두 높일 수 있도록 설계**되었다.
 
## 2025-11-19 Wed
### 컬렉션과 순회 탐색 모듈
* 엘릭서에는 컬렉션으로 취급되는 타입이 여럿 존재하며, 대표적으로 리스트나 맵 또는 범위 등을 예로 들 수 있다.
* 이러한 타입들의 내부 구현은 서로 다르지만, 순회 탐색이 가능하다는 주요한 공통점을 갖고 있기도 하다.
* 이렇듯 순회 가능한 타입들은 `Enumerable` 프로토콜을 구현했다고 표현할 수 있으며, 엘릭서는 순회 함수를 모아두는 다음과 같은 두 모듈을 제공한다.
    1. `Enum`: 일반적인 용도로 컬렉션을 순회하기 위한 함수들이 위치한다.
    2. `Stream`: 컬렉션을 순회할 때 지연 연산을 적용하며, 특수한 상황에서는 매우 유용하게 사용될 수 있다.

## 2025-11-20 Thu
### Enum 모듈과 안정 정렬
* `Enum` 모듈은 컬렉션을 순회 또는 필터링, 합치기나 나누기 등 컬렉션 자체를 다루는 여러 연산을 제공한다.
    * `Enum` 모듈은 엘릭서 진영에서 가장 많이 사용하는 라이브러리이기도 하다.
* 반면, `Enum` 모듈은 정렬을 위한 함수인 `Enum.sort`를 제공하며 다음과 같은 사용성을 갖는다.
```elixir
list = ["there", "was", "a", "crooked", "man"]
Enum.sort list # ["a", "crooked", "man", "there", "was"]
Enum.sort list, &(String.length(&1) <= String.length(&2)) # ["a", "was", "man", "there", "crooked"]
```
* 이 때, 정렬 알고리즘은 값이 동일한 값들에 대해 순서를 보장하는지 여부에 따라 안정 정렬과 불안정 정렬로 구분할 수 있다.
    1. 안정 정렬의 경우, 같은 값의 순서를 원본과 같이 유지한다.
    2. 불안정 정렬의 경우, 같은 값의 순서가 반드시 원본과 같이 유지된다고 보장할 수 없다.
* 때문에 **자신만의 정렬 알고리즘을 작성하는 경우에는 반드시 `<=`와 같이 등호 기호를 포함시켜 안정 정렬을 활용할 수 있도록 하는 것이 바람직**하다.
    * 이렇듯 **값 비교시 등호를 작성하는 것만으로도 '같은 값의 경우 원래 순서를 유지'하도록 하는 의도를 명시하여 안정 정렬을 적용**할 수 있다. 

## 2025-11-21 Fri
### Stream과 지연 연산
* 앞서 다루었던 `Enum` 모듈은 탐욕스러운(greedy) 연산을 적용하므로, 주어진 컬렉션에 대해 포함된 모든 요소에 명시된 연산을 적용한다.
    * 또한, 수행된 결과는 일반적으로 새로운 컬렉션이 되므로 주어진 리스트가 커질수록 연산 효율은 낮아질 수 있다.
* 연산에 따라서는 `Enum` 모듈의 방식과는 달리 각 요소마다 필요한 연산들을 순차로 적용하고 싶을 수 있으며, 엘릭서는 이를 위해 `Stream` 모듈을 제공한다.
* 엘릭서의 스트림은 다음과 같이 쉽게 생성할 수 있으나, 결과를 즉시 반환하는 `Enum.map`과는 달리 스트림 자체가 반환된다.
```elixir
s = Stream.map [1,3,4,6], &(&1 + 1)
IO.puts inspect s # #Stream<[enum: [1, 3, 4, 6], funs: [#Function<49.65765362/1 in Stream.map/2>]]>이 출력된다.
```
* 이렇게 생성된 스트림 역시 `Enumerable` 프로토콜의 구현체에 해당하므로, `Enum.to_list` 등을 활용하여 리스트로 변환할 수 있다.
* **엘릭서 스트림은 중간 결과를 저장하는 리스트를 반환하지 않으며, 대신 원본 컬렉션의 각 값에 차례대로 주어진 함수를 적용**한다.
    * 이렇듯 `Enum` 방식은 데이터가 모두 들어올 때까지 대기하지만, 스트림 방식은 데이터가 들어올 때마다 처리를 적용할 수 있다.
* 또한, **스트림은 지연 계산을 처리하므로 모든 입력이 미리 준비되어 있을 필요가 없기에 무한 스트림 형태의 기능을 제공**할 수도 있다.
    * 때문에 일반적인 리스트가 아닌, 네트워크나 센서 등을 통한 끝없는 외부 데이터 수신 용도에 적합할 수 있다.

## 2025-11-22 Sat
### Stream 다루기
* 가장 간단한 형태의 스트림은 다음과 같이 만들 수 있으며, 이 경우 반환형은 리스트가 아닌 `Stream`인 것을 확인할 수 있다.
    * 이 때, **반환되는 스트림은 임의의 컬렉션에 대해 수행해야 할 연산의 명세가 포함**된다.
    * 이렇듯 스트림은 연산의 명세만을 포함하므로, 이를 실제 결과로 변환하기 위해서는 Enum 모듈을 활용할 수 있다.
```elixir
s = Stream.map [1,3,5,7], &(&1 + 1)
#Stream<[enum: [1, 3, 5, 7], funs: [#Function<49.65765362/1 in Stream.map/2>]]>
Enum.to_list s # [2,4,6,8]
```
* 이러한 동작은 스트림 역시 `Enumerable` 프로토콜을 구현하는 타입이기 때문에 가능하며, 이러한 특성으로 인해 스트림에 스트림을 전달하여 조합할 수도 있다.
```elixir
1..100 
    |> Stream.map(&(&1*&1)) 
    |> Stream.map(&(&1+1)) 
    ...
    |> Enum.to_list
```
* 이 때, **스트림은 각 단계마다 중간 결과를 담는 리스트를 생성하는 대신 원본 컬렉션의 각 값에 대해 순차대로 정해진 연산을 적용하는 방식으로 동작**한다.
* 일반적으로 **스트림을 활용하는 연산은 컬렉션을 활용한 연산보다 느리지만, 스트림은 모든 데이터가 준비될 때까지 기다릴 필요가 없다는 큰 장점**을 갖는다.

## 2025-11-23 Sun
### 무한한 값을 처리할 수 있는 Stream
* 스트림은 지연 계산을 수행하므로 모든 입력 값이 준비되어 있지 않아도 무방하며, 이는 다음과 같은 간단한 예시를 통해 간접적으로 확인하는 것이 가능하다.
```elixir
Enum.map(1..10_000_000, &(&1+1)) |> Enum.take(5) # 약 5초 소요된다.
Stream.map(1..10_000_000, &(&1+1)) |> Enum.take(5) # 즉시 값을 반환한다.
```
* 물론, 상술한 예제는 유한한 값을 다루지만 함수를 활용하여 스트림을 만드는 방식을 통해 무한한 값을 반환하는 스트림을 활용하는 것 역시 가능하다. 