...
* 애플리케이션을 제작하는 과정에서, hook 메소드를 사용할 경우 상황에 따른 알고리즘의 진행 방식을 쉽게 변경할 수 있다.
  * 이로 인해 **템플릿 메소드 패턴을 사용하는 과정에서는 hook가 필요한 경우가 반드시 발생하기 마련**이다.
...
* **할리우드 원칙은 템플릿 메소드 패턴에도 적용되며, 고수준의 개념을 다루는 템플릿 메소드는 상대적으로 저수준 개념인 서브클래스를 적시에 사용**한다.
  1. **템플릿 메소드를 포함하는 추상 부모 클래스는 작업을 처리하기 위한 알고리즘을 다루며, 필요한 경우에만 메소드 구현을 위해 서브클래스를 호출**한다.
  2. 반면, 서브클래스는 자잘한 메소드 구현을 제공하기 위해서만 사용된다.
  3. **추상 부모 클래스의 클라이언트 코드는 구체 클래스가 아닌 추상 부모 클래스 자체에 추상화된 부분에만 의존하므로, 전체 시스템의 의존성을 줄일 수 있다**.
  4. **추상 부모 클래스를 확장하는 구체 클래스는 호출당하기 전까지 절대로 추상 부모 클래스를 직접 호출하지 않는다**.

### 할리우드 원칙 주의사항
* 할리우드 원칙에는 의존성 역전 원칙과는 다음과 같은 차이가 존재한다.
  1. 할리우드 원칙: **저수준 구성 요소가 작업(컴퓨테이션)에 참여하면서도 저수준 구성 요소와 고수준 계층 간의 의존을 없애도록 구성 요소를 구축하는 기법**
  2. 의존성 역전 원칙: **구체 클래스의 사용은 가능한 한 줄이되, 추상화된 것에 의존해야 함을 강조하는 원칙**
* 이렇듯 상술한 두 원칙은 객체를 분리한다는 점에서 공통점이 존재하지만, 의존성을 지양하는 데에 있어서는 의존성 역전 원칙이 더 강하고 일반적인 내용을 강조한다.
  * 반면, 할리우드 원칙은 저수준 구성 요소를 사용할 수 있는 가능성을 열어두되 다른 클래스가 구성 요소에 너무 의존하지 않도록 하는 디자인 구현 기법이다.
* **저수준 구성 요소에서도 상속 계층 위에 있는 클래스의 메소드를 상속으로 호출할 수 있으나, 이 경우 순환 의존성이 발생하지 않도록 주의**를 기울여야 한다.

### 템플릿 메소드와 유사한 패턴
* 템플릿 메소드 패턴은 전략 패턴 또는 팩토리 메소드 패턴과 혼동되기 쉬우나, 크게 다음과 같은 차이를 보인다.
  1. 템플릿 메소드 패턴: **알고리즘을 구성하는 임의의 단계를 구현하는 방법을 서브클래스에서 결정**한다.
  2. 전략 패턴: **런타임에서 변경할 수 있는 행동 자체를 캡슐화하되, 어떤 행동을 사용할지는 서브클래스에서 결정**한다.
  3. 팩토리 메소드 패턴: **구체 클래스의 인스턴스 생성 방법을 서브클래스에서 결정**한다.

### 실무에서의 템플릿 메소드 패턴
* **템플릿 메소드 패턴은 사용 빈도가 매우 높은 축에 속하지만, 고전적인 방식과는 달리 변형된 형태를 갖는 경우가 잦다**.
  * 물론 고전적인 템플릿 메소드 패턴에만 익숙해지는 것도 좋지만, 가능하다면 숨겨진 패턴을 찾아낼 수 있는 능력을 함양하는 것이 바람직하다.
* 이러한 **템플릿 메소드 패턴은 일반적으로 프레임워크에서 자주 사용되지만, 주의 깊게 살펴보지 않으면 적용된 부분을 찾지 못할 수도 있다**.
  * 이 경우, **템플릿 메소드 패턴은 프레임워크로 작업이 처리되는 방식을 제어하면서도 알고리즘의 각 단계를 사용자가 직접 제어할 수 있도록 한다**.

## 2022-11-18 Fri
### 서브클래스를 사용하지 않는 템플릿 메소드
* Java의 Arrays 클래스의 경우, 정렬에 사용할 수 있는 정적 메소드인 sort 메소드가 제공된다.
  * **해당 메소드는 내부적으로 Comparable 인터페이스를 사용하며, 해당 인터페이스는 compareTo 메소드만을 정의**한다.
  * 이는 모든 배열에 대해 정렬을 적용할 수 있도록 정적 메소드를 사용했기 때문이며, 정적 메소드는 임의의 부모클래스에 포함되지 않으므로 확장이 불가능하다.
  * **대신에 sort는 Comparable 인터페이스를 활용하며, 이를 통해 정렬 대상 배열을 정렬할 수 있다는 점을 사전에 알아낼 수 있게 된다**. 
* 이렇듯 인터페이스를 활용하는 경우, 이를 구현하기만 하면 서브클래스를 만들 필요 없이도 템플릿 메소드 패턴의 구현이 가능하다.
  * 예를 들어, sort의 경우 Comparable 인터페이스가 정의하는 compareTo 메소드를 구현하는 것으로 정렬 알고리즘을 사용할 수 있게 된다.
  * 이러한 정렬 과정은 sort 메소드에 포함된 알고리즘에 의해 처리되며, 어떠한 클래스도 해당 과정을 수정할 수 없다.
  * 대신, sort는 Comparable 인터페이스가 제공하는 compareTo 메소드에 의존하며 이 과정에서 일반적인 템플릿 메소드에서 사용되는 상속이 적용되지 않는다.
* **이러한 방식은 상속이 아닌 구성을 사용하므로 전략 패턴과 혼동될 수 있으나, 두 패턴은 다음과 같은 점에서 분명한 차이**를 보인다.
  1. **전략 패턴은 구성에 사용되는 클래스에서 알고리즘을 완전히 구현**한다.
  2. 반면, **상술한 방식의 템플릿 메소드 패턴은 알고리즘을 불완전하게 구현하며 compareTo 메소드를 다른 클래스에서 제공**해주어야 한다.

### 실무에서의 디자인 패턴 적용
```
> 실무에서 디자인 패턴을 적용하는 방법이 항상 같을 수는 없다.
```
* **디자인 패턴을 실전에서 적용하는 경우, 항상 사전적인 방법 그대로를 사용할 수는 없다**.
  * 대신 주어진 상황과 구현 상의 제약에 적절히 맞출 수 있어야 하며, 상술한 Arrays.sort 역시 이러한 예시에 해당한다.
* 이러한 방식은 사전적인 의미에서 온전한 템플릿 메소드가 아님에도 디자인 패턴의 목표를 충실히 따를 수 있다.

### 전략 패턴과 템플릿 메소드 패턴 돌아보기
* **전략 패턴은 일련의 알고리즘 군을 정의하고, 이러한 알고리즘을 바꾸어가며 사용**할 수 있게 한다.
  * 이 때, 각각의 알고리즘은 캡슐화되므로 클라이언트는 쉽게 서로 다른 알고리즘을 사용할 수 있다.
* 반면, **템플릿 메소드는 알고리즘의 개요를 정의하며 실제 작업의 일부는 서브클래스에서 처리**한다.
  * **이를 통해 알고리즘의 각 단계마다 다른 구현을 사용할 수 있으나, 알고리즘의 전체적인 구조 자체는 유지**할 수 있게 된다.
* 전략 패턴은 알고리즘을 구현하기 위해 상속을 사용하는 대신 구성을 활용하며, 클라이언트가 알고리즘을 구현할지 말지 선택할 수 있도록 한다.
* 반면, 템플릿 메소드 패턴은 알고리즘의 구조를 관리하며 코드의 중복을 거의 없앨 수 있다.
  * **알고리즘은 전부 같으나 코드의 한 두 줄만 다른 경우에는 템플릿 메소드 패턴을 사용하는 것이 전략 패턴봐도 효율적**이다.
  * 이 경우, 중복되는 코드는 모두 부모 클래스에 포함되며 이를 서브클래스에서 공유받을 수 있다.
* **전략 패턴은 객체 구성을 사용하므로 템플릿 메소드 패턴보다 일반적으로 유연**하다.
* 반면, 템플릿 메소드 패턴은 서브클래스에서 일부 행동을 정의할 수 있게 하면서도 코드의 재사용성을 높이는 기본 메소드를 제공한다.
  * 이러한 특징 덕에 템플릿 메소드 패턴은 프레임워크를 제작하는 경우에 많이 사용된다.
* **템플릿 메소드 패턴은 알고리즘의 일부를 부모 클래스에서 구현하는 메소드에 의존하지만, 전략 패턴은 어떠한 것에도 의존하지 않고 알고리즘을 직접 구현**한다.

### 템플릿 메소드 패턴 - 결론
```
> 템플릿 메소드 패턴은 알고리즘의 틀을 정의하며, 이를 통해 알고리즘의 일부 단계를 서브클래스에서 구현하도록 할 수 있다.
> 때문에 알고리즘의 구조는 그대로 유자하되, 알고리즘의 특정 단계만을 서브클래스에서 재정의할 수 있다.
```
* **템플릿 메소드 패턴은 알고리즘의 각 단계를 정의하는 템플릿 메소드를 활용하며, 일부 단계는 서브클래스에서 구현하도록 떠넘길 수 있는 디자인 패턴**이다.
* 템플릿 메소드 패턴은 코드의 재사용성에 큰 도움을 준다.
* 템플릿 메소드를 포함하는 추상 부모 클래스는 구체 메소드와 추상 메소드 및 hook 메소드를 정의할 수 있다.
  * **추상 메소드는 서브클래스에서 구현하는 반면, 아무 것도 하지 않거나 기본 동작을 정의하는 hook 메소드는 필요한 경우에만 서브클래스에서 재정의**한다.
* 템플릿 메소드 패턴은 할리우드 원칙이 적용된 좋은 예시이다.
  * 이 때, **할리우드 원칙이란 저수준 모듈이 어느 시점에 어떻게 사용될지는 고수준 모듈에서 결정되어야한다는 점을 강조하는 원칙**이다.
* **템플릿 메소드는 실무에서도 자주 사용되는 유용한 패턴이지만, 반드시 정석적인 형태로만 사용되는 것은 아니다**.
* **전략 패턴과 템플릿 메소드 패턴은 모두 알고리즘을 캡슐화한다는 점에서 공통점이 있으나, 각각 구성과 상속을 사용한다는 점에서 차이**를 보인다.
  * 반면, **팩토리 메소드 패턴은 일종의 특화된 형태의 템플릿 메소드 패턴**으로 볼 수 있다.