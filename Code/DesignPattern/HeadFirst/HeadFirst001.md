...
### 공통된 용어로서의 디자인 패턴
```
> 실무자 사이에서 복잡한 업무적 내용을 간단한 단어로 축약할 수 있다면 의사소통은 더욱 쉬워질 수 있다.
```
* **디자인 패턴은 모든 개발자가 이해할 수 있는 용어를 제공하며, 이를 통해 복잡한 패턴을 장황하게 설명하지 않고서도 간단하게 소통할 수 있도록 한다**.
  * 이렇듯 디자인 패턴을 이해하는 경우 다른 개발자에게 현재 내가 생각하고 있는 복잡한 디자인을 빠르고 정확하게 전달할 수 있어 훨씬 효율적인 소통이 가능하다.
  * 또한, **객체 수준에서의 생각을 패턴 수준에서의 고차원적인 사고로 끌어올리므로 아키텍쳐를 설계하는 수준까지도 견인**할 수 있다.

### 패턴으로 소통하는 경우의 이점
* 디자인 패턴으로 소통할 경우, 구구절절한 이야기를 할 때와 비교하여 다음과 같이 효율적인 의사소통이 가능하다.
  1. 패턴 이름만으로 패턴의 내용은 물론 특성과 제약 조건 등을 함께 의논할 수 있다.
  2. **패턴을 활용하는 것으로 객체와 클래스를 구현하는 것과 관련된 자잘한 내용에 시간을 허비하지 않고, 디자인 수준에서 초점을 유지**할 수 있다.
  3. 패턴을 팀원 모두가 잘 이해하는 경우, 오해의 소지를 줄이고 작업을 빠르게 진행할 수 있다.

### 디자인 패턴과 라이브러리
* 라이브러리와 프레임워크는 적절히 사용했을 때 막강한 결과를 얻을 수 있으나, 이해하기 쉽고 유연한 애플리케이션의 구조를 설계하는 데에 도움을 줄 수는 없다.
  * 이렇듯 **디자인 패턴은 더 높은 단게에 속하며, 라이브러리와 프레임워크가 직접적으로 도움을 줄 수 없는 부분을 보조하는 것이 디자인 패턴에 해당**한다.
* 때문에 **디자인 패턴을 이해한 후에 적절히 사용할 수 있다면, 어떤 코드가 왜 유연성이 떨어지는지 빠르게 이해하고 패턴을 도입하여 손쉽게 개선**할 수 있다. 
* 관리하기 쉬운 객체지향 시스템을 만드는 방법 중 하나는 추후에 애플리케이션이 어떻게 성숙하고 변화할지 생각하는 것이며, 디자인 패턴은 이러한 내용을 포함한다.

### 객체지향과 디자인 패턴의 오해
```
> 객체지향의 특징과 기본만을 이해하면 유연하고 유지보수성이 높은 시스템을 쉽게 제작할 수 있다는 것은 흔히 알려진 객체지향의 오해에 불과하다.
```
* **유연하고 유지보수성이 높은 객체지향 애플리케이션을 구축하기 위해서는 상당한 노력을 기울여야 하며, 디자인 패턴에 대해 이해하는 것이 필수적**이다.
  * 이 때, **디자인 패턴은 쉽지 않은 여러 객체지향 시스템 구축 방법을 여러 개발자들의 경험을 기반으로 모아둔 사례**로 이해할 수 있다.
* 물론 **디자인은 결국 예술이므로, 장점과 더불어 명확한 단점 역시 존재**한다.
  * 그러나 **많은 개발자들이 오랜 기간 고민하여 정의한 디자인 패턴을 잘 따를 수 있다면 그렇지 못한 경우보다 훨씬 좋은 디자인을 만들 수 있게 된다**.
...
1. 바뀌는 부분을 캡슐화한다.
2. 상속보다는 구성을 활용한다.
3. 구현보다는 인터페이스에 맞추어 개발을 진행한다.
```
1. 객체지향의 기초적인 지식만으로는 훌륭한 객체지향 개발자가 될 수 없다.
2. 좋은 객체지향 디자인은 재사용성과 확장성, 유지보수성을 갖추어야 한다.
3. 패턴은 이미 검증된 객체지향 개발 경험의 산물이다.
4. **패턴은 코드를 바로 제공하지 않는 대신 디자인 문제의 보편적인 해법을 제공하므로, 애플리케이션에 적절한 패턴을 찾아 적용하는 것은 개발자의 역할**이다.
5. 대부분의 패턴과 디자인 원칙은 애플리케이션의 변화와 관련된 문제와 연관되며, 애플리케이션의 일부를 나머지와 무관하게 변경하는 방법을 제공한다.
   * 이를 위해서는 **애플리케이션에서 변화할 가능성이 있는 부분을 찾아 캡슐화할 수 있어야 한다**.