...
### 상태 패턴과 전략 패턴의 차이
```
> 상태 패턴과 전략 패턴이 구성과 위임을 사용하는 점에서는 공통점이 있으나, 클라이언트가 구성과 위임을 사용하도록 하는 방법은 완전히 다르다.
> 상태 패턴은 몇 가지 상태를 정해둔 후 미리 정해진 상태 전환 규칙에 따라 알아서 상태를 변경하고, 결과적으로 동작이 변경된다.
> 전략 패턴은 객체 스스로 상태를 변경하는 것을 권장하지 않으며, 대신 클라이언트가 어떠한 행동을 구현하는 전략 객체를 전달해야 한다.
```
...
* 때로는 코드의 중복성을 줄이기 위해 하나의 구체 상태 클래스에 여러 기능을 포함시키고 싶을 수 있으나, 이는 단일 책임 원칙을 위배하기 쉬운 결정일 수 있다.
  * 이 역시도 코드의 중복성을 줄이는 것과 구체 상태의 책임을 명확히 하는 점에서의 트레이드오프로 볼 수 있다.
  * 때문에 **여러 장단점을 고려한 후, 상황에 따라 가장 적절한 방식을 선택하는 것이 바람직**하다.

## 2022-11-26 Sat
### 상태 패턴 적용 시 고려사항
* 상태 패턴을 적용하는 경우, 다음과 같은 내용을 고려하여 상황에 맞는 결정을 내리는 것이 바람직하디.
  1. State를 인터페이스로 구현할 것인가, 추상 클래스로 구현할 것인가?
  2. 상태 전이 코드를 Context와 ConcreteState 중 어느 곳에 구현할 것인가?
  3. Context 객체 자체가 여럿이 존재할 수 있는가? 
* 특히 3.의 경우 **여러 객체가 존재할 수 있다면, 구체 상태 객체를 정적 인스턴스 변수로 정의하는 것이 바람직**하다.

### 상태 패턴 - 결론
```
> 상태 패턴은 내부 상태가 변경됨에 따라 객체의 행동을 변경하므로 마치 객체의 클래스가 변경된 것 같은 효과를 준다.
```
* 상태 패턴은 각각의 상태를 별도의 클래스로 캡슐화하며, 내부 상태를 바탕으로 서로 다른 행동을 수행할 수 있도록 한다.
  * 이 때, **Context 객체는 내부에서 참조하는 현재 상태 객체에게 행동을 위임하는 식으로 동작**한다.
  * 또한, **각 상태가 별도의 클래스로 캡슐화되므로 추후 변경 사항을 국지화할 수 있게 된다**.
  * 때문에 상태 패턴은 클래스에서 상태를 관리해야하는 경우에 유용하게 사용될 수 있다.
* 상태 패턴과 전략 패턴은 동일한 클래스 다이어그램을 사용하는 등 공통점이 많지만, 그 용도는 완전히 다르다.
  * **상태 패턴은 Context의 내부 상태 전이에 따라 객체가 알아서 행동을 변경할 수 있도록 할 수 있다**.
  * 반면, **전략 패턴은 Context 클래스를 작성하는 과정에서 행동과 알고리즘을 결정**하게 된다.
* 일반적으로 상태 전이와 관련된 코드는 구체 상태 클래스에 구현하지만, Context 클래스에 구현할 수도 있다.
* **상태 패턴을 적용하는 경우 각각의 상태를 개별 클래스에 캡슐화하므로 디자인에 필요한 클래스의 개수가 늘어나게 된다**.
  * 때문에 **환경에 따라 Context 객체가 여럿이 존재할 수 있는 경우, 각 구체 상태 클래스를 여러 Context 인스턴스에서 공유하도록 만들 수 있다**.