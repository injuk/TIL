...
### 컴포지트 패턴 구현의 주의사항
* **우선 컴포넌트를 인터페이스 또는 추상 클래스로 구현한 후, 각각 리프 노드와 복합 객체 역할을 수행할 클래스가 이를 확장하도록 구현**한다.
  * 때문에 임의의 복합 객체에 또 다른 복합 객체나, 리프 노드가 공통의 인터페이스를 구현하기만 한다면 마음대로 추가하여 계층 구조를 풍부하게 할 수 있다.
* **컴포넌트는 리프와 복합 객체 모두가 구현해야 할 인터페이스가 되지만, 리프와 복합 객체는 각각 역할이 달라 모든 상황에 맞는 메소드를 정의할 수는 없다**.
  * 때문에 리프와 복합 객체 모두에 대응할 수 있는 메소드를 정의하되, 추상 클래스로서 기본 구현으로 예외를 던지도록 작성할 수 있다.
  * 이 경우, **리프와 복합 객체는 해당 추상 클래스를 상속받는 과정에서 자신에게 맞는 메소드만을 재정의하여 구현**될 수 있다.
* **컴포지트 패턴이 적용된 복합 계층 구조를 사용하는 클라이언트 역시, 모든 구성 요소를 참조하는 대신 최상위 복합 객체만을 참조하도록 작성**될 수 있다.
* **컴포지트 패턴에서는 복합 객체와 리프 노드에서 달리 동작하는 메소드가 존재할 수 있으며, 이에 따라 투명성과 안정성이 반비례**한다.
  1. 복합 객체에서는 존재하지만 리프 노드에서는 메소드를 정의하지 않는 경우, 안전성은 높아지지만 투명성은 낮아진다.
  2. 둘 모두에 메소드를 정의하되 리프 노드에서는 예외를 던질 경우, 안전성은 떨어지지만 투명성은 높아진다.
* **투명성이란, 클라이언트가 자신이 상호작용하는 대상이 복합 객체인지 또는 리프 노드인지 전혀 몰라도 되는 경우에 높아진다**.
  * 반면, **안전성은 인터페이스 준수를 위해 의미 없는 메소드가 추가되는 경우에 낮아진다**.
  * 이렇듯 **객체지향 디자인에서는 언제나 장점과 단점을 잘 따져봐야 하며, 복합 객체 작성시에도 상황에 따라 안전성과 투명성을 조절**해야 한다.

## 2022-11-23 Wed
### 컴포지트 패턴과 단일 책임 원칙
* **컴포지트 패턴에서는 단일 클래스에서 계층 구조를 관리하는 작업과 개별 복합 객체의 작업을 모두 처리하게 되므로, 단일 책임 원칙을 위배**하게 된다.
  * 그러나 **컴포지트 패턴은 단일 책임 원칙을 의도적으로 위배하는 대신, 복합 객체와 리프 노드를 같은 방식으로 처리할 수 있도록 투명성을 확보**한다.
* 컴포지트 패턴을 구성하는 Component는 두 종류의 기능을 담당하므로 상대적으로 안전성이 떨어질 수 있으나, 이는 디자인 상의 결정 사항으로 판단할 수도 있다.
* 이렇듯 **디자인 패턴과 디자인 원칙은 무분별하게 따르기보다는 상황에 따라 원칙을 적절히 적용하는 것이 바람직**하다.
  * 디자인 원칙에서 제시하는 가이드를 무작정 따를 수도 있으나, 그보다는 원칙이 디자인에 어떠한 영향을 줄지 항상 고민한 후에 원칙을 적용하는 것이 바람직하다.
  * 즉, **디자인 원칙에 위배되는 방식으로 디자인을 적용해야하는 경우도 얼마든지 발생할 수 있다**.

### 컴포지트 패턴 정리하기
```
> 컴포지트 패턴은 여러 객체를 모아서 관리할 때 매우 유용하다. 
> 컴포지트 패턴은 복잡한 객체 구조에서 올바른 객체에 대한 올바른 메소드 호출을 확인하기 위해 코드를 지저분하게 만들 필요가 없게 한다.
```
* 컴포지트 패턴은 부분 - 전체 계층 구조를 갖는 객체 컬렉션에서, 각 객체를 모두 같은 방식으로 다루고자하는 경우에 사용될 수 있다.
  * 객체를 같은 방식으로 다루므로 복합 객체와 리프 노드 모두에 대해 같은 메소드를 호출하게 되고, 이를 위해 모든 객체의 인터페이스는 동일해야 한다.
  * 물론, **모든 객체의 인터페이스를 통일시키는 과정에서 어떤 객체에는 아무런 의미 없는 메소드가 포함될 수도 있으나 이는 자연스러운 현상**이다.
* 객체에 따라 정의될 수 있는 아무 의미 없는 메소드는 그럴싸한 기본 구현을 정의하거나 null 또는 false를 반환할 수 있다.
  * 또는 예외를 던지도록 하되, 클라이언트가 이에 대비하도록 강제할 수도 있다.
* **컴포지트 패턴은 일반적으로 최상위 복합 객체를 뿌리로 시작하는 트리 형태로 구성되며, 자식 노드는 모두 복합 객체 또는 리프 노드가 된다**.
* **컴포지트 패턴은 용도에 따라 자식 노드의 순서나 부모 노드를 가리키는 포인터, 또는 캐싱 등 고려해야할 것이 매우 많은 패턴**에 속한다.
  * 그러나 **컴포지트 패턴을 통해 복잡한 클라이언트를 단순화할 수 있으며, 여기저기에 객체의 종류를 확인하기 위한 if 문을 작성하지 않을 수 있다**.

### 반복자 패턴 - 결론
```
> 반복자 패턴은 컬렉션의 구현 방법을 노출하지 않고도 클라이언트가 집합체 내부의 모든 요소에 접근할 수 있는 방법을 제공한다.
```
* **반복자 패턴을 활용하는 것으로 집합체를 대상으로 하는 반복 작업을 별도의 객체로 캡슐화하여 분리**할 수 있다.
* **반복자 패턴을 통해 반복 작업 자체에 대한 인터페이스를 통일시킬 수 있으므로, 집합체 내부의 객체를 활용하는 코드를 보다 다형적으로 작성**할 수 있다.

### 컴포지트 패턴 - 결론
```
> 컴포지트 패턴은 객체를 트리 구조로 구성하여 부분 - 전체 계층 구조를 구현한다.
> 컴포지트 패턴을 통해 클라이언트로부터 개별 객체와 복합 객체를 같은 방법으로 다룰 수 있게 된다.
```
* 컴포지트 패턴은 개별 객체와 복합 객체를 모두 담을 수 있는 하나의 구조를 제공하며, 이를 모두 같은 방법으로 다룰 수 있도록 한다.
  * 이 때, 복합 구조에 포함되는 것을 구성 요소라고 한다.
  * 또한, 구성 요소는 다시 복합 객체와 리프 노드로 구분할 수 있다.
* **컴포지트 패턴을 적용하는 경우, 객체 투명성과 안정성 사이에서 적절한 균형을 찾는 등 여러 장단점을 반드시 고려**해야 한다.