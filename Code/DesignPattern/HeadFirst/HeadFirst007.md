...
> 객체 지향에서, 임의의 객체가 의존하거나 연관되는 객체는 가능한 한 적을수록 좋다. 
```
...
* 반면, 최소 지식 원칙에는 다음과 같은 단점 역시 존재한다.
  1. 메소드 호출을 처리하는 래퍼 클래스를 더 작성해야 한다.
  2. **더 많은 래퍼 클래스로 인해 시스템은 복잡해지고, 개발 시간은 늘어나며 성능 역시 함께 떨어질 수 있다**.

### 최소 지식 원칙 준수하기
* 최소 지식 원칙을 준수하기 위해 가능한 한 다음의 가이드라인에서 명시된 객체와만 상호작용하는 것이 바람직하다.
  1. 객체 자체
  2. 메소드에 매개변수로 전달된 객체
  3. 메소드를 생성하거나, 인스턴스를 만들어낸 객체
  4. 객체에 속하는 구성 요소
* 이러한 가이드라인을 준수할 경우, 다른 객체가 알고 있는 또 다른 객체에 요청하기보다는 다른 객체가 대신 요청하도록 만들어 나갈 수 있다.
  * 이를 토대로 직접적으로 연관되어 의존하는 객체의 수를 줄이고, 결합도를 낮출 수 있다.

### 디미터의 법칙
```
> 디미터의 법칙은 최소 지식 원칙과 완전히 같다.
```
* 디미터의 법칙은 법칙이라는 명칭으로 인해 마치 반드시 준수해야할 것 같은 뉘앙스를 줄 수 있으나, 엄밀히 말해 어떠한 원칙도 법칙이라고 볼 수는 없다.
  * **각각의 디자인은 장단점이 존재하므로, 오히려 모든 원칙은 상황에 따라 모든 요인을 따져본 후에 적절하게 준수하는 것이 바람직**하다.

### 파사드 패턴과 최소 지식 원칙
* 파사드 패턴이 적용된 클래스를 사용하는 클라이언트 코드는 파사드 클래스와 연관되는 것으로 다른 서브시스템들과의 결합도는 낮아진다.
* 이 경우 파사드 클래스는 클라이언트 대신 모든 서브시스템 구성 요소를 관리하므로, 클라이언트 코드는 단순함을 유지하면서도 유연할 수 있다.
  * 예를 들어, 파사드 클래스의 구성 요소가 수정되더라도 클라이언트 코드는 아무런 영향을 받지 않을 수 있다.
  * 반면, 서브시스템에서도 최소 지식 원칙을 최대한 준수하는 것이 바람직하다.
* **임의의 객체가 다른 객체와 너무 깊이 연관되어 시스템이 복잡해진 경우, 파사드 패턴의 도입을 고려**할 수 있다.

### 어댑터와 파사드 패턴 - 결론
```
> 어댑터와 파사드 패턴은 인터페이스를 변경하거나, 클라이언트와 서브시스템간의 결합을 더 느슨하게 만들어준다.
```
* 어댑터 패턴과 파사드 패턴은 모두 인터페이스를 변환하지만, 다음과 같은 차이점이 존재한다.
  1. 어댑터 패턴은 인터페이스를 변환하여 인터페이스 호환성 문제로 함께 사용할 수 없었던 클래스를 혼용할 수 있도록 돕는다.
  2. 파사드 패턴은 서브시스템에 위치한 일련의 인터페이스를 통합 인터페이스로 묶어주며, 이로 인해 고수준 인터페이스를 정의하여 사용성을 높여준다.
* 즉, 기존 클래스를 사용하는 과정에서 인터페이스 호환 문제가 발생하면 어댑터 패턴을 고려할 수 있다.
  * 반면, 커다랗거나 많은 인터페이슬르 단순화하거나 통합하는 경우에는 파사드 패턴을 고려할 수 있다.
  * 이 경우, 파사드 패턴의 실제 작업은 파사드 클래스에 구성되는 서브시스템에 위임된다.
* **어댑터 패턴은 인터페이스를 클라이언트가 원하는 형태로 변환하지만, 파사드 패턴은 클라이언트를 복잡한 서브시스템과 분리하는 역할을 수행**한다.
* 어댑터 패턴과 데코레이터 패턴 및 파사드 패턴은 객체를 감싸는 공통점이 존재하여 혼동될 수 있으나, 다음과 같은 차이점이 존재한다.
  1. 어댑터 패턴은 객체를 감싸 인터페이스를 변환하기 위해 사용된다.
  2. 데코레이터 패턴은 객체를 감싸 새로운 행동을 추가하기 위해 사용된다.
  3. 파사드 패턴은 일련의 객체 집합을 감싸 단순화하기 위해 사용된다.