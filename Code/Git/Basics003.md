...
## 2023-06-24 Sat
### pull 과정에서 리베이스를 적용하기
* `git pull --rebase` 옵션을 활용하여 리베이스할 수도 있으며, 이는 페치와 리베이스를 순차적으로 적용하는 것과 같이 동작한다.
* 리베이스는 작업 내용을 푸시하기 전이나, 혼자 작업하는 프로젝트에서는 문제를 일으키지 않는다.
  * 그러나 **이미 공개하여 다른 개발자들과 함께 사용하는 커밋을 리베이스하면 반드시 문제가 생길 수 밖에 없다**.
  * 때문에 협업하는 프로젝트에 리베이스를 적용하고 싶은 경우, 협업자들에게 공유하여 `git pull --rebase` 옵션을 적극적으로 사용하는 것이 바람직하다.

## 2023-06-25 Sun
### 리베이스와 머지 비교하기
```
> 히스토리란 본디 작업한 내용을 기록하는 역사이며, 역사는 후세를 위해 기록하고 보존되어야 한다.
```
* 히스토리는 작업 내역이나 프로젝트의 진행 흐름을 보여줄 수 있으며, 이러한 관점에서 두 방식 중 사용할 것을 신중히 결정해야 한다.
  * 이러한 각각의 관점에서는 커밋 히스토리를 변경하는 것도, 지저분한 머지 커밋을 남기는 것도 저마다의 단점으로 해석될 수 있다.
* **리베이스나 머지 중 어느 것이 나은지에 대한 이야기는 답이 쉽지 않으므로, 프로젝트나 팀의 성향에 맞추어 결정하는 것이 바람직**하다.
* 그럼에도 일반적으로는 **로컬 브랜치에서 작업할 때에만 리베이스를 사용하되, 푸시한 커밋에 대해서는 절대 리베이스하지 않는 것을 기조**로 삼을 수 있다.