...
* 두 파일에 대한 수정 내역을 별도의 커밋으로 관리해야 하는 상황에서, 두 파일을 모두 Staging area에 삽입했다면 `git reset` 명령을 활용할 수 있다.
  * 예를 들어, `git reset HEAD [파일명]` 명령을 입력하여 임의의 파일을 Unstaged 상태로 변경할 수 있다.
* 이 때, **`git reset` 명령을 --hard 옵션과 함께 사용한다면 워킹 디렉토리의 파일까지 함께 수정됨에 주의**를 기울여야 한다.
  * 그러나 --hard 옵션을 명시하지 않은 경우, `git reset` 명령은 기본적으로 Staging area의 파일만을 조작하므로 안전하다.

## 2023-05-26 Fri
### Modified 상태의 파일을 되돌리기
* 임의의 파일을 수정했을 때, 최큰 커밋된 버전으로 되돌리고 싶은 상황에서는 `git checkout -- [파일명]` 옵션을 활용할 수 있다.
  * 이 때, 이러한 옵션은 해당 파일에 대해 진행한 워킹 디렉토리 상의 모든 수정 내역을 버리는 방식으로 동작한다.
  * 때문에 해당 옵션은 위험한 명령이며, 최근 커밋으로 파일을 덮어 씌우기에 수정한 내용이 모두 정말로 마음에 들지 않을 때에만 사용하는 것이 바람직하다.
* 상술했듯 **깃은 그 특성 상 커밋하지 않고 버린 것을 절대로 되돌릴 수 없으므로, 해당 방식보단 `stash`나 `branch`를 활용하는 것이 바람직**하다.

## 2023-05-27 Sat
### 원격 저장소 활용하기
* 깃으로 협업한다는 것은 곧 인터넷 또는 네트워크 상의 원격 저장소를 관리해나가며, 해당 장소에 데이터를 `push`하거나 `pull`하는 것을 의미한다.
  * 이 때, 원격 저장소의 관리란 리포지토리 자체에 대한 관리에 브랜치 관리를 포함한다.
* **임의의 프로젝트에 등록된 원격 저장소는 `git remote` 명령어로 확인할 수 있으며, 클론된 프로젝트의 경우 origin 원격 저장소가 자동으로 등록**된다.
  * 해당 명령은 원격 저장소의 단축된 이름을 출력하며, URL이 포함된 더 자세한 내용을 보고 싶은 경우에는 `git remote -v` 옵션을 활용할 수 있다.
  * 이 때, 해당 명령은 원격 저장소가 여러 곳에 존재하는 프로젝트의 경우 등록된 모든 원격 저장소를 출력하므로 다른 개발자의 컨트리뷰션을 쉽게 가져올 수 있다.
```shell
[~] git remote
origin
[~] git remote -v
origin  https://github.com/injuk/TIL.git (fetch)
origin  https://github.com/injuk/TIL.git (push)
[~] 
```
* 반면, 원격 저장소를 클론하기 위한 `git clone` 명령은 자동으로 원격 저장소를 `origin`이라는 이름으로 추가한다.
  * 나아가 클론이 완료된 후에는 자동으로 로컬 저장소의 master 브랜치가 원격 저장소의 master 브랜치를 추적하도록 한다.

## 2023-05-28 Sun
### 새로운 원격 저장소 추가하기
* `git remote add [단축_이름] [깃_URL]` 명령을 입력하는 것으로 프로젝트에 새로운 원격 저장소를 추가할 수 있다.
  * 이러한 **명령어를 입력한 이후에는 깃 URL 대신 단축된 이름을 토대로 원격 저장소와 상호 작용하는 것이 가능**해진다.
  * 예를 들어, 임의의 깃 저장소에 대한 단축된 이름이 `temp`라면 `git fetch temp` 와 같은 형태의 명령어로 원격 저장소의 데이터를 가져올 수 있다.
* 새로이 원격 저장소를 추가한 후 `git fetch` 등의 명령어로 데이터를 가져온 후에는 로컬 브랜치에 머지하거나 체크아웃하여 데이터를 활용할 수 있다.

## 2023-05-29 Mon
### 원격 저장소의 데이터를 가져오기
* `git fetch` 명령과 `git pull` 명령은 모두 원격 저장소의 데이터를 로컬 저장소로 가져오는 역할을 수행하지만, 다음과 같은 차이가 존재한다.
  1. fetch: **원격 저장소의 데이터를 모두 로컬 저장소로 가져오지만, 자동으로 머지하지는 않으므로 개발자가 로컬 작업을 정리한 후 직접 머지**해야 한다.
  2. pull: **원격 저장소의 데이터를 모두 로컬 저장소로 가져온 후, 자동으로 로컬 브랜치와 머지**시킨다.

## 2023-05-30 Tue
### 원격 저장소에 데이터를 밀어넣기
* 협업 과정에서 다른 사람들과 프로젝트를 공유하기 위한 원격 저장소를 Upstream 저장소로 표현할 수 있으며, 자신의 작업물을 이에 밀어넣을 수 있다.
  * 이 경우, `git push [원격_저장소_이름] [브랜치_이름]` 명령을 활용할 수 있다.
...
## 2023-06-01 Thu
### 원격 저장소의 이름 변경과 삭제
* `git remote rename [현재_단축된_이름] [변경할_단축된_이름]` 명령을 활용하여 원격 저장소의 이름을 변경할 수 있다.
  * 이 경우, 명령어를 입력한 이후에는 원격 저장소의 브랜치 명 역시 `[변경할_단축된_이름]/[브랜치_이름]`으로 참조해야 한다.
* 나아가 원격 저장소의 정보가 변경되거나 더 이상 기여하지 않고자 하는 경우, `git remote rm [단축된_이름]` 명령을 통해 원격 저장소를 제거할 수 있다.

## 2023-06-02 Fri
### 깃 태그 활용하기
* 깃은 여타 VCS와 마찬가지로 태그 기능을 지원하며, 일반적으로 태그는 버전 별 릴리즈를 위해 사용된다.
* 태그를 조회하고자 하는 경우, `git tag` 명령을 입력하는 것으로 프로젝트에 할당된 태그를 확인할 수 있다.
  * 나아가 임의의 태그를 검색하고자 하는 경우, `git tag -l '[검색어]'` 옵션을 활용할 수 있다.
* 또한, 깃의 태그는 크게 다음과 같은 두 종류로 분류될 수 있다.
  1. Lightweight: 단순히 특정 커밋에 대한 포인터 역할만을 수행한다.
  2. Annotated: 깃이 관리하는 데이터베이스에 태그 작성자와 이메일, 태그 할당일 및 태그 메시지 등의 정보를 저장한다.
* 때문에 꼭 입력해야 할 정보가 있는 경우에는 Annotated 태그를 활용하되, 그 이외의 경우에는 단순한 태그인 Lightweight 태그를 활용하는 것이 권장된다.
* Annotated 태그의 경우 `git tag -a [태그명]` 옵션에 더해 -m 옵션을 추가하여 태그에 메시지를 함께 저장할 수 있다.
  * 이렇게 저장된 태그는 `git show [태그명]` 명령을 활용하여 태그 및 커밋 정보를 확인할 수 있다.
* Lightweight 태그의 경우 아무런 옵션 없이 `git tag [태그명]` 명령어를 사용하며, 이는 기본적으로 커밋 체크섬만을 저장한다.
  * 때문에 `git show [태그명]` 명령을 활용하더라도 별도의 태그 정보는 확인할 수 없으며, 오로지 커밋 정보만을 확인할 수 있게 된다.
* 미처 **태깅하지 몫한 예전 커밋을 태그하고자 하는 경우, 해당 커밋의 체크섬에 대해 `git tag [태그명] [커밋_체크섬]` 명령을 활용**할 수 있다.

## 2023-06-03 Sat
### 깃 태그 공유하기
* 로컬 프로젝트에서 생성한 태그는 기본적인 `git push` 명령으로는 원격 저장소에 저장되지 않으므로, 이를 다른 개발자와 공유할 수 없다.
  * 대신 `git push origin [태그명]` 명령을 입력하여 생성해둔 태그를 원격 저장소에 push해주어야 한다.
  * 나아가 로컬 프로젝트에서 생성한 태그가 많은 경우, `git push origin --tags` 옵션을 활용하여 원격 저장소에 없는 태그를 모두 전송할 수도 있다.
* 또한 공유된 태그의 경우, 다른 개발자가 원격 저장소를 클론하거나 pull 하는 것으로 태그 정보를 공유받아 사용할 수 있다.

## 2023-06-04 Sun
### 태그와 브랜치의 차이
* 일견 태그는 브랜치와 유사한 것처럼 보일 수 있으나, 다음과 같은 결정적인 차이점이 존재한다.
  1. 태그는 브랜치와 달리 태그가 가리키는 커밋을 변경할 수 없다.
  2. 태그를 기반으로 체크아웃할 수 없으며, 대신 태그가 가리키는 커밋에 대해 `git checkout -b [브랜치명] [태그명]` 명령으로 새 브랜치를 생성해야 한다.

## 2023-06-05 Mon
### 깃 alias 활용하기
* `git config --global alias.co checkout`과 같은 옵션을 활용하는 것으로 깃의 긴 명령어를 짧은 단축어로 사용할 수 있다.
  * 이러한 기능은 깃의 사용성을 더 높여줄 수 있으므로, 자주 사용하는 명령어와 옵션에 대해서는 적극적으로 alias를 적용하는 것이 권장된다.
* 덧붙여 **`git config --global alias.vis '!gitk'`와 같이 ! 기호를 명시하는 것으로 외부 명령을 실행할 수도 있다**.