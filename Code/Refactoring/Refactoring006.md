...
## 2022-04-19 Tue
### 위임 숨기기
* 모듈화 설계의 핵심은 캡슐화이며, 이는 동시에 가장 중요한 요소이기도 하다.
* 캡슐화는 각 모듈이 시스템의 다른 부분에 대해 반드시 알아야 하는 점을 줄여준다.
  * 때문에 **캡슐화가 잘 적용되어 있다면 무언가를 변경해야만 하는 경우 함께 고려해야하는 모듈의 수가 적어지므로 변경이 쉬워질 수 있다**.
* 객체의 필드가 가리키는 객체의 멤버를 호출하려면 클라이언트는 반드시 이 객체도 알고 있어야 한다.
  * 예를 들어, a.b.c()와 같은 형태에서 클라이언트 코드는 반드시 b 객체의 존재를 알고 있어야 하며 명시적으로 호출해야 한다.
  * 이러한 방식은 변경에 취약하며, 위임 객체 b의 인터페이스가 변경되면 a를 사용하는 모든 클라이언트 코드의 수정이 필요하다.
* **위임 객체에 대한 의존성을 없애기 위해 서버 객체 자체에 위임 메소드를 정의하는 것으로 위임 객체를 클라이언트로부터 숨길 수 있다**.
  * 이를 통해 위임 객체가 수정되더라도 서버 객체 이외의 수정을 필요로하지 않을 수 있게 되며, 클라이언트는 어떠한 영향도 받지 않도록 할 수 있다.

### 위임 숨기기 - 절차
1. 위임 객체의 각 메소드에 해당하는 위임 메소드를 서버 객체에 정의한다.
2. 클라이언트가 위임 객체 대신 서버 객체를 호출하도록 수정하되, 하나 씩 바꿀 때마다 테스트를 진행한다.
3. 모두 수정한 후, 서버로부터 위임 객체를 얻어내는 접근자를 제거한다.
4. 테스트를 진행한다.

### 중개자 제거하기
* **위임 숨기기 기법을 통해 위임 객체를 캡슐화하여 이점을 얻을 수 있지만, 이는 적절한 균형을 잡는다는 전제가 필요**하다.
  * 예를 들어, 클라이언트가 위임 객체의 또 다른 기능을 사용할 때마다 위임 메소드를 추가하는 경우를 생각할 수 있다.
* **위임 객체의 기능이 충분히 많은 경우, 서버 객체는 단순히 메시지를 전달하기만 하는 중개자로 전락하는 상황이 발생할 수 있다**.
  * 이러한 상황에서는 클라이언트가 위임 객체의 존재를 아는 것이 좋을 수 있으며, 특히 디미터의 법칙을 맹신할 때 쉽게 발생할 수 있다.
  * **디미터의 법칙은 유용하지만, 그 과정에서 위임 또는 래퍼 메소드가 너무 늘어나는 부작용이 있을 수 있으므로 상황에 맞게 응용하는 것이 중요**하다.
* 명확히 어느 정도 선까지만 숨기되, 그 이후로는 공개하는 기준은 존재하지 않는다.
  * 대신 **필요할 때 언제든지 상황에 맞는 균형점으로 옮길 수 있도록 위임 숨기기와 중개자 제거하기 기법을 활용할 수 있다**. 
  * 또는 두 기법을 적절히 섞어 사용해도 무방하며, 자주 쓰이는 위임은 그대로 두는 것이 클라이언트 입장에서의 편의성은 오히려 높아진다.

### 중개자 제거하기 - 절차
1. 위임 객체를 얻을 수 있는 게터를 정의한다.
2. 위임 메소드를 호출하는 클라이언트가 모두 게터를 거치도록 수정하되, 하나씩 바꿀 때마다 테스트를 진행한다.
3. 모든 수정이 완료된 후에 위임 메소드를 제거한다.

### 알고리즘 교체하기
* 임의의 목적을 달성하는 방법은 여럿이 있으며, 그 중에서도 다른 것보다 쉬운 방식은 분명히 존재한다.
* 더 쉬운 방식을 찾아내었다면 반드시 상대적으로 복잡한 기존 코드를 쉬운 방식으로 변경하는 것이 바람직하다. 
  * 리팩토링은 복잡한 대상을 단순한 단위로 나눌 수 있도록 하지만, 때로는 알고리즘 전체를 간결한 것으로 교체해야하는 경우가 생기기 마련이다.
  * 또는 알고리즘에 살짝 변화를 주고 싶은 경우에도 변경에 유연한 알고리즘으로 바꾼 후에 처리하는 것이 편할 수 있다.
* **해당 리팩토링 기법은 반드시 메소드가 가능한 한 잘게 나누어져 있어야한다는 사실을 전제**한다.
  * 거대하고 복잡한 알고리즘을 교체하기란 상당히 어려운 작업이므로, 알고리즘을 간소화하는 작업부터 적용하는 것이 교체 작업을 더 쉽게 만들어줄 수 있다.

### 알고리즘 교체하기 - 절차
1. 교체할 코드를 하나의 함수에 모아둔다.
2. 해당 함수만을 사용하여 동작을 검증하는 테스트 코드를 작성한다.
3. 대체할 알고리즘을 준비한 후 정적 검사를 수행한다.
4. 기존 알고리즘과 새로운 알고리즘의 결과를 비교하는 테스트를 수행한다.
   * 두 알고리즘의 결과가 같다면 리팩토링을 종료한다.
   * 결과가 같지 않은 경우, 기존 알고리즘을 참고하여 신규 알고리즘을 테스트하고 디버깅을 수행한다.