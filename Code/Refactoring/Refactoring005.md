...
### 단계 쪼개기
* 서로 다른 두 대상을 한 번에 다우는 코드는 각각 별개의 모듈로 나누는 것을 고려할 수 있다.
  * 이를 통해 두 대상을 동시에 생각할 필요 없이 하나에만 집중할 수 있다.
* **두 대상을 별개의 모듈로 나누는 가장 쉬운 방법 중 하나는 동작을 연이은 두 단계로 쪼개는 것**이다.
  * 예를 들어, 본 작업에 들어가기에 앞서 입력값을 다루기 편한 형태로 가공하거나 로직을 순차적인 단계로 분리한다.
  * 분리된 각 단계는 자신만의 문제에 집중할 수 있으므로, 나머지 단계에 관한 자세한 정보를 알 필요가 없다.
  * 이렇듯 단계를 쪼개는 기법은 주로 덩치 큰 소프트웨어에 적용이 가능하다.
* **다른 단계로 볼 수 있는 코드 영역들이 마침 서로 다른 데이터와 함수를 사용하고 있다면 단계를 쪼갤 수 있다는 좋은 신호**로 볼 수 있다.

### 단계 쪼개기 - 절차
1. 두 번째 단계에 해당하는 코드를 독립된 함수로 추출한 후, 테스트를 진행한다.
2. 중간 데이터 구조를 만든 후, 앞에서 추출한 함수의 인수로 추가한다.
3. 테스트를 진행한다.
4. 추출한 두 번째 단계 함수의 매개변수를 하나씩 검토한다.
   * 이 중 **첫 번째 단게에서 사용되는 것은 중간 데이터 구조로 옮겨준다**.
   * 각 매개변수를 옮길 때마다 별도의 테스트를 진행한다.
   * 간혹 두 번째 단계에서 사용하면 안되는 매개변수를 발견한다면, 각 매개변수를 사용한 결과를 중간 데이터 구조의 필드로 추출한다.
     * 사용되면 안되는 매개변수란 함수의 역할을 생각하면 판단이 쉬워질 수 있다.
   * 그 후 필드의 값을 설정하는 문장을 호출하는 곳으로 옮겨준다.
5. **첫 단계 코드를 함수로 추출하며, 중간 데이터 구조를 반환**하도록 한다.
   * 이 때, 첫 단계는 변환기 객체로 추출해도 무방하다.
   * **함수로 추출하는 방식과 변환기 객체에 메소드를 정의하는 방식에 우열은 없으며, 핵심은 언제나 단계를 명확히 분리하는 데에 있다**.