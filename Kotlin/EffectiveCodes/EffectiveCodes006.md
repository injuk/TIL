...
* 특히 대칭적 동작은 일반적으로 모든 사람이 기대하는 동작이며, 이를 위배했을 때의 디버깅 난이도는 매우 높은 축에 속하므로 주의해야 한다.
...
### 확장 함수와 멤버 메소드의 차이
```
> 확장 함수는 개발자에게 많은 자유와 유연성을 제공하며, 일반적으로 API의 필수적인 부분은 멤버 요소로 두되 그렇지 않은 경우에 확장 함수를 고려한다.
```
* 클래스의 **메소드는 멤버 또는 확장 함수로 정의될 수 있으며, 두 방식은 각자의 장단점을 갖기에 어떤 방식이 우월하다고 단정지을 수 없다**.
  * 때문에 상황에 맞는 방식을 사용해야 하며, 반드시 필요한 경우에만 사용해야 한다.
* 멤버 메소드와 비교했을 때, 확장 함수는 다음과 같은 특징을 갖는다.
  1. 확장 함수는 일반적으로 클래스 외부의 다른 패키지에 위치하므로, 따로 가져와서 사용해야 한다.
  2. 가져와 사용한다는 특징으로 인해 같은 타입에 대해 같은 이름으로 여러 메소드를 정의할 수 있으며, 충돌 역시 발생하지 않는다.
  3. 확장 함수는 멤버 요소보다 더 낮은 우선 순위를 갖는다.
  4. **가상 멤버 함수가 아니므로, 컴파일 시점에 정적으로 선택되어 오버라이드가 불가능**하다.
  5. **확장 함수는 클래스가 아닌 타입에 정의하는 것이므로, nullable 또는 구체적인 제네릭 타입에도 정의하는 것이 가능**하다.
  6. **확장 함수는 클래스 내부에 존재하지 않으므로 클래스 레퍼런스에서 멤버 요소로 표시되지 않고, 때문에 어노테이션 프로세서가 따로 처리하지도 않는다**.
* 이러한 특징으로 인해 **같은 이름으로 다른 동작을 하는 확장 함수는 지양하는 것이 바람직하며, 이 경우에는 멤버 메소드로의 구현이 더 적절**하다.
* 또한, **가상 멤버거 아니라는 특징으로 인해 확장 함수는 상속을 목적으로 설계된 요소에는 적용하지 않아야** 한다.

### 확장 함수는 멤버 요소로 정의하는 것은 지양하기
```
> 어떤 클래스에 대한 확장 함수를 정의하는 경우, 이러한 함수를 멤버 요소로 추가하는 것은 권장되지 않는다.
```
* 확장 함수는 첫 번째 인자로 리시버를 받는 단순 일반 함수로 컴파일되며, 이로 인해 클래스 멤버나 인터페이스 내부에 정의하는 것이 가능하다.
* 그러나 어디까지나 이러한 코드가 **가능하다는 것일 뿐, DSL 정의와 같은 경우를 제외하면 이러한 방식은 사용하지 않는 것이 바람직**하다.
  * 일반적으로 이러한 방식은 가시성을 제한하기 위해 사용되나, 가시성 제한은 private과 같은 가시성 한정자를 명시하는 것이 더 명확하다.
  * 때문에 정말 의미가 있는 경우에는 사용해도 무방하지만, 일반적으로는 그 단점을 인식하고 사용하지 않는 것이 바람직하다.
* 또한 **멤버 요소로 정의된 확장 함수는 리플렉션을 활용한 레퍼런스를 지원하지 않으며, 직관적이지 않아 코드의 가독성 및 유지보수성을 크게 떨어트린다**.