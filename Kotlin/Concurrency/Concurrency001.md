...
### 일시 중단 연산과 일시 중단 함수
```
> 일시 중단 연산은 다른 일시 중단 함수 또는 코루틴에서만 호출되곤 한다.
```
* **일시 중단 연산은 임의의 스레드를 차단하지 않는 대신 실행을 일시 중단할 수 있는 연산을 의미**한다.
  * 스레드를 차단하는 것은 불편하므로, 자체적인 실행을 중단하는 연산을 활용하여 스레드를 다른 연산에서 활용할 수 있다.
* **일시 중단 함수는 함수 형태의 일시 중단 연산이며, `suspend` 연산자를 통해 쉽게 식별**할 수 있다.
  * 이러한 일시 중단 함수는 작업을 처리하던 중 일시 중지된 동안 실행 스레드가 다른 연산을 수행하는 데에 사용될 수 있다.
* **람다 역시 일시 중단 람다로서 사용될 수 있으며, 일시 중단 람다는 또다른 일시 중단 함수를 호출하는 것으로 자신의 실행**을 중지할 수 있다.

## 2023-07-28 Fri
### 코루틴 디스패처란?
* **코루틴 디스패처는 코루틴을 시작하거나 재개할 스레드를 결정하기 위해 사용되며, `CoroutineDispatcher` 인터페이스를 구현**한다.
  * 예를 들어 `DefaultDispatcher`나 `Unconfined`, `CommonPool` 등을 사용할 수 있다.
  * `CommonPool`은 스레드 풀로부터 코루틴을 실행 및 재개하지만, `Unconfined`는 현재 스레드에서 코루틴을 시작하며 어떠한 스레드에서도 재개할 수 있다.
* 또한, 디스패쳐와 함께 필요에 따라 스레드나 스레드 풀을 정의하기 위해 사용할 수 있는 다음과 같은 빌더가 제공된다.
  1. `newSingleThreadContext`: 단일 스레드로 디스패처를 생성하며, 코루틴은 항상 같은 스레드에서 시작되거나 재개된다.
  2. `newFixedThreadPoolContext`: 지정된 크기의 스레드 풀과 디스패처를 생성하며, 런타임은 디스패처에서 코루틴을 시작하거나 재개할 스레드를 결정한다.

## 2023-07-29 Sat
### 코루틴 빌더란?
* **코루틴 빌더는 일시 중단 람다를 받아 이를 실행하기 위한 코루틴을 생성하는 함수이며, 코루틴은 다음과 같은 다양한 코루틴 빌더를 제공**한다.
  1. `async()`: **결과 또는 예외를 포함하는 `Deferred<T>`를 반환하므로, 결과가 반환되는 코루틴을 시작하기 위해 사용**된다.
  2. `launch()`: **결과를 반환하지 않는 코루틴을 시작하며, 대신 자기 자신이나 자식 코루틴을 취소하기 위한 `Job`을 반환**한다.
  3. `runBlocking()`: **코루틴의 실행이 끝날 때까지 현재 스레드를 블로킹하며, 블로킹 코드를 일시 중단 연산과 연결하기 위해 작성**된다.

## 2023-07-30 Sun
### 결론 - 동시성과 병렬성, 그리고 코틀린
* 동시성이란, 애플리케이션이 하나 이상의 스레드에서 중첩되어 실행되는 경우를 의미한다.
* 병렬성이란, 애플리케이션이 실행될 때 적어도 둘 이상의 스레드가 함께 실행되는 경우를 의미한다.
  * 동시성은 병렬성 없이 발생할 수도 있으며, 현대적인 처리 장치는 스레드 간의 효과적인 인터리빙과 스레드 중첩을 활용한다.
* 코틀린은 동시성에 대해 현대적이고 신선한 접근 방식을 취한 프로그래밍 언어로, 가독성 있고 유연하면서도 논 블로킹인 동시성 코드를 작성할 수 있도록 지원한다.