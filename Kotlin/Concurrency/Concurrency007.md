...
## 2023-09-23 Sat
### 스레드 한정, 액터, 그리고 뮤텍스 - 결론
* 스레드의 캐싱과 메모리 접근의 원자성을 잘 고려하는 등 공유 상태를 가질 경우, 일부 스레드 수정 사항이 유실되어 상태의 일관성이 깨질 수 있다.
* 상술한 동시성 문제를 회피하기 위한 방법은 크게 다음과 같은 두 방식을 고려할 수 있다.
  1. 오직 **하나의 스레드만이 상태와 상호 작용할 수 있도록 보장하여 상태를 읽기 전용으로만 공유**한다.
  2. 또는 **코드 블록을 원자적으로 만들기 위해 잠금을 활용하여 해당 코드 블록에 접근하는 모든 스레드의 동기화를 강제**한다.
* **스레드 한정이란, 코루틴 컨텍스트를 단일 스레드로 구성된 디스패쳐와 함께 사용하여 코루틴의 실행을 단일 스레드로 강제하는 기법을 의미**한다.
* **액터란 송신 채널과 코루틴의 조합을 의미하며, 액터를 단일 스레드로 한정하여 메시지를 기반으로 한 강력한 동기화 기법을 구현**할 수 있다.
  * 이 때, 임의의 스레드는 메시지를 전송하여 변경을 요청할 수 있으나 실제 변경은 특정한 스레드에서만 실행된다.
* 액터 역시 코루틴이므로 여러 방식으로 시작할 수 있으며, 나아가 코루틴의 스레드 제한과 조합하여 활용할 수도 있다.
* **뮤텍스는 코틀린에서 잠금을 활용하여 코루틴을 동기화하기 위해 사용될 수 있으며, 이를 통해 동기화된 작업을 재개할 때까지 코루틴을 일시 중단**할 수 있다.