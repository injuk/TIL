...
### Java와 Kotlin의 상호운용성
* Kotlin은 Java와의 상호운용성을 중요시하지만, Java는 null 가능성을 지원하지 않는다.
* 상호운용성을 보장하기 위해, Kotlin은 Java의 널 가능성 정보와 관련된 어노테이션을 활용한다.
  1. @Nullable: Kotlin은 널이 될 수 있는 타입으로 받아들인다.
  2. @NotNull: Kotlin은 널이 될 수 없는 타입으로 받아들인다.
* 널 가능성과 관련된 어노테이션이 없는 경우, Java의 타입은 Kotlin의 플랫폼 타입이 된다.

### 플랫폼 타입
* 플랫폼 타입은 Kotlin이 널 관련 정보를 알 수 없는 타입이다.
  * 모든 타입을 널이 될 수 있는 타입으로 판단할 수도 있었지만, 절대 null이 될 수 없는 경우에 대해 불필요한 검사를 제외하기 위해 플랫폼 타입이 도입되었다.
  * 만약 컬렉션 등에 대해 매번 null 검사를 강제하면 null 안정성으로 얻는 이득보다 null 검사로 인한 손해가 더 커질 수 있다.
  * 때문에 **Kotlin은 개발자가 Java의 타입을 가져오는 경우, 그 타입을 적절히 처리할 수 있도록 책임을 넘기는 방향으로 설계**되었다.
* Kotlin에서 플랫폼 타입을 선언할 수는 없으며, 오직 Java 코드에서 가져온 타입만 플랫폼 타입이다.
* Java의 경우와 마찬가지로 플랫폼 타입에 대해 수행하는 모든 연산은 개발자가 책임져야 한다.
  * 플랫폼 타입은 널이 될 수 있는 타입이나, 널이 될 수 없는 타입 무엇으로 취급해도 무방하다.
  * 컴파일러는 모든 연산을 허용하며, null 안정성 검사 로직을 자유롭게 정의한다.
  * 이 과정에서 개발자가 잘 못 처리한 코드는 NPE가 발생한다.
* **Kotlin 컴파일러는 Java API 호출 시 함수 내부에서 파라미터의 값을 검사하지 않고, 함수 호츌 시점에 검사**한다.
  * 즉, 잘못된 인자가 함수 내부에 전달되어 예외 발생 시점을 파악하기 어려워지는 일이 없도록 발생 가능한 예외는 최대한 빨리 던지도록 한다.
* **그럼에도 가장 좋은 것은 null 값을 제대로 처리하여 런타임 예외를 발생시키지 않는 것**이다.
  * 대부분의 Java API는 null 관련 어노테이션을 사용하지 않기 때문이다.

### Kotlin과 Java가 혼합된 상속 계층
* Kotlin에서 Java의 메소드를 오버라이드할 때, 메소드의 파라미터와 반환형에 대한 널 가능성을 정의해야 한다.
  * 즉, 각각을 널이 될 수 있는 타입으로 오버라이드할지 널이 될 수 없는 타입으로 오버리아드할지 결정해야 한다.
  * Kotlin 컴파일러는 두 구현 방식을 모두 받아들인다.
* Java 클래스나 인터페이스를 Kotlin에서 구현한 경우에는 널 가능성 처리를 신중히 고려해야 한다.