...
### async와 await
* 프로미스는 비동기 코드를 깔끔하고 가독성 있게 작성할 수 있도록 해주지만, 순차적 비동기 코드의 작성에서는 여전히 차선책에 불과하다.
  * 프로미스 체인이 콜백 지옥보다는 낫지만, 여전히 then 함수를 활용한 프로미스 체인과 각각의 작업에 대응되는 새로운 함수를 작성해야 한다.
  * 이는 자주 사용되는 제어 흐름에 대한 작업으로는 여전히 과하다.
* **JS에서는 흔한 비동기 순차 실행 흐름을 처리하기 위한 적절한 방법이 필요했으며, 이에 async와 await는 ECMAScript 표준으로서 정의**되었다.
* async와 await 구문을 활용하면 각 비동기 작업에서 다음 구문을 실행하기 전까지 결과를 기다리는 것처럼 보이는 코드를 작성할 수 있다.
  * async와 await 구문을 활용하는 비동기 코드는 전통적인 동기식 코드와 비슷한 가독성을 지닐 수 있다.
* **async와 await 구문은 오늘 날 비동기 작업 처리에 있어 권장되는 방식이지만, 기존의 모든 비동기 제어 흐름을 대신할 수는 없다**.
  * 또한, async와 await는 그 자체로 프로미스에 크게 의지한다.

### async와 await의 표현
* async 함수는 프로미스가 결정될 때까지 함수의 실행을 일시 정지하기 위해 사용되는 await 표현식을 사용할 수 있는 특별한 함수이다.
* **async와 await 표현은 비동기 작업처럼 보이지 않지만, 함수는 동기적으로 실행되지 않는다는 사실에 유의**해야 한다.
  * **각각의 await 표현에서 함수의 실행은 보류되고, 상태가 저장되며 제어는 이벤트 루프로 반환**된다.
  * **대기 중인 각각의 프로미스가 해결될 때 제어는 async 함수로 반환되어 프로미스의 이행 값이 반환**된다.
* **async 함수는 실제로는 항상 프로미스를 반환하며, 이는 Promise.resolve에 함수의 반환 값이 전달된 후 호출자에게 반환된 것**과 같다.
  * async 함수의 호출 자체는 다른 비동기 작업과 마찬가지로 즉시 수행되며, 프로미스를 동기적으로 반환한다.
  * **반환된 프로미스는 함수에 의해 생성된 결과 또는 에러에 따라 상태가 결정**된다.
* async와 await 표현은 프로미스를 더 간단하게 사용하기 위한 문법 설탕이다.
  * **async와 await를 사용하는 모든 비동기 제어 흐름 패턴은 비용이 많이 드는 작업에 프로미스 또는 프로미스 API를 사용**한다.

### async와 await에서의 오류 처리
* async와 await 표현은 비동기 작업은 물론 오류 처리에서도 가독성을 크게 향상시킨다.
* async와 await는 try - catch 동작을 정규화하여 동기적인 throw와 비동기적 프로미스의 거부 모두 같은 방식으로 오류 처리를 가능케 한다.
* 이렇듯 async / await 구문의 최대 강점은 비동기적인 순차 실행을 쓰기 쉽고 읽기 쉽게 만드는 능력이다.

## 2022-05-28 Sat
### 병렬 실행
* async와 await를 활용하여 일련의 비동기 작업들을 병렬로 실행하는 방법은 크게 다음과 같다.
  1. 순수하게 await 표현식만을 사용하는 방법
  2. Promise.all에 의존하는 방법
* 상술한 두 방식 모두 구현이 간단하지만, 일반적으로는 Promise.all 방식이 권장된다.
  * async / await에만 의존하는 방식은 for 순회를 사용하게 되지만, 이러한 방식은 거부된 프로미스와 관련된 부작용이 있다.
  * 반면, Promise.all은 인자로 전달된 배열에 제공된 프로미스 중 하나가 거부되는 즉시 거부한다.

### 무한 재귀 프로미스 체인 문제
* 무한 프로미스 해결 체인의 경우 메모리 누수가 발생하기 쉬우며, Promise / A+ 명세를 엄격히 지킨 구현 역시 이러한 이슈에서 자유로울 수는 없다.
  * 개발 과정에서 작업이 정해진 끝을 갖지 않거나, 잠재적으로 무한한 데이터를 입력으로 받는 경우는 흔히 존재한다.
  * 이러한 상황에서 잘 못 구현된 무한 재귀 프로미스 체인은 메모리 누수를 발생시켜 애플리케이션을 망가트릴 수 있다.
* 예를 들어, 아래의 코드는 반환되는 프로미스의 상태가 다음 재귀 호출에 의해 결정되므로 절대로 resolve되지 않는다.
  * **이러한 상황은 결정되지 않는 일련의 프로미스 체인을 생성하므로 메모리 누수를 유발**한다.
```
function leakingLoop() {
  return delay(1)
    .then(() => {
      console.log(`Tick ${Date.now()}`);
      return leakingLoop(); // 다른 프로미스를 반환한다.
    });
}
```
* 해당 문제를 해결하기 위해서는 프로미스 해결 체인을 끊어주기 위해 return 문을 제거해야 한다.
  * 이는 leakingLoop에 의해 반환되는 프로미스의 상태가 다음 재귀 호출에 의해 반환된 프로미스에 의존하지 않도록 한다.
```
...
```
* 그러나 이러한 방식은 각 프로미스의 상태가 서로 연결되지 않으므로, 재귀 호출 과정에서 발생한 에러를 전파하지 않는다.
* 재귀의 깊이에 관계 없이 비동기 작업이 실패하는 경우 즉시 거부할 수 있도록 만들기 위해서는 다음과 같은 방식을 고려할 수 있다.
  1. 프로미스 생성자를 사용하기
  2. async / await와 무한 반복문 사용하기
* 프로미스 생성자를 사용하는 방식은 아래와 같으며, 비동기 작업이 실패할 경우 즉시 reject한다.
  * 해당 방법 역시 각 재귀에서 생성된 프로미스 간 링크는 존재하지 않는다.
```
  ...
  return new Promise((resolve, reject) => {
    (function internalLoop() {
      delay(1)
        ...
        .catch(error => {
          reject(error); // 비동기 작업이 실패하면 즉시 거부한다.
        });
    })();
  });
}
```
* async / await를 활용하는 코드는 다음과 같으며, 해당 방식 역시 기존 재귀 방식의 동작을 보존하면서도 발생하는 모든 오류는 함수 호출자에게 전파한다.
```
...
```
* 반면, while 루프가 아닌 비동기 재귀를 사용하는 아래와 같은 구현 방식은 여전히 메모리 누수의 발생 가능성을 갖게 된다.
  * 아래의 코드는 절대 resolve되지 않는 무한한 프로미스 체인을 계속해서 생성하게 되므로, 메모리 누수 문제에서 자유로울 수 없다.
```
...
```
* **무한 프로미스 체인을 개발해야만하는 경우, 상술한 내용을 참고하여 메모리 누수를 발생시키는 조건이 있는지 신중히 확인**할 수 있어야 한다.

### 결론
* 프로미스 와 async / await는 제어 흐름에서 자주 사용되는 순차 실행 흐름을 크게 간소화한다.
  * 이로 인해 작성된 비동기 코드는 간결하고 깔끔하며 가독성이 좋아진다.
  * 두 방식은 동기적 동작과 비동기 동작이 혼합된 까다로운 상황에서 명료한 방안을 제공하며, 특히 오류 관리 측면에서 직관적이고 실수 발생 여지가 적어진다.
* 반면, 콜백은 여전히 Node.js와 JS 생태계에서 널리 사용되므로 반드시 알아두어야 할 개념이다.
* 단지 **일상적인 개발 과정에서 프로미스와 async / await는 콜백에 비해 커다란 진보이며, 이로 인해 Node.js에서의 비동기 코드를 다루는 표준**이 되었다. 