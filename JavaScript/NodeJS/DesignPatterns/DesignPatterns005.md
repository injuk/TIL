...
## 2022-05-24 Tue
### 병렬 실행 패턴
* 병렬 실행 흐름에 일반적으로 적용할 수 있는 패턴은 다음과 같다.
```
const tasks [ /*...*/ ];
...
```
* 상술한 패턴에 적은 수정을 가하는 것 만으로도 다음과 같은 기능을 수행하도록 수정할 수 있다.
  1. 각 작업의 결과를 컬렉션에 모으기
  2. 배열의 요소를 필터링 또는 매핑하기
  3. 일정한 작업의 수가 완료되는 즉시 finish() 호출하기
     * 이러한 경우는 경쟁이라고도 지칭할 수 있다.
* 다중 작업이 병렬로 실행되는 경우, 외부 리소스에 대한 경쟁 상태를 가질 수도 있다.

### 동시 작업과 경쟁 상태
* 멀티 스레드와 블로킹 IO를 함께 사용하는 경우, 병렬 작업 실행으로 인한 문제가 발생하기 쉽다.
  * 그러나 **여러 비동기 작업을 병렬로 실행하는 것은 리소스 측면에서 직관적이고, 비용이 적게 드는 방식**이기도 하다.
  * **이는 Node.js의 중요한 장점 중 하나이며, 필요한 경우에만 사용하는 복잡한 기술보다 일반화된 방식으로서의 병렬화를 사용**한다. 
* Node.js에서는 모든 것이 단일 스레드에서 실행되므로 여러 복잡한 동기화 메커니즘이 필요하지 않다.
  * **그러나 이것이 경쟁 조건을 전혀 가지지 않는다는 것을 의미하지는 않으며, 오히려 매우 일반적으로 마주할 수 있는 상황**이다.
  * 이 때, **문제의 근본적인 원인은 비동기 작업의 호출과 작업의 결과에 대한 통지 사이에서 발생하는 지연**이다.
* 두 비동기 작업의 병렬 실행에서 경쟁 상태는 쉽게 발생할 수 있으며, 이를 해결하기 위해 여러 비동기 작업의 동시 실행을 배제할 수 있는 변수를 도입할 수 있다.
  * 예를 들어 이미 시작된 비동기 작업을 Set에 추가하고, 다른 비동기 작업이 시작될 때마다 Set에 포함된 작업을 확인하여 진행을 결정할 수 있다.
  * 이 경우, 처리해야할 비동기 작업의 갯수가 많다면 이미 종료된 작업은 Set으로부터 제거하여 메모리 소비량을 유지할 수 있다.
* 이렇듯 **경쟁 상태는 단일 스레드 환경에서도 많은 문제를 일으킬 수 있다**.
  * 경우에 따라서는 데이터가 손상되며, 일반적으로 경쟁 상태는 일시적인 특성이므로 디버그하기도 매우 어렵다.
  * 때문에 병렬 작업 실행시 경쟁 상태의 발생 가능성을 신중히 고려하는 것이 바람직하다.

### 제한된 병렬 실행
* 제어되지 않는 병렬 작업들을 생성하는 것은 종종 과부하의 원인이 될 수 있다.
  * 예를 들어, 수 천개의 파일을 읽는 등의 상황에서는 프로세스에서 가능한 모든 파일 기술자를 사용하므로 리소스가 쉽게 부족해질 수 있다.
* 또한 사용자의 요청을 다루기 위해 무한한 병렬 작업을 생성하는 서버는 DoS 공격에도 취약하다.
* 결과, **일반적으로 병렬 작업의 수를 제한하는 것은 복원력 있는 애플리케이션을 만드는데에 큰 도움이 될 수 있다**.
  * 처음에는 동시 실행의 제한을 초과하지 않는 선에서 최대한 많은 작업을 생성한다.
  * **작업이 완료될 때마다 한도에 도달하지 않도록 하나 또는 그 이상의 작업을 생성**한다.

### 전역적인 동시성 제한
* 일반적으로, 제한된 동시성 패턴의 구현은 실행할 일련의 작업들이 이미 존재하거나 작업의 수가 시간에 따라 선형적으로 증가하는 경우에 적절하다.
* 그러나 하나의 작업이 두 개 이상의 작업을 직접적으로 생성하는 경우, 전역의 동시성을 제한한 구현은 적합하지 못하다.
  * 이 경우, 작업의 전체 수를 제한하기 위해 큐를 도입할 수 있다.

### 큐를 사용하여 해결하기
* 우리가 정말 원하는 것은 동시에 실행 가능한 다운로드 작업의 전체 수를 제한하는 것이다.
* 이 경우, 동시성 제한 패턴을 적절히 수정하거나 큐를 도입할 수 있다.
* 큐를 사용하는 경우, 기본적으로 동시 제한을 초과하지 않는 만큼의 가능한 작업을 큐로부터 시작한다.
  * 각 작업이 완료되면 실행 중인 작업의 수를 갱신하고 비동기적으로 다른 작업을 시작한다.
* 큐를 도입하는 것으로 얻을 수 있는 장점은 크게 다음과 같다.
  1. 새로운 작업을 동적으로 추가할 수 있다.
  2. **동시 실행 제한에 대한 엔티티를 중앙에서 관리할 수 있으므로, 함수 실행의 모든 인스턴스로부터 작업의 동시 실행 제한을 공유할 수 있게 된다**.

### 큐 향상시키기
* **비동기 작업 중 하나가 실패한 경우, 또는 큐에 있는 모든 작업이 완료된 경우 등 큐의 작업 진척 상황을 통지받기 위해 EventEmitter를 활용**할 수 있다.
  * 예를 들어, 큐 클래스가 EventEmitter를 상속받아 확장되도록 구현한다.
* **일반적으로 큐 기반 시스템은 내부의 동작에서 에러가 발생했더라도 큐의 동작을 의도적으로 유지**한다.
  * 즉, 진행 중인 다른 작업을 정지하거나 대기 중인 작업을 삭제하도록 동작할 필요가 없다.
  * **시스템이 충돌하도록 두는 것보다는 에러의 발생을 예상하고, 에러를 명시한 후 재시도 또는 복구 전략을 적용하는 것이 바람직**하다.

### 비동기 라이브러리 활용하기
* 상술한 모든 제어 흐름 패턴의 경우, 재사용 가능하고 일반적인 솔루션을 구축하기 위한 기반으로 사용될 수 있다.
* 제어 흐름 알고리즘을 재사용 가능한 함수로 만드는 것은 비동기 제어 흐름을 정의하는 더 선언적이고 표현적인 방식으로 이어질 수 있다.
  * async 라이브러리는 정확하게 이러한 역할을 수행해줄 수 있다.
* **async는 Node.js와 JS에서 일반적으로 사용하는 비동기 코드 처리를 위한 라이브러리**이다.
* async 라이브러리는 크게 다음과 같은 기능을 제공한다.
  1. 다양한 환경에서의 여러 유형의 작업 실행을 크게 단순화하는 함수들을 제공한다.
  2. 컬렉션을 비동기적으로 처리할 수 있는 유용한 헬퍼를 제공한다.
* **async와 유사한 라이브러리는 많이 존재하지만, async는 꾸준한 인기로 인해 Node.js에서는 사실상 표준에 해당**한다.
  * 이러한 특징은 비동기 작업을 정의하기 위한 콜백을 사용할 때에 특히 두드러진다.
* async 라이브러리가 제공할 수 있는 주요한 능력들은 크게 다음과 같다.
  1. 컬렉션을 순회하며 비동기 함수들을 실행시킨다.
  2. 모든 함수의 출력이 다음 함수의 입력이 되는 비동기 함수 체인을 실행한다.
  3. 상술한 큐와 기능적으로 동등한 큐의 추상화를 제공한다.
  4. 첫 번째 작업이 완료되었을 때 다수의 비동기 함수를 병렬적으로 실행하거나 정지하는 race와 같은 비동기 패턴을 제공한다.
* **비동기 패턴을 이해하고 있는 경우, 일상적으로 사용되는 제어 흐름을 위한 구현에 얽매일 필요가 없다**.
  * **대신 async와 같이 널리 알려졌으며 테스트에도 유용한 라이브러리를 활용하는 것이 애플리케이션의 품질 향상에 직접적인 도움**을 줄 수 있다.