...
### MongoDB의 ObjectId
* RDS의 경우 모든 row에 대해 PK가 존재하듯, MongoDB에는 각 도큐먼트를 구분하는 ObjectId(이하 oid)가 존재한다.
  * 이 때, oid는 기본적으로 도큐먼트의 `_id.$oid` 경로에 명시되며 24자리의 16진수로 표현된다.
* oid는 곧 12 바이트로 표현되며, DB마다 유일성을 보장할 수 있도록 다음과 같은 부분으로 분류되어 구성된다.
  1. 4바이트: ms 단위의 timestamp를 활용하여 결정된다.
  2. 5바이트: 랜덤한 값으로 결정된다.
  3. 3바이트: RDS의 AI와 유사하게 점차 늘어나는 값으로 결정된다.
* **용도에 따라서는 다른 값을 PK 용도로 써도 무방하지만, 되도록이면 MongoDB 차원에서 자동으로 결정되는 oid를 사용하는 것이 바람직**하다.

### 샤딩이란?
* **샤딩은 여러 장비에 걸쳐 데이터를 분할하는 기법을 의미하며, 일반적인 파티셔닝과 유사**하다.
* 샤딩에 의해 분리된 샤드들은 물리적인 개별 노드를 의미하며, 각각 데이터 청크를 저장한다.
  * 이 때, **각 데이터 청크를 어떤 샤드에 저장할지 결정하기 위해서는 규칙이 필요하며 샤딩 키가 이러한 역할을 수행**한다.
* Mongos 라우터는 사용자가 요청한 도큐먼트가 어느 샤드에 위치해 있는지 샤딩 키에 따라 알아낼 수 있으며, 이로 인해 요청에 빠르게 응답할 수 있다.
  * 또한, Mongos 라우터로 인해 여러 샤드들로 구성된 MongoDB는 외부에서 볼 때 마치 하나의 DB처럼 보일 수 있으며 이를 mongod라는 용어로 지칭한다.
* 이렇듯 **샤딩을 적절히 활용할 경우 데이터의 양과 노드 수가 늘어나더라도 응답 시간을 고르게 유지할 수 있으며, 효과적인 부하 분산이 가능**하다.
* 그러나 샤딩은 대규모 서비스에서만 고려되어야 하는 기능이며, 일반적인 규모의 서비스에서는 고려되는 경우가 많지 않다.

### Prisma와 MongoDB
* Prisma는 ORM 자체적으로 MongoDB를 지원하며, 사용되는 API 역시 PG의 경우와 같다.
* 때문에 개발자는 설정과 `schema.prisma`만 관리하며, 세부 사항인 저장소의 종류를 모르는 상태에서도 ORM을 기반으로 코드를 작성할 수 있다.
* 그러나 MongoDB의 유연성으로 인해 도큐먼트 형태에 대한 제한이 사실 상 없으므로, 자칫 잘못된 데이터가 삽입될 경우 무결성이 깨지기 쉬운 점에 주의해야 한다.