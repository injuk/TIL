...
### Named 쿼리란?
* Named 쿼리란, 미리 정의하여 이름을 부여한 후에 사용하는 JPQL에 해당한다.
* Named 쿼리는 동명의 어노테이션을 엔티티에 명시하는 것으로 활용이 가능하며, 그 예시는 다음과 같다.
  * 이 때, **name 속성은 아무렇게나 작성해도 무방하지만 관례상 `엔티티명.식별자`의 형태로 작성**한다.
```
@Entity
@NamedQuery(
...
public class Member {}
```
* 이 때, name 속성에 할당된 이름을 다음과 같이 활용하여 미리 정의된 쿼리를 사용할 수 있다.
  * **Named 쿼리를 사용하는 경우, 엔티티 매니저가 제공하는 createNamedQuery 메소드를 활용**한다.
```
List<Member> members = em.createNamedQuery("Member.findByName", Member.class)
  .setParameter("name", "nameA")
  .getResultList();
```

### Named 쿼리의 특징
* Named 쿼리는 동적 쿼리로 사용이 불가능하며, 오로지 정적 쿼리로만 사용이 가능하다.
* Named 쿼리는 상술한 바와 같이 어노테이션을 활용하거나, xml에 정의하여 사용할 수 있다.
* **Named 쿼리는 애플리케이션 로딩 시점에 초기화 후 재사용이 가능**하다.
  * **정적 쿼리는 동적으로 변경되지 않으므로, 애플리케이션 로딩 시점에 JPA 또는 하이버네이트가 Named 쿼리를 SQL로 파싱한 후에 캐싱**한다.
  * 이렇듯 SQL로 파싱하는 과정에서 약간의 비용이 들 수 있으나, 애플리케이션 로딩 시점에 파싱을 마친 후에는 캐싱되므로 오히려 장점으로 볼 수 있다.
* **Named 쿼리는 애플리케이션 로딩 시점에 쿼리의 유효성을 검증하며, 이는 Named 쿼리를 통해 제공받을 수 있는 가장 큰 장점에 해당**한다.
  * 즉, 휴먼 에러로 인해 쿼리가 잘 못 정의된 경우 애플리케이션이 빌드되지 않으므로 SQL 문법 문제로 인해서는 런타임에 버그가 발생하지 않는다.
* **실무에서는 엔티티 클래스가 너무 장황해지므로 Named 쿼리 자체는 권장되지 않으며, 대신 Spring 데이터 JPA를 통해서 더 깔끔하게 사용**하게 된다.

### xml을 활용하여 Named 쿼리를 관리하는 경우
* Named 쿼리의 경우, xml에 정의된 Named 쿼리가 항상 우선권을 갖는다.
* 또한, 애플리케이션 운영 환경에 따라 여러 벌의 xml 파일을 작성하여 배포할 수 있다.
  * 즉, 운영 환경에 따라 다른 Named 쿼리를 사용할 수 있다.

### 벌크 연산이란?
* SQL의 update 또는 delete 문의 경우, JPA 변경 감지 기능만으로는 너무 많은 SQL이 실행되는 경우가 있다.
  * 예를 들어, 변경된 데이터가 100건이라면 update SQL 역시 100번 호출될 수 있다.
  * 이는 기본 키를 명시하여 수정하거나 제거하는 경우를 제외하고 모든 상황에서 발생이 가능하다.
* JPA는 벌크 연산보다는 실시간 성 단 건 조회 또는 수정에 최적화되어 있으나, 현실적으로 벌크 연산을 배제할 수는 없으므로 벌크 연산 개념을 제공한다.
* **벌크 연산을 활용하는 경우, 쿼리 한 번으로 테이블의 여러 엔티티를 나타내는 row를 변경**할 수 있다.
  * 이 때, executeUpdate 메소드를 호출하며 그 결과로는 영향 받은 엔티티의 수가 반환된다.
```
int affectedRows = em.createQuery("update Product p set p.price = p.price * 1.1 where p.stock < :stock")
  .setParameter("stock", 10)
  .executeUpdate();
```
* **벌크 연산은 update와 delete를 모두 지원하며, 하이버네이트의 경우 벌크 insert 역시 지원**된다.

### 별크 연산과 주의사항
* **벌크 연산은 기본적으로 영속성 컨텍스트를 무시하고 데이터베이스에 직접 쿼리를 요청**한다.
* 때문에 **이러한 문제점을 보완하기 위한 방법을 사용해야 하며, 이는 크게 다음과 같이 분류되는 두 방법 중 하나를 선택**하게 된다.
  1. 벌크 연산을 먼저 실행하기: 우선 영속성 컨텍스트에 영향을 주는 어떠한 작업도 하지 않고, 깨끗한 상태에서 벌크 연산부터 수행한다.
  2. 또는 벌크 연산을 수행한 후에 영속성 컨텍스트를 초기화하기: 영속성 컨텍스트에 무언가 이미 존재하는 경우, JPQL 수행으로 인해 우선 flush 된다.
     * 즉, **벌크 연산 이전에 JPQL로 인해 flush가 호출되므로 영속성 컨텍스트는 깔끔하게 초기화**된다.
     * 때문에 벌크 연산을 수행한 후, 영속성 컨텍스트를 초기화하여 다시 깔끔한 상태를 유지할 수 있도록 한다.