...
* 이 때, **중요한 것은 merge 메소드는 영속성 컨텍스트로부터 조회한 값을 영속 상태로 만들어 값을 수정한 후에 반환한다는 점**이다.
...
## 2022-07-10 Sun
### 컨트롤러와 서비스에서의 엔티티 의존성
* 계층형 아키텍쳐에서는 일반적으로 컨트롤러에서 서비스, 서비스에서 리포지토리 방향으로 호출한다.
* 이 때, 웹 계층에서 전달된 정보를 컨트롤러와 서비스 중 어느 계층에서 엔티티로 만들지 고민해볼 수 있다.
  * 예를 들어, 컨트롤러 단에서 new 연산자를 통해 엔티티를 생성하여 서비스 계층으로 넘겨줄 수 있다.
  * 또는 **컨트롤러 단에서는 식별자 등 최소한의 정보만을 서비스 계층에 전달하고, 서비스 계층에서 엔티티를 다룰 수도** 있다.
* 이 때, **두 방식 중에서는 서비스 계층에서 엔티티에 조금 더 의존적인 로직을 작성하는 것이 권장**된다.
  * 이러한 방식이 컨트롤러를 보다 깔끔하게 유지하며, 장기적인 유지보수성을 높여줄 수 있다.
  * 또한, **컨트롤러 단에서 엔티티를 조회하여 서비스 계층으로 전달하는 것보다 서비스 계층의 트랜잭션 안에서 엔티티를 처리하는 것이 더 바람직**하다.
  * 덧붙여 **서비스 계층에서 엔티티를 관리하는 방식은 엔티티들이 영속성 컨텍스트 안에서 관리될 수 있으므로 더 효율적**이다.
* **중요한 것은 JPA의 영속성 컨텍스트를 최대한 활용하는 것이며, 컨트롤러를 활용하는 방식은 준영속 엔티티를 전달하게 되므로 이 점에서 불리함**이 있다.