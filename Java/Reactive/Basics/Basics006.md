...
## 2023-12-10 Sun
### publishOn()과 subscribeOn() 혼용하기
```
> publishOn()과 subscribeOn()을 적절히 혼용할 경우, 원본 Publisher가 데이터를 Emit하는 스레드와 이를 가공 및 처리하는 스레드를 분리할 수 있다. 
```
* 상술한 두 연산자는 원본 `Publisher`의 동작을 나머지와 구분하기 위해 혼용할 수 있으며, 이 경우 실행 스레드를 분석하는 것은 자칫 어려워지기 쉽다.
  * 즉, 두 연산자를 어떤 방식으로 활용하느냐에 따라 실행 스레드의 동작은 달라질 수 있다.
* 어떠한 `Operator` 체인을 명시하는 과정에서 상술한 연산자를 적용하지 않는 경우, 별도의 스케쥴러가 추가되지 않으므로 스레드는 변경되지 않는다.
  * 이 경우, `Operator` 체인 상에서 최초의 스레드는 main이 되므로 `Subscriber`가 데이터를 전달받아 처리하는 과정 역시 모두 main에서 진행된다.
* 반면, `publishOn() Operator` 를 하나 적용한 경우에는 해당 연산자에서 명시한 스케쥴러 유형의 스레드가 실행된다.
  * 다시 말해, **`publishOn() Operator`가 명시된 이후의 과정을 처리하는 실행 스레드는 모두 `publishOn()`에 명시된 스레드**가 된다.
* 용도에 따라서는 **`publishOn()`을 `Operator` 체인 상에서 한 번 이상 사용할 수도 있으며, 이 경우에도 상술한 원칙이 적용**된다.
  * 즉, `Operator` 체인 상에 명시된 각각의 `publishOn()` 이후마다 실행 스레드가 달라지게 된다.
  * 이러한 복수의 `publishOn()`은 예를 들어 `Operator` 체인 상에서 용도와 목적에 따라 실행 스레드를 분리하고자 하는 경우에 유용하게 사용될 수 있다.
* **상술한 내용은 모두 main 스레드가 최초의 실행 스레드가 되지만, `subscribeOn() Operator`를 활용하는 경우 최초의 실행 스레드는 변경**될 수 있다.
  * 이는 **`subscribeOn() Operator`가 구독이 발생한 직후의 실행 스레드를 지정하기 때문**이다.
  * 이 경우, 이후의 `Operator` 체인을 처리하는 실행 스레드는 별도의 `publishOn() Operator`가 적용되었는지에 따라 변경될 수 있다.
  * 이렇듯 **해당 연산자는 `Operator` 체인 상 어디에 있든 간에 구독 직후, 즉 `Publisher`가 데이터를 `Emit`하기 전에 실행 스레드를 변경**한다.

## 2023-12-11 Mon
### Scheduler의 종류
* Reactor의 스케쥴러는 그 기능에 따라 여러 종류로 분류되며, 각각 요구사항에 맞는 것을 선택하는 것이 중요하다.
* `Schedulers.immediate()`는 별도의 스레드를 추가로 생성하는 대신, 현재 스레드에서 작업을 처리하기 위해 사용한다.
  * 이는 내부적으로 외부로부터 전달받은 스케쥴러를 활용하는 API를 제작하는 경우에 사용될 수 있다.
* `Schedulers.single()`은 스레드를 단 하나만 생성하여 스케쥴러가 제거되기 전까지 재사용하는 방식에 해당한다.
  * 이는 하나의 스레드를 계속해서 재사용하며 다수의 작업을 처리하는 경우에 유용하며, 당연히 지연 시간이 짧은 작업을 처리하는 데에 활용하는 것이 바람직하다.
* `Schedulers.newSingle()`은 호출 시마다 새로운 스레드를 하나 생성하는 방식에 해당한다.
  * 이 때, 첫 번째 인자로 스레드의 이름을 지정할 수 있고 두 번째 인자로 해당 스레드를 보조 스레드인 데몬 스레드로 동작시킬지 여부를 명시할 수 있다.
  * 예를 들어 두 번째 인자를 `true`로 두어 데몬 스레드로 동작시킨 경우, 해당 스레드는 주 스레드가 종료되는 경우에 자동으로 함께 종료된다.
* **`Schedulers.parallel()`은 논블로킹 IO에 최적화된 스케쥴러로서, CPU 코어 수 만큼의 스레드를 생성하여 동작**한다.
  * 이는 블로킹 IO 작업에 최적화되어 있는 `Schedulers.boundedElastic()`과 대비된다.
* `Schedulers.fromExecutorService()`는 이미 사용 중인 `ExecutorService`가 존재하는 경우 이를 기반으로 스케쥴러를 생성하는 방식에 해당한다.
  * 반면, `ExecutorService`로부터 명시적으로 스케쥴러를 생성하여 할당하는 방식은 Reactor 상에서 권장되지 않는다.
* `Schedulers.newSingle()`이나 `Schedulers.newParallel()` 등의 `newXXXX()` 형태의 API는 새로운 스케쥴러 인스턴스를 생성하여 동작한다.
  * 이는 `Schedulers.parallel()` 등 **대응되는 스케쥴러들이 각각 Reactor가 제공하는 디폴트 스케쥴러 인스턴스를 사용하여 동작하는 것과 대비**된다.
  * 즉, **해당 API를 활용할 경우 스레드의 이름이나 생성 가능한 디폴트 스레드 수 등을 개발자가 직접 조절하여 커스텀 스레드 풀을 생성**할 수 있다.

## 2023-12-12 Tue
### Schedulers.boundedElastic()이란?
```
> 실행 시간이 긴 블로킹 IO 작업은 전용 스레드를 할당하여 처리하는 것으로 전체적인 처리 시간을 효율적으로 사용할 수 있다.
```
* **`Schedulers.boundedElastic()`은 `ExecutorService` 기반의 스레드 풀을 생성하여 활용하는 방식에 해당**한다.
  * 이 경우, **스레드 풀 안에서 정해진 수만큼의 스레드를 활용하여 작업을 처리한 후에 작업 종료 시 스레드를 반환하여 재사용**할 수 있도록 한다.
  * 이 때, 기본적으로는 CPU 코어 수의 10배 만큼의 스레드를 생성하며 모든 스레드가 작업 중인 경우 최대 10 만 개의 작업을 큐에 추가로 대기시킬 수 있다.
* 일반적으로 **실무에서는 DB나 HTTP 요청 등 블로킹 IO로부터 반환된 데이터를 데이터 소스로 활용하는 경우가 많다**.
  * 이 경우에 `Schedulers.boundedElastic()`을 적용하여 효과적인 작업 처리를 기대할 수 있다.
  * 이는 **실행 시간이 상대적으로 훨씬 긴 블로킹 IO 작업이 다른 논블로킹 IO 작업에 영향을 주지 않도록 전용 스레드를 할당하는 방식**에 해당한다. 