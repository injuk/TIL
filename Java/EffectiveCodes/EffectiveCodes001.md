...
## 2022-02-24 Thu
### 6. 불필요한 객체 생성 피하기
* 똑같은 인스턴스를 계속해서 찍어내기 보다 하나를 재사용하는 것이 좋을 때가 많다.
* 예를 들어 String의 경우, **다음과 같이 생성하지 말아야** 한다.
```
String = new String("string");
```
* 이러한 방식은 매 번 새로운 string 인스턴스를 찍어내며, 실수로라도 반복문 안에서 호출할 경우 무의미한 인스턴스가 엄청나게 생성된다.
* 반면 다음의 방식은 같은 JVM 내부에서 같은 문자열 리터럴에 대해 항상 같은 객체가 사용되는 것이 보장된다.
```
String = "string";
```
* 생성자 대신 정적 팩토리 메소드를 제공하는 불변 클래스에서는 팩토리 메소드를 활용하여 불필요한 객체의 생성을 피할 수 있다.
  * **생성자는 호출 시마다 새로운 인스턴스를 생성하지만, 팩토리 메소드는 그렇지 않다**.
  * 불변 클래스 뿐만 아니라 가변 객체 역시 사용 중에 변경되지 않는 것이 확실한 경우에는 재사용이 가능하다.
* **생성 비용이 비싼 객체를 반복해서 사용하는 경우에는 객체를 캐싱하는 것이 바람직**하다.
* 반대로, 아주 무거운 객체가 아니라면 굳이 객체 생성을 모두 피하기 위한 객체 풀을 만들지 않아야 한다.
  * 일반적으로 객체 풀은 가독성과 성능 모두를 떨어트린다.
  * **반례로, 데이터베이스 연결 같은 경우 생성 비용이 매우 비싸므로 재사용이 권장**된다.
* 불변 객체는 명백하게 재사용이 안전하다.
* **오토박싱은 불필요한 객체를 생성하는 또 다른 예시**이다.
  * 주로 원시 타입과 래퍼 클래스를 혼용하는 과정에서 생성된다.
  * **원시 타입과 래퍼 클래스는 의미상 차이가 없지만 성능상 유의미한 차이를 갖는다**.
* 항상 **박싱된 래퍼 클래스보다는 기본 타입을 사용하고, 명확히 의도되지 않은 오토 박싱은 피해야 한다**.
* **기존 객체를 재사용해야 하는 경우에는 새로운 객체를 생성하지 않는 것이 바람직**하다.
  * 불필요한 객체 생성은 코드의 형태와 성능에 영향을 준다.

### 7. 사용을 마친 참조는 해제하기
* GC의 존재가 메모리 관리에 신경쓰지 않아도 좋다는 의미는 아니다.
* 사용을 마친 참조를 유지하는 코드를 방치할 경우, 프로그램을 오래 실행할수록 GC와 메모리 사용량이 늘어나 성능이 저하될 수 있다.
* GC 언어에서는 메모리 누수를 찾기 어렵고 까다롭다.
  * 객체 참조를 살려두면 해당 객체 뿐만 아니라 그 객체가 참조하는 모든 객체는 회수되지 않는다.
  * 즉, 몇 개의 객체라도 사용 후 회수되지 않는다면 많은 객체에 영향을 주는 악영향을 미칠 수 있다.
* 사용을 마친 객체는 null 처리하여 참조를 해제해볼 수 있지만, 항상 바람직한 방식은 아니다.
  * 오히려, 항상 바람직한 방식은 참조를 담은 변수를 유효 범위인 스코프 밖으로 밀어내는 것이다.
  * 이는 변수의 범위에 신경 써서 작업헀다면, 자연스럽게 이루어지는 일이다.
* 메모리 누수는 특히 다음 상황에서 발생하기 쉽다.
  1. 자기 메모리를 직접 관리하는 클래스,
  2. 캐시 역시 메모리 누수의 주 원인이다.
  3. 리스너, 콜백 등을 등록하고 명시적으로 해지하지 않는 경우, 해당 정보는 계속해서 쌓여갈 것이다.
  * 이러한 경우, 콜백을 약한 참조로 저장하여 GC가 즉시 수거할 수 있도록 한다.
* 메모리 누수는 그 원인을 찾기 어려우므로, 가능하다면 이러한 종류의 문제 예방법 이해하고 있어야 한다.

## 2022-02-25 Fri
### 8. finalizer와 cleaner를 사용하지 않기
* Java에서 지원되는 객체 소멸자이나, 둘 모두 기본적으로 쓰지 말아야 한다.
  * 특히 finalizer는 예측 불가하고, 위험할 수 있다.
  * cleaner는 finalizer보다는 안전하지만, 여전히 예측할 수 없고 느리고 불필요하다.
* C++의 destructor와는 다른 개념이며, Java에서는 이러한 역할을 GC에 맡기는 것이 좋다.
* finalizer와 cleaner가 위험한 이유는 다음과 같다.
  1. 즉시 수행된다는 보장이 없다.
  2. 실행은 전적으로 GC에 의존하며, 따라서 가비지 컬렉터의 구현에 따라 동작이 달라지므로 일관성이 없다.
  3. Java 언어 명세에서는 **두 방식 모두 수행 시점은 물론, 수행 여부까지도 보장하지 않는다**.
  4. 두 방식 모두 심각한 성능 문제를 동반한다.
  5. finalizer의 경우, 동작 중 발생한 예외는 무시된다.
* 만약 종료해야 할 자원을 담고 있는 객체를 안전하게 파괴하여 자원을 회수하고 싶다면 AutoClosable 인터페이스를 구현한다.
  * **사실 try with resource를 활용하는 것이 더 바람직**하다.
* finalizer와 cleaner의 주된 용도는 다음과 같다.
  1. 자원 소유자가 AutoClosable을 구현했지만, 인터페이스에 포함된 close 메소드를 호출하지 않은 경우에 대한 안전장치.
  2. Java 객체가 아닌 네이트브 피어와 연결된 객체.
    * 이 경우, Java 객체가 아니므로 애당초 GC의 대상이 되지 않는다.
* 그러나 **위 두 경우에 딱 맞는다고 하더라도 불확실성과 성능 저하를 충분히 고려한 후에 도입을 결정**해야 한다.

### 9. try with resources를 활용하기
* Java 라이브러리에는 close 메소드로 직접 닫아주어야 하는 자원이 많으며, 이러한 처리에는 전통적으로 finally를 활용해왔다.
* 그러나 여러 자원을 활용하는 경우 try - finally 블록이 중첩되어 가독성이 떨어지며, 디버깅도 어려워지게 된다.
* **이러한 아쉬움은 try-with-resources 문법을 통해 모두 해결**되었다.
  * 해당 문법은 AutoClosable 인터페이스를 구현한 클래스에 적용할 수 있다.
  * 서드 파티의 수많은 라이브러리들이 AutoClosable을 활용하고 있으며, 닫아야 하는 자원을 구현할 때에 AutoClosable을 반드시 구현하는 것이 좋다.
* try-with-resources 문법은 가독성이 좋을 뿐만 아니라 문제의 원인을 파악하기에도 좋다.
* **반드시 회수되어야 하는 자원은 finally보다 try-with-resources 문법을 활용**하는 것이 바람직하다.
  * 이 원칙에 예외는 없다. 이러한 경우, 항상 finally보다 try-with-resources 문법이 좋다!
