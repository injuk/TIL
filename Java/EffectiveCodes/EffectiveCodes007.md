...
* **옵셔널을 반환하는 메소드에서는 절대로 null을 반환하지 않아야 한다**.
  * 이는 옵셔널을 사용하는 취지에 정면으로 위배되기 때문이다.
* **옵셔널은 검사 예외와 취지가 유사하며, 반환 값이 없는 경우가 존재함을 클라이언트 코드 개발자에게 명확히 알려준다**.
  * 비검사 예외를 던지거나 null을 반환한다면, API 사용자는 이를 인지하지 못하여 디버깅 시점을 놓칠 수도 있다.
  * 반면 검사 예외를 던지면 클라이언트 코드에서 이에 대응하는 코드를 반드시 작성해야만 한다.
  * **옵셔널을 반환하는 메소드는 클라이언트가 값을 받지 못한 경우에 취할 행동을 선택하도록 강제**한다.
* **옵셔널을 반환하는 메소드로부터 값을 전달받지 못한 경우에 대한 방어 코드로서, 개발자는 기본 값을 설정하거나 예외를 던질 수 있다**.
  * orElse(값): 값을 전달받지 못한 경우에 대한 기본값을 설정한다.
  * orElseThrow(예외): 값을 전달받지 못한 경우에 예외를 던진다.
* 또는 boolean을 반환하는 isPresent 메소드를 활용할 수도 있다.
  * **isPresent 만으로도 모든 작업을 수행할 수 있으나, 되도록이면 다른 메소드들을 사용하는 것이 가독성과 코드 퀄리티를 높여**준다.
* 옵셔널은 항상 좋은 방법이 될 수는 없으며, 컨테이너틀 반환하거나 성능이 중요한 상황에는 적절하지 않을 수 있다.
  * **컬렉션, 스트림, 배열, 옵셔널 등의 컨테이너 타입은 옵셔널로 감싸는 대신 빈 컨테이너를 반환하는 것이 바람직**하다.
  * 이로 인해 클라이언트 코드에서 빈 옵셔널에 대응하는 코드를 작성할 필요가 없어진다.
* **옵셔널은 결과가 없을 수도 있으며, 이 경우를 클라이언트 코드가 특별히 처리해야 하는 경우에 사용**한다. 
* 박싱된 기본 타입을 감싸는 옵셔널은 두 겹이 추가되므로 기본 타입보다 무거울 수 밖에 없다.
  * 때문에 Java API에서는 int, long, double 전용 옵셔널 클래스인 OptionalInt, OptionalLong, OptionalDouble을 제공한다.
  * **따라서, 박싱된 기본 타입을 옵셔널로 감싸는 일은 지양**해야 한다.

## 2022-03-12 Sat
### 옵셔널의 결론
* 메소드가 값을 반환하지 못할 수도 있고, 호출 시마다 반환값이 없는 경우를 처리해야 하는 메소드는 옵셔널을 활용하는 것을 고려한다. 
* 옵셔널 반환은 성능 저하가 뒤따르므로, 성능이 중요한 환경에서는 null을 반환하거나 예외를 던지도록 한다.
* 옵셔널은 반환값 이외의 용도로는 거의 사용되지 않는다.

### 공개된 API 요소에는 항상 문서화 주석을 사용하기
* API를 쓸모있게 만드려면 잘 정리된 문서도 함께 작성해야 한다.
* API 문서는 코드 변경시마다 수정이 필요하지만, Java는 자바독이라는 유틸리티가 이를 돕는다.
* **문서화 주석을 작성하는 규칙은 공식 언어의 명세에는 속하지 않지만, Java 개발자라면 반드시 알고 있어야 할 업계 표준 API**에 속한다.
* API를 올바르게 문서화하려면 공개된 모든 클래스, 인터페이스, 메소드, 필드 선언에 문서화 주석을 작성해야 한다.
* **기본 생성자에는 문서화 주석을 추가할 수 없으므로, 공개 API는 기본 생성자를 절대로 사용하지 않아야 한다**.
* 메소드용 문서화 주석에는 해당 메소드와 클라이언트 코드 사이의 규약을 기술해야 하며, how보다는 what에 초점을 맞추어 작성한다.
  * 즉, **메소드가 어떻게 동작하는지를 해설하기보다 메소드가 무엇을 하는지를 작성**한다.
* 각 문서화 주석의 첫 문장은 해당 요소의 요약된 설명을 작성해야 한다.
  * 요약 설명은 반드시 설명 대상의 기능을 고유하게 기술해야 한다.
  * 즉, **한 클래스 또는 인터페이스 안에서 요약 설명이 같은 멤버가 존재할 수 없다**.
* **문서화 주석에서 제네릭, 열거 타입, 어노테이션은 특히 주의를 기울여 작성**해야 한다.
  * **제네릭 타입이나 제네릭 메소드의 문서화의 경우, 반드시 모든 타입 매개 변수에 주석을 추가**해야 한다.
  * **열거 타입을 문서화하는 경우, 열거 타입 자체는 물론 열거 타입의 상수들과 public 메소드에도 주석을 추가**해야 한다.
  * **어노테이션 타입을 문서화하는 경우, 어노테이션 타입 자체는 물론 멤버들에게도 모두 주석을 추가**해야 한다.
* **API 문서화에서 자주 누락되는 설명은 두 가지로, 스레드 안전성과 직렬화 가능성**이다.
  * 클래스 또는 정적 메소드가 스레드 안전하든 그렇지 않든 간에 반드시 스레드 안전 수준을 API 설명에 포함시켜야 한다.
  * 직렬화 가능한 클래스의 경우, 직렬화 형태를 API 설명에 함께 기술한다.
* **상술한 원칙을 모두 준수한 깔끔한 문서를 작성했더라도, 반드시 자바독 유틸리티가 생성한 웹페이지를 직접 읽어봐야 한다**.
  * 프로그램을 테스트하면 수정해야 할 부분이 눈에 띄듯이, 생성된 API 문서를 읽어보면 수정해야 할 주석이 눈에 띄게 된다.
* 이렇듯 **문서화 주석은 자신이 개발한 API를 문서화하는 가장 훌륭하고 효과적인 방법**이다.