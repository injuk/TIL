...
### 반환 타입으로 스트림보다는 컬렉션을 사용하기
* 일련의 원소 시퀀스를 반환하는 메소드는 수 없이 많지만, Java 7 까지는 컬렉션 인터페이스를 사용하는 것으로 귀결되곤 했다.
  * 예외적으로 향상된 for문에서만 사용하거나 반환된 원소 시퀀스들이 일부 Collection 메소드를 구현할 수 없는 경우에는 Iterable을 사용했다.
  * 또는 성능이 중요하거나, 원소들이 기본 타입이라면 배열을 사용했다.
* Java 8 이후 스트림 API가 추가되면서 이러한 결정을 내리는 것은 매우 복잡해졌다.
* 상술했듯 스트림 API는 반복을 지원하지 않으므로, for 반복문과 적절히 조합했을 때 좋은 코드가 나오는 특징이 있다.
  * 향상된 for문으로 스트림을 반복할 수 없는 이유는 스트림이 Iterable을 확장하지 않기 때문이다.
* **객체 시퀀스를 반환하는 메소드를 작성할 때, 해당 메소드가 항상 스트림 파이프라인에서만 사용되는 것이 확실시된다면 스트림을 반환**해도 좋다.
  * 반대로 반환된 객체들이 반복문에서만 사용될 것이 확실하다면 Iterable을 반환한다.
* 그러나 **공개 API를 작성하는 경우, 스트림 파이프라인을 사용하는 개발자와 반복문을 사용하는 개발자를 모두 배려**해야 한다.
* Collection 인터페이스는 Iterable의 하위 타입인 동시에 stream 메소드를 제공하므로, 반복과 스트림 모두를 지원할 수 있다.
  * 따라서 **원소 시퀀스를 반환하는 공개 API의 반환 타입에는 두 경우에 모두 대응할 수 있는 Collection이나 그 하위 타입을 사용하는 것이 최선**이다.
* 만약 컬렉션에 포함되는 시퀀스의 크기가 메모리에 올려도 될 만큼 충분히 작다면 ArrayList와 같은 표준 컬렉션 구현체를 반환한다.
  * 그러나 **시퀀스의 크기가 너무 크다면, 아무런 조치 없이 컬렉션을 반환하지는 말아야** 한다.
* **반환할 시퀀스의 크기가 크지만 표현을 간단하게 바꿀 수 있다면 AbstractCollection과 같은 추상 컬렉션을 사용하여 전용 컬렉션을 구현할지 검토**한다.
  * 이 경우, Iterable 용 메소드 외에 contains와 size를 구현해야 한다.
  * **contains와 size를 구현할 수 없는 요구사항의 경우에는 스트림이나 Iterable 중 더 자연스러운 쪽을 반환하는 것이 바람직**하다.
* **추후 Java 언어 차원에서 Stream 인터페이스가 Iterable을 지원하도록 수정된다면 그 때에는 스트림을 반환**하도록 한다.

### 스트림의 병렬화는 주의하기
* Java 언어는 다른 주류 언어에 비해 동시성 프로그래밍 측면에서 항상 앞서갔지만, 이를 올바르고 빠르게 작성하기는 여전히 어려운 작업이다.
* **스트림은 잘 못 병렬화하면 성능이 나빠질 뿐만 아니라 결과 자체가 잘못되기 쉽다**.
* 주로 스트림의 소스가 참조 지역성이 높은 경우에 병렬화의 효과를 볼 수 있다.
  * 이는 ArrayList, HashMap, HashSet, ConcurrentHashMap 인스턴스 또는 배열, int 범위, long 범위 등이 포함된다.
  * 이 중 **참조 지역성이 가장 뛰어난 것은 기본 타입의 배열로, 참조가 아닌 값 자체가 메모리에 연속되서 저장되기 때문**이다.
* 스트림 파이프라인의 종단 연산 역시 병렬화의 효과에 큰 영향을 주는 요소이다.
  * 축소 연산과 match 연산은 적합하며, 가변 축소를 수행하는 collect 메소드는 컬렉션을 합치는 부담이 크기에 적합하지 않다.
* **상술한 모든 조건을 만족하더라도 파이프라인의 작업이 병렬화에 필요한 비용을 상쇄하지 못한다면 스트림 병렬화에 의한 성능 향상은 미미**할 수 있다.
  * 이를 간단하게 예측하려면 스트림의 원소 수와 원소 별 수행되는 코드를 곱한 값을 확인할 수 있다.
  * **일반적으로 원소 수와 원소 당 코드 수를 곱한 값이 최소 수십만에 이르러야 스트림 병렬화에 의한 성능 향상을 체감**할 수 있다.
* 이렇듯 **스트림 병렬화는 오직 성능 최적화 수단이며, 실제로 스트림 파이프라인을 병렬화하는 작업은 자주 발생하지 않는 점을 기억**해야 한다.
  * 이는 스트림을 절대 병렬화하지 말라는 의미가 아니며, 조건이 잘 맞춰지면 명시적인 parallel 메소드의 호출만으로 큰 성능 향상을 체감할 수 있다.

### 스트림 병렬화의 결론
* **연산 결과도 올바르고 성능도 빨라진다는 확인이 없으면 스트림 파이프라인 병렬화는 시도조차 하지 않는 것이 바람직**하다.
* 잘못 병렬화된 스트림은 정상적으로 동작하지 않으며, 성능도 떨어트리는 요인이다.
* **병렬화하는 것이 좋다는 확신이 있더라도 코드를 이중으로 검증하며, 운영 환경과 유사한 조건에서 많은 테스트를 수행하여 성능 지표를 확인**한다. 
* **스트림을 병렬화할 수 있고, 병렬화하는 것이 옳다는 확신과 이를 뒷받침할 테스트 결과가 충족되었을 때에만 병렬화 코드를 운영 환경에 반영**해야 한다.