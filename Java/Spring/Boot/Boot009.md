...
## 2025-09-06 Sat
### 거짓 알림은 제거하기
* 모니터링을 도입하던 초기에 테스트를 위해 낮은 기준을 적용하는 경우를 예로 들었을 때, 휴먼 에러에 의해 이러한 알림이 운영 환경에 그대로 적용될 수 있다.
    * 예를 들어, CPU 사용량이 30%만 초과하더라도 알림이 오도록 설정했을 수 있다.
* 실무적인 관점에서 이러한 거짓 알림은 큰 의미를 갖지 못하기도 하지만, 알림 자체에 무뎌져 정말 심각한 알림이 발생한 경우의 대처에 악영향을 줄 수 있다.
    * 거짓 알림이 지속적으로 누적될 경우, 개발자들이 쓸모 없는 알림이 수신된 것으로 착각하는 데에 그치지 않고 모든 알림을 무시하는 상황에까지 이를 수 있다.
    * 양치기 소년의 일화에서 알 수 있듯, 거짓이 지속되면 정말 중요한 진실의 의미가 퇴색될 수 있다.