...
### 참고 - 인터페이스는 항상 필요할까?
```
> '이론적으로는' 모든 객체에 인터페이스를 도입하여 역할과 구현을 명확히 분리하는 것이 바람직하다.
> 그러나 실제로는 반드시 인터페이스가 필요한 것은 아니라는 점을 기억해둘 필요가 있다.
```
* 인터페이스는 구현을 변경할 가능성이 있는 객체에 대해서 매우 효과적이다.
  * 그러나 실무에서는 구현을 다시는 변경할 필요가 없는 클래스도 많이 작성하며, 이러한 경우에는 인터페이스가 그다지 실용적이지 않다고 볼 수도 있다.
* 어떠한 **구현체를 작성하기 전에 항상 기계적으로 인터페이스를 작성하기보다는 변경 가능성을 포함한 여러 이유를 고려하여 결정하는 것이 바람직**하다.

## 2024-09-23 Mon
### 프록시의 단점과 동적 프록시
* 상술한 프록시의 경우 클라이언트와 서버 측 코드를 수정하지 않고도 원하는 부가 기능을 추가할 수 있다는 큰 장점을 얻을 수 있다.
* 그러나 **프록시는 인터페이스 또는 구현체 기반 등 어떠한 상황에서도 프록시 대상 서버 클래스마다 프록시를 정의해야한다는 치명적인 단점이 존재**한다.
  * 예를 들어 동일한 부가 기능을 적용하는 경우, 모든 프록시 대상 서버마다 중복 코드가 포함된 프록시를 정의하게 된다.
  * 물론 부가 기능을 적용할 대상 서버 클래스가 많아질수록 프록시 역시 동일한 비율로 증가하게 된다.
* 이 경우 프록시 클래스를 단 하나만 정의하여 모든 곳에 적용할 방법을 고민하게 되며, 동적 프록시는 이러한 문제의 해결책이 될 수 있다.
