...
## 2024-12-28 Sat
### @target 주의사항
* `@target` 지시자를 비롯하여 `args`와 `@args` 지시자는 단독으로 사용하는 대신, 다른 지시자와의 혼용이 권장된다.
  * 예를 들어, `execution(* package..*(..)) && @target(package.MyAnnotation)` 형태로 대상을 줄이는 방식으로 사용하는 것이 바람직하다.
* 이는 **상술한 지시자들은 객체의 인스턴스화가 완료되어 실행되는 시점에 어드바이스 적용 여부를 확인할 수 있기 때문**이다.
  * **런타임 시점의 포인트컷 적용 여부는 결국 프록시의 생성이 전제되며, 반대의 경우에는 여부 판단 자체가 불가능**하다.
* 반면, 스프링 컨테이너는 컨테이너화가 진행되는 애플리케이션 로딩 시점에 프록시를 생성하므로 상술한 지시자에 대해 모든 스프링 빈에 AOP 적용을 시도하게 된다. 
  * 그러나 이 경우, **스프링의 내부 등록 빈 중에는 `final` 등으로 인해 프록시를 생성할 수 없는 빈이 존재하므로 오류가 발생**하게 된다.
  * 따라서 상술한 지시자들을 활용하는 경우, 표현식은 반드시 프록시 적용 대상을 최대한 축소하는 형태로 작성되어야 한다.

## 2024-12-29 Sun
### @annotation과 @args 기반 표현식
