...
* `@target` 지시자를 비롯하여 `args`와 `@args` 지시자는 단독으로 사용하는 대신, 다른 지시자와의 혼용이 권장된다.
  * 예를 들어, `execution(* package..*(..)) && @target(package.MyAnnotation)` 형태로 적용 대상을 줄이는 방식으로 사용하는 것이 바람직하다.
* 이는 **상술한 지시자들은 객체의 인스턴스화가 완료되어 실행되는 시점에 어드바이스 적용 여부를 확인할 수 있기 때문**이다.
  ...
  * 반면, 스프링 컨테이너는 컨테이너화가 진행되는 애플리케이션 로딩 시점에 프록시를 생성한다.
* 때문에 상술한 지시자들에 대해 스프링은 모든 스프링 빈에 AOP 적용을 시도하나, 스프링의 내부 등록 빈 중에는 `final` 등으로 인해 프록시를 생성할 수 없는 빈이 존재하므로 오류가 발생하게 된다.
