...
## 2024-12-12 Thu
### @Around 어드바이스란?
* 메소드 실행 전후에 수행되며, 다른 것과 비교했을 때 가장 강력한 어드바이스에 속한다.
  * 예를 들어, 조인 포인트의 실행 여부와 전달 값의 변환 및 반환 값 변환에 더해 에외 변환과 트랜잭션 처리까지 모두 가능하다.
* 이 때, `@Around` 어드바이스는 반드시 첫 번째 파라미터로 `ProceedingJoinPoint`를 전달해주어야 한다.
  * 해당 클래스는 `proceed()` 메소드를 제공하므로, 이를 한 번 이상 호출하는 것으로 단순히 대상을 실행하거나 여러 번 호출하여 재시도할 수 있다. 

## 2024-12-13 Fri
### 동일한 애스팩트 안에서의 조인 포인트 우선 순위
* 스프링은 5.2.7 버전부터 동일한 `@Aspect` 내부에서 정의된 조인 포인트에 아래와 같은 우선 순위를 할당한다.
  1. `@Around`
  2. `@Before`
  3. `@After`
  4. `@AfterReturning`
  5. `@AfterThrowing`
* 이 때, **상술한 순서는 어디까지나 어드바이스의 적용 순서이므로 호출 순서와 반환 순서는 정반대**가 된다.
* 반면, **동일한 애스팩트 내부에 동일한 유형의 어드바이스가 둘 이상 존재할 경우 순서는 보장되지 않는 점에 주의**를 기울여야 한다.
  * 이 경우에도 순서를 보장하고자 하는 경우, 앞서 다룬 방법을 활용하여 `@Aspect`를 분리하고 `@Order` 어노테이션을 활용해주어야 한다.

## 2024-12-14 Sat
### @Around 이외의 어드바이스 유형이 갖는 의의
* `@Around` 어드바이스가 굉장히 범용적으로 설계되었기에 다양한 상황에서 활용될 수 있는 것은 사실이나, 개발자에 의한 휴먼 에러 발생 가능성이 있다.
  * 예를 들어, **`@Around`는 반드시 `joinPoint.proceed()`를 호출해주어야만 대상 클래스를 호출**한다.
* 반면, `@Before` 또는 `@After` 등의 어드바이스는 적은 기능을 제공하는 대신 상술한 실수가 발생할 가능성이 원천적으로 봉쇄된다.
* 무엇보다도 **코드가 단순하며, 어드바이스의 유형 자체로부터 코드의 의도가 명확히 전달된다는 가독성 측면의 장점 역시 무시할 수 없다**.
  * 예를 들어, 어떠한 동작 이전에 관점을 적용하는 경우 `@Around`보다는 `@Before` 어드바이스가 의도를 더 명확히 전달한다.
* **좋은 설계에는 언제나 제약이 따르며, 이러한 제약은 개발 생산성을 낮추기보다는 휴먼 에러를 미연에 방지한다는 점에 초점을 두는 것이 바람직**하다.
  * 이러한 제약으로 인해 코드의 역할 역시 명확해지며, 가독성 향상으로 인해 코드의 유지보수성 역시 함께 증대되는 효과가 있다.