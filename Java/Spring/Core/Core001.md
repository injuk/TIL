...
## 2022-06-20 Mon
### 스프링과 객체 지향 설계
* **스프링은 DI 컨테이너를 제공함으로써 개방 폐쇄 원칙과 의존성 역전 원칙을 가능케한다**.
  * DI 컨테이너는 Java 객체들을 어떠한 컨테이너에 넣어두고, 의존 관계를 관리하는 기능을 제공한다.
  * 즉, 클라이언트 코드의 변경 없이 기능의 확장이 가능하며, 마치 부품을 교체하듯이 쉽게 개발을 진행할 수 있다.
* **스프링을 사용하지 않는다면 개방 폐쇄 원칙과 의존성 역전 원칙을 완전히 준수하기 위해 필요한 코드가 너무 많다. 배보다 배꼽이 크다**!
  * 때문에 이를 프레임워크화한 것이 스프링이다.
  * 스프링을 사용하지 않고 순수 Java 만으로 개발을 진행하더라도, 두 원칙을 지키기 위해서는 결국 DI 컨테이너를 제작하게 된다.

### 좋은 애플리케이션 설계하기
* **좋은 애플리케이션을 만들기 위해서는 모든 객체 설계시에는 역할과 구현을 분리하고, 이상적으로는 모든 설계에 인터페이스를 적용**해야 한다.
  * 이를 적용할 경우, 어떠한 데이터베이스를 사용할지 결정되지 않은 상태에서도 우선 인터페이스를 정의해두는 것으로 개발 진행이 가능하다.
  * **가장 이상적인 상태는 우선 모든 인터페이스를 정의해둔 후에 개발을 진행하는 것**이다.
* 그러나 **인터페이스 자체에는 언제나 추상화라는 비용이 수반되는 점 역시 고려**해야 한다.
  * 추상화라는 비용은 인터페이스를 도입하는 것으로 인해 발생하는 성능 문제가 아니다.
  * 예를 들어, 추상화가 잘 적용된 코드를 처음 읽는 개발자는 어떠한 방법으로든 실제 구현체를 확인하는 데에 더 많은 비용이 들게 된다.
* **기능을 확장할 가능성이 전혀 없는 경우, 우선 구체 클래스를 직접 사용하고 추후에 인터페이스를 추출하는 리팩토링을 적용하는 방법을 고려할 수도 있다**.
  * 반면, 기능을 확장할 가능성이 설계 당시부터 예상되는 경우라면 반드시 인터페이스를 정의하고 진행해야 한다.