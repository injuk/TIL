...
* **n개의 컬럼을 사용하는 다중 컬럼 인덱스의 경우, 다음과 같은 조건 아래에서 작업 범위 결정 조건으로 인덱스를 사용**할 수 있다.
  ...
  2. **n보다 작은 임의의 컬럼인 i 번째 컬럼에 대해 =, IN, <, >, 또는 LIKE를 활용한 좌측 일치 패턴으로 비교**하는 경우
     * 이 때, **좌측 패턴 일치를 활용하는 LIKE 비교는 사실상 < 또는 > 비교와 동급으로 이해**할 수 있다.
...
### UQ 인덱스와 일반적인 세컨더리 인덱스의 읽기 성능 비교
```
...
* 그러나 다음과 같은 경우, UQ 인덱스의 생성은 지양하는 것이 바람직하다.
  1. UQ 인덱스를 추가하는 것으로 성능이 좋아질 것이라고 착각하는 경우
  2. 동일한 테이블에서 완전히 같은 컬럼에 대해 UQ 인덱스와 세컨더리 인덱스를 중복해서 생성한 경우
  3. 동일한 테이블에서 완전히 같은 컬럼에 대해 UQ 인덱스와 PK를 중복해서 생성한 경우
* 특히, **UQ 인덱스 역시 일반적인 세컨더리 인덱스와 완전히 같은 역할을 수행할 수 있으므로 2.와 3.과 같은 중복은 지양**해야 한다.
  * 이렇듯 **반드시 필요하지 않은 경우에는 UQ 인덱스보다는 그렇지 않은 세컨더리 인덱스의 생성을 고려하는 것이 바람직**하다.

### MySQL에서의 FK
* **MySQL에서, FK는 InnoDB 스토리지 엔진에서만 생성이 가능**하다.
  * 이 때, FK 제약 조건이 설정되면 자동으로 연관되는 테이블의 컬럼에도 인덱스가 생성된다.
  * 또한, **FK가 제거되지 않은 상태라면 자동으로 생성된 상술한 인덱스를 삭제**할 수 없다.
* InnoDB에서, FK 관리에는 다음과 같은 주요한 특징이 존재한다.
  1. 테이블의 변경으로 인해 쓰기 잠금이 발생하는 경우에만 잠금 경합(잠금 대기)이 발생한다.
  2. FK와 연관되지 않은 컬럼에 대한 변경은 최대한 잠금 경합을 발생시키지 않는다.

### 자식 테이블의 변경 사항이 대기하는 경우
```
> 자식 테이블의 FK 컬럼에 대한 INSERT 또는 UPDATE 등의 변경사항은 반드시 부모 테이블의 확인이 필요하다.
> 이렇듯 자식 테이블의 FK 컬럼에 대한 변경이 부모 테이블 레코드에 걸린 쓰기 잠금이 해제될 때까지 기다리는 것을 잠금 확장이라고 한다.
```
* 예를 들어, 자식 테이블이 FK 컬럼을 변경하고자 했으나 이미 다른 커넥션에서 부모 테이블의 해당 레코드에 대해 쓰기 잠금을 걸어둔 경우가 있다.
  * 이 경우, 자식 테이블의 변경은 부모 테이블에 걸린 해당 레코드에 대한 쓰기 잠금이 해제될 때까지 기다린다.
  * 즉, **우선 부모 테이블의 해당 레코드에 쓰기 잠금을 획득한 커넥션이 ROLLBACK 또는 COMMIT을 통해 트랜잭션을 종료해야 한다**.
  * **부모 테이블의 쓰기 잠금을 획득한 커넥션의 트랜잭션이 종료된 경우에야 비로소 자식 테이블에서 대기 중이던 쓰기 변경 사항이 처리**될 수 있다.
* **반면, 자식 태이블의 FK가 아닌 컬럼에 대한 변경은 FK로 인한 잠금 확장이 발생하지 않는다**.

### 부모 테이블의 변경 사항이 대기하는 경우
* 상술한 시나리오에서 순서가 뒤바뀌어 자식 테이블에 대한 변경 사항의 적용을 부모 테이블이 대기하는 경우도 발생할 수 있다.
* 이 경우, **자식 테이블의 FK 제약 조건에 `ON DELETE CASCADE` 제약 조건이 적용되었다면 부모 테이블 역시 자식 테이블의 변경 완료를 대기**해야 한다.
  * 이는 `ON DELETE CASCADE` 제약 조건으로 인해 부모 레코드가 삭제되는 경우 자식 레코드 역시 동시에 삭제되는 식으로 동작하기 때문이다.

### FK 생성 시 주의사항
* **MySQL에서 FK를 물리적으로 생성하는 경우, 상술한 바와 같은 잠금 경합을 반드시 고려하여 모델링을 진행하는 것이 바람직**하다.
* **물리적으로 FK 컬럼을 생성하는 경우, 자식 테이블에 레코드가 추가되는 시점에 부모 테이블에 해당 참조 키가 실제로 존재하는지 확인**한다.
  * 그러나 물리적인 FK 생성시의 고려 사항은 이러한 당연한 사실만을 확인하는 것을 의미하지 않는다.
  * **오히려 이러한 체크 작업이 아닌, 이러한 체크를 위해 연관된 테이블에 읽기 잠금을 건다는 사실을 고려**해야 한다.
  * 또한, **이러한 테이블 간의 잠금 확장은 그 존재만으로 인해 전체적으로 쿼리의 동시 처리에 영향을 줄 수 있다는 사실을 반드시 이해**해야 한다.