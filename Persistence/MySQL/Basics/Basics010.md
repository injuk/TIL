...
* 일반적으로 **쿼리를 커버링 인덱스로 처리할 수 있는 경우와 그렇지 못한 경우의 차이는 적게는 수십 배에서 크게는 수백 배까지 날 수 있다**.
  ...
  * 이 경우, 인덱스의 크기가 필요 이상으로 커져 메모리 낭비가 심해지고 레코드에 대한 변경 작업이 너무 느려질 수 있는 등의 위험한 상황이 발생할 수 있다.
* 접근 방법이 eq_ref나 ref, range, index 등 인덱스를 사용하는 계획에서는 모두 Extra 컬럼에 Using index가 표시될 수 있다.
  * 즉, **인덱스 레인지 스캔뿐만 아니라 인덱스 풀 스캔을 사용하는 경우에도 커버링 인덱스가 적용**될 수 있다.
  * 또한, 이러한 경우 역시 커버링 인덱스를 사용하지 못하는 인덱스 풀 스캔에 비해서 훨씬 빠른 처리가 가능하다.

### InnoDB와 클러스터링 인덱스
* **InnoDB 엔진을 사용하는 모든 테이블은 클러스터링 인덱스로 구성되며, 모든 세컨더리 인덱스는 레코드의 실제 주소값 대신 PK 값을 갖는다**.
  * 때문에 **임의의 컬럼 하나만을 갖는 인덱스를 생성하더라도 실제로는 PK 값이 함께 저장되는 효과**를 갖는다.
  * 이러한 클러스터링 인덱스의 특징으로 인해 MySQL에서의 쿼리는 커버링 인덱스로 처리될 가능성이 높다.
* 상술한 이유에서, **InnoDB의 세컨더리 인덱스는 레코드를 가리키는 주소로 사용하기 위한 PK로 인해 인덱스가 추가 컬럼을 하나 더 갖는 효과**를 얻을 수 있다.

### index 접근 방법과 Using index
* 두 개념은 유사한 이름으로 인해 혼동되기 쉽지만, 성능 상 다음과 같은 큰 차이가 있다.
  1. index 접근 방법: type 컬럼에 표시되며, 인덱스 풀 스캔으로 처리하므로 인덱스 레인지 스캔보다 훨씬 느리다.
  2. Using index: Extra 컬럼에 표시되며, 커버렁 인덱스이므로 커버링 인덱스를 사용하지 않는 경우보다 훨씬 빠르게 처리된다.
* 또한, **Using index로 표현되는 커버링 인덱스는 실행 계획의 type 컬럼의 내용과 관계 없이 사용**될 수 있다.

### Extra 컬럼 - Using index condition
* 해당 내용은 옵티마이저가 인덱스 컨디션 푸시 다운 최적화를 적용한 경우에 표시된다.
  * 이 때, 인덱스 컨디션 푸시 다운이란 인덱스를 범위 제한 조건으로 사용하지 못하는 경우에도 인덱스에 포함된 컬럼에 대한 조건이 있는 경우에 사용된다.
  * 인덱스 컨디션 푸시 다운 최적화가 적용된 경우, 인덱스에 포함되는 컬럼에 대한 조건이 있는 경우 이를 모두 모아 스토리지 엔진으로 전달한다.
  * 이렇듯 **추가적인 인덱스 컬럼을 전달받은 스토리지 엔진은 전달 받은 인덱스를 활용하여 최대한 필터링까지 마친 후에 결과를 반환**하게 된다.

### Extra 컬럼 - Using index for group-by
* GROUP BY 절의 처리를 위해 MySQL 서버는 다음과 같은 작업을 수행한다.
  1. 우선 그룹화 기준 컬럼으로 정렬을 수행한다.
  2. 정렬된 결과를 그룹화한다.
* **이러한 작업은 기본적으로 부하가 크게 걸리는 고부하 작업에 해당하므로, 성능 상 아쉬움**이 있을 수 밖에 없다.
* 그러나 **GROUP BY의 처리가 이미 정렬되어 저장된 B-Tree 인덱스를 활용하는 경우, 정렬된 인덱스 컬럼을 단지 순서대로 조회하여 그룹화만 수행**할 수 있다.
  * 이 경우, 당연히 **레코드에 대한 정렬 없이 인덱스로부터 필요한 부분만 조회하면 되므로 상당히 효율적이고 빠르게 처리**될 수 있다.
  * 또한, 이렇듯 정렬된 인덱스를 활용하여 GROUP BY 절이 처리되는 경우 Extra 컬럼에는 해당 내용이 표시된다.

### 타이트 인덱스 스캔을 활용하는 GROUP BY 처리
* **인덱스를 통해 GROUP BY 절을 처리할 수 있다고 하더라도 AVG()나 SUM(), COUNT()와 같이 조회 대상 값이 모든 인덱스를 조회해야 하는 경우**가 있다.
  * 이 경우에는 필요한 레코드만 듬성하게 조회할 수 없으므로, GROUP BY 절에 대한 처리를 위해 인덱스를 사용하더라도 루스 인덱스 스캔이라고 지칭하지 않는다.
  * 때문에 **이러한 쿼리의 실행 계획의 Extra 컬럼에서는 상술한 Using index for group-by 메시지를 확인할 수 없다**.

### 루스 인덱스 스캔을 활용하는 GROUP BY 처리
* 다음과 같은 경우, 인덱스를 듬성듬성하게 조회하는 루스 인덱스 스캔을 적용할 수 있다.
  1. 단일 컬럼으로 구성된 인덱스에서 그룹화 컬럼 이외에는 아무것도 조회하지 않는 쿼리
  2. **다중 컬럼으로 구성된 인덱스에서 GROUP BY 절이 인덱스를 사용할 수 있고 MIN(), MAX()와 같이 인덱스 첫 값 또는 마지막 값만 조회해도 되는 경우**
* 이 때, GROUP BY 절에서 인덱스를 사용하기 위해서는 우선 GROUP BY 절에서 인덱스를 사용할 수 있는 요건이 갖춰저야만 한다.
* 그러나 **중요한 것은 우선 WHERE 절에서 사용하는 인덱스에서도 GROUP BY 절의 인덱스 사용 여부가 다음과 같이 영향을 받는다는 사실**이다.
  1. WHERE 절이 없는 경우: GROUP BY 절의 컬럼과 SELECT로 조회하는 컬럼이 루스 인덱스 스캔을 사용할 수 있는 조건만 갖추어도 무방하다.
  2. WHERE 절이 있지만 인덱스를 사용하지 못하는 경우: 우선 GROUP BY를 위해 인덱스를 조회한 후에 WHERE 절을 처리하기 위해 레코드를 조회한다.
     * 즉, **이 경우에는 첫 레코드 또는 마지막 레코드만으로 처리가 불가능하여 루스 인덱스 스캔으로 처리될 수 없어 타이트 인덱스 스캔을 활용**해야 한다.
  3. WHERE 절이 있고 인덱스를 사용하는 경우: index_merge 이외에는 하나의 인덱스만 사용할 수 있으므로 GROUP BY가 인덱스를 사용할 조건이 까다로워진다.
     * 이 경우, **WHERE 절과 GROUP BY 절의 처리가 모두 같은 인덱스를 공통으로 사용할 수 있는 경우에만 루스 인덱스 스캔이 가능**하다.
     * 반면 **서로 다른 인덱스를 사용하는 경우, 일반적으로 옵티마이저는 WHERE 절이 인덱스를 사용하도록 실행 계획을 수립하는 경향**을 보인다.
     * 설령 **작업 범위를 전혀 좁히지 못하는 WHERE 절이 주어지더라도 GROUP BY 절보다 우선적으로 인덱스를 사용하도록 실행 계획이 수립**되곤 한다.
* 반면, **WHERE 절과 GROUP BY 절이 모두 같은 인덱스를 사용할 수 있는 쿼리에서도 루스 인덱스 스캔이 사용되지 않을 수도 있다**.
  * 예를 들어, WHERE 절에서 검색된 레코드 수가 적은 경우 루스 인덱스 스캔을 적용하지 않더라도 매우 빠르게 처리된다.
  * 이렇듯 **루스 인덱스 스캔은 대량의 레코드에 대한 그룹화 시에 큰 성능 향상을 보이므로, 옵티마이저는 적절한 손익 분기점을 판단하는 식으로 동작**한다.

### Extra 컬럼 - Using index for skip scan
* 해당 내용은 MySQL 8.0 버전에서 도입된 인덱스 스킵 스캔을 사용한 경우를 의미하며, 이는 루스 인덱스 스캔 최적화를 확장하는 최적화에 해당한다.
  * 인덱스 스킵 스캔이란, 옵티마이저가 인덱스의 선행 컬럼 없이 인덱스 후행 컬럼만으로도 인덱스를 사용할 수 있도록 하는 최적화 기능이다.
  * 이는 마치 **선행 컬럼에 대한 동등 조건이 AND 조건으로 연결된 것처럼 동작하여 인덱스를 사용할 수 있도록 하므로, 선행 컬럼의 값의 종류가 적어야** 한다.

### Extra 컬럼 - Using join buffer(Block Nested Loop), Using join buffer(Batched Key Access), Using join buffer(hash join)
```
> 해당 내용은 쿼리가 조인을 수행하기 위해 조인 버퍼를 활용했다는 것을 의미한다.
> 이 때, 괄호 안의 내용은 조인 버퍼를 활용하는 동시에 어떤 조인 방식으로 쿼리를 처리했는지를 의미한다.
```
* 일반적으로 조인되는 컬럼은 빠른 쿼리 실행을 위해 인덱스를 생성해야 한다.
  * 이 때, **조인에 필요한 인덱스는 조인에 사용되는 두 테이블의 컬럼 모두에 필요한 것이 아니라 드리븐 테이블의 컬럼에만 필요**하다.
  * **옵티마이저는 조인되는 두 테이블의 컬럼으로부터 인덱스를 조사하고, 인덱스가 없는 테이블이 있으면 해당 테이블을 먼저 조회하여 조인을 실행**한다.
  * 이는 드라이빙 테이블에 비해 드리븐 테이블이 검색 위주로 사용되므로, 인덱스가 없으면 조인 성능에 매우 큰 영향을 주기 때문이다.
* 이 때, **드리븐 테이블의 조인 컬럼에 적절한 인덱스가 존재하지 않는 경우 MySQL 서버는 블록 네스티드 루프 조인 또는 해시 조인을 사용**한다.
  * 반면, 드리븐 테이블에 적절한 인덱스가 있다면 문제될 것은 아무것도 없다.
  * 또한 **블록 네스티드 루프 조인과 해시 조인은 모두 조인 버퍼를 사용하며, 이 경우 Extra 컬럼에는 Using join buffer가 표시**된다.
* `join_buffer_size` 시스템 변수에서 최대로 할당 가능한 조인 버퍼 크기를 설정할 수 있으나, 조인 시 인덱스를 활용할 수 있다면 이를 신경 쓸 필요가 없다.
  * 반면, **조인 버퍼를 사용해야 하는 경우에는 일반적인 웹 서비스에서 너무 적거나 과하지 않은 1MB 정도의 크기를 설정해두는 것이 바람직**하다.
* MySQL 8.0 버전에서는 역시 조인 버퍼를 활용하는 해시 조인이 도입되었으며, DW 등 대용량 쿼리를 실행하는 경우 조인 버퍼를 더 크게 설정해야 한다.
* 해당 문구는 이전부터 존재했으나, 5.6 버전부터는 Batched Key Access와 Hash join이 도입되어 조인 알고리즘이 괄호 안에 표시되도록 개선되었다.

### Extra 컬럼 - Using MRR
* MySQL 엔진은 실행 계획을 수립한 후 이에 맞추어 스토리지 엔진의 API를 호출하여 쿼리를 처리한다.
  * 그러나 스토리지 엔진 단위로는 쿼리 실행의 전체적인 큰 그림을 알 수 없으므로, 최적화에는 그만큼 어려움이 따른다.
  * 때문에 아무리 많은 레코드를 조회하는 과정이더라도 스토리지 엔진은 MySQL 엔진이 넘겨주는 키를 기준으로 레코드를 한 건씩 읽어 반환하는 식으로 동작한다.
* MySQL 서버는 이러한 단점을 보완하기 위해 Multi Range Read라는 최적화 기법을 도입한다.
  * **해당 최적화는 여러 키 값을 한 번에 스토리지 엔진으로 전달하고, 스토리지 엔진은 이를 정렬하여 최소한의 페이지 접근으로 필요한 레코드를 조회**한다.
  * MySQL 엔진으로부터 전달받은 키를 우선 정렬하여 버퍼링하므로, MRR 최적화 기법이 도입되면 각각의 스토리지 엔진은 디스크 IO를 최소화할 수 있게 된다.
* **Extra 컬럼에 해당 내용이 표시되는 경우, 드라이빙 테이블로부터 읽은 레코드를 토대로 드리븐 테이블 검색을 위한 키 값을 모아 MRR 엔진에 전달**하게 된다.
  * **MRR 엔진은 전달 받은 키 값을 정렬하여 드리븐 테이블을 최적화된 방법으로 접근하게 되므로, 드라이빙 테이블의 Extra 컬럼에 해당 내용이 표시**된다.

### Extra 컬럼 - Using sort_union, Using union, Using intersect
* 쿼리의 실행 계획에서 type 컬럼이 index_merge로 표기되는 경우, 둘 이상의 인덱스가 동시에 사용될 수 있다.
  * 이 때, 실행 계획의 Extra 컬럼에서는 각 인덱스로부터 읽어들인 내용을 어떻게 병합했는지 더 상세하게 설명하기 위해 해당 내용을 표시한다.
* type 컬럼이 index_merge로 표시된 경우, Extra 컬럼에는 다음과 같은 내용 중 하나가 출력될 수 있다.
  1. Using intersect: 각 인덱스를 사용할 수 있는 조건이 AND로 연결된 경우, 각 처리 결과에서 교집합을 추출하도록 작업이 수행된다.
  2. Using union: 각 인덱스를 사용할 수 있는 조건이 OR로 연결된 경우, 각 처리 결과에서 합집합을 추출하도록 작업이 수행된다. 
  3. Using sort_union: 2.와 같은 작업을 수행하지만, 2.의 방식으로 처리될 수 없는 경우 이 방식으로 처리된다.
     * **해당 방식은 2.와 달리 PK만 먼저 조회하여 정렬하고 병합한 후에야 비로소 레코드를 읽어 반환한다는 차이점이 존재**한다.
* 이 때, Using union과 Using sort_union은 모두 인덱스를 사용할 수 있는 조건이 OR 조건으로 연결된 경우에 사용된다.
  * 일반적으로 Using union은 일치하는 레코드 수가 상대적으로 많지 않고, Using sort_union은 < 또는 > 등 상대적으로 많은 경우에 사용된다.
  * 그러나 **대부분의 경우, 레코드 수와 관계 없이 WHERE 절에 동등 조건이 사용되면 Using union이 사용**된다.
  * **반면, < 또는 > 등 동등 조건 이외의 조건이 사용된 경우 대부분 Using sort_union이 사용**된다.

### Extra 컬럼 - Using temporary
```
> 해당 내용은 임시 테이블을 사용했음을 의미한다.
> 이 때, 임시 테이블이 메모리 상에 생성되었는지 디스크 상에 생성되었는지는 실행 계획만으로는 판단할 수 없다. 
```
* **MySQL 서버에서는 쿼리를 처리하는 동안 중간 결과를 담기 위해 임시 테이블을 사용할 수도 있으며, 이는 메모리 또는 디스크 상에 생성**될 수 있다.
  * 예를 들어, **인덱스를 사용하지 못하는 GROUP BY 절을 포함하는 쿼리는 임시 테이블을 사용하는 대표적인 형태의 쿼리에 해당**한다.
* 반면, 실행 계획의 Extra 컬럼에 해당 내용이 표시되지 않았더라도 내부적으로는 임시 테이블을 사용하는 경우도 많다.
  * 때문에 **해당 내용이 Extra 컬럼에 표시되지 않았다고 해서 절대 임시 테이블을 사용하지 않았다고 확신하지 않도록 주의**를 기울여야 한다.
* 이 때, 메모리 또는 디스크에 임시 테이블을 생성하여 처리하는 쿼리의 예시는 다음과 같다.
  1. FROM 절에 서브쿼리가 사용된 경우: 이는 일반적으로 **파생 테이블이라고 지칭하나, 실체는 임시 테이블이므로 무조건 임시 테이블을 생성**한다.
  2. `COUNT(DISTINCT 컬럼명)` 형태를 포함하는 쿼리: 해당 쿼리가 인덱스를 사용할 수 없는 경우, 임시 테이블이 생성된다.
  3. UNION 또는 UNION DISTINCT를 사용하는 쿼리: 해당 쿼리 역시 임시 테이블을 사용하여 결과를 병합한다.
     * 반면, MySQL 8.0 버전부터는 UNION ALL은 더 이상 내부적으로 임시 테이블을 사용하지 않도록 개선된다.
  4. 인덱스를 사용하지 못하는 정렬 작업: **해당 작업 역시 임시 버퍼 공간을 사용하며, 정렬 대상 레코드가 많아지면 디스크를 사용**하게 된다.
     * **정렬 버퍼 역시 실체는 임시 테이블이며, 쿼리가 정렬을 수행하게 되는 경우 실행 계획의 Extra 컬럼에는 Using filesort가 표시**된다.

## 2022-10-17 Mon
### Extra 컬럼 - Using where
* MySQL 서버 아키텍쳐는 크게 다음과 같은 두 개의 계층으로 나누어 볼 수 있다.
  1. MySQL 엔진
  2. 스토리지 엔진
* 이 때, **각각의 스토리지 엔진은 디스크나 메모리로부터 필요한 레코드를 읽거나 저장하는 역할을 담당**한다.
* 반면, **MySQL 엔진은 스토리지 엔진으로부터 전달받은 레코드를 가공하거나 연산하는 작업을 수행**한다.
  * 이렇듯 **MySQL 엔진 게층에서 별도의 가공 처리를 적용하여 필터링한 경우에만 Extra 컬럼에 Using where 가 표시**된다.
  * 그러나 **스토리지 엔진으로부터 조회한 모든 레코드에 대해 MySQL 엔진이 별도의 필터링 또는 가공을 적용하지 않은 경우, 해당 내용은 표시되지 않는다**.
* 또한 여러 **비교 조건을 작업 범위 결정 조건과 체크 조건으로 구분했을 때, 각 조건은 다음과 같은 계층에서 처리**된다.
  1. 작업 범위 결정 조건: **각 스토리지 엔진 계층에서 처리**된다.
  2. 체크 조건: **MySQL 엔진 계층에서 처리**된다.
* 예를 들어 **다음 쿼리의 경우, `emp_no BETWEEN 10001 AND 10100` 처리는 작업 범위 결정이지만 `gender='F'` 조건은 체크 조건**이 된다.
  * 이는 **gender 컬럼에 대해 적절히 사용할 수 있을만한 인덱스가 존재하지 않기 때문이며, 인덱스가 존재하는 경우에는 작업 범위 결정 조건**이 될 수 있다.
  * **만약 선행 조건을 만족하는 레코드가 100 건이고, 후행 조건까지 만족하는 레코드가 37 건이라면 Using where 처리는 63 건을 버리는 처리를 의미**한다.
  * 즉, 스토리지 엔진은 100 건을 읽어 MySQL 엔진에게 반환했으나 MySQL 엔진은 자체적인 필터링 처리를 적용하여 그 중 63 건을 버리게 된다.
```
EXPLAIN
SELECT * 
FROM employees 
WHERE emp_no BETWEEN 10001 AND 10100  # 이는 작업 범위 결정 조건이므로 스토리지 엔진 계층에서 처리된다. 
AND gender='F';                       # 이는 체크 조건이므로 MySQL 엔진 계층에서 처리된다.
```

### Using where와 함께 쿼리의 성능 파악하기 
* Using where는 실행 계획의 Extra 컬럼에 가장 자주 표시되는 내용이며, 이로 인해 쉽게 무시해버리는 경우가 많다.
* **실제로는 왜 해당 내용이 표시되었는지 이해할 수 없는 경우도 잦으므로, 해당 내용이 실제로 성능 상 악영향을 미치는지 선별하는 능력이 필수적**이다.
  * MySQL 8.0 버전부터는 실행 계획에 filtered 컬럼이 함께 표시되므로, 성능 상 이슈를 보다 쉽게 파악할 수 있다.
  * **이러한 정보를 토대로 필요한 레코드들을 조회하기 위해 무의미한 레코드를 얼마나 많이 조회했는지의 비율을 통해 쿼리의 성능을 파악하는 것이 바람직**하다.

### Extra 컬럼 - Zero limit
* 작업 중에는 MySQL 서버로부터 실제 데이터의 값이 아닌 쿼리 결과의 메타데이터만 필요한 경우가 있다.
  * **예를 들어, 쿼리의 결과에 포함되는 컬럼의 개수 또는 각 컬럼의 타입이 이러한 메타데이터에 해당**한다.
* 이렇듯 **쿼리 결과에 대한 메타데이터만 필요한 경우, 쿼리의 끝에 `LIMIT 0`을 명시하는 것으로 간단하게 메타데이터를 조회**할 수 있다.
* 이 경우, **옵티마이저는 메타데이터만 조회하고자 하는 사용자의 의도를 이해하고 실제 테이블의 레코드를 전혀 조회하지 않은 채 메타데이터만 반환**한다.
  * 이렇듯 **메타데이터만을 조회하기 위해 쿼리의 끝에 `LIMIT 0`을 명시한 쿼리의 실행 계획에서는 Extra 컬럼에서 해당 내용을 확인**할 수 있다.