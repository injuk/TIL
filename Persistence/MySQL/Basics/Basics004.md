...
## 2022-09-23 Fri
### 체인지 버퍼
* DBMS 상에서 **INSERT 또는 UPDATE 등을 통해 데이터 파일이 변경된 경우, 해당 테이블에 포함된 인덱스 역시 업데이트**되어야 한다.
  * 그러나 인덱스 업데이트 작업은 랜덤 디스크 IO가 필수적이므로, 인덱스가 많을수록 해당 작업은 많은 비용이 필요하다.
* InnoDB 스토리지 엔진은 인덱스 페이지가 버퍼 풀에 캐싱되어 있는지 여부에 따라 다음과 같이 변경 작업을 디스크에 반영한다.
  1. 버퍼 풀에 존재하는 경우: 즉시 업데이트를 수행한다.
  2. 존재하지 않는 경우: **디스크로부터 읽어와 업데이트해야 하는 경우이므로, 즉시 업데이트하는 대신 임시 공간에 캐싱한 후 사용자에게 결과를 반환**한다.
* **버퍼 풀에 존재하지 않는 인덱스에 대한 변경은 이렇듯 임시 공간을 활용하여 성능을 향상시키며, 이 때 사용하는 임시 메모리 공간이 체인지 버퍼**이다.
  * 이 때, 사용자에게 결과를 반환하기 전에 반드시 중복성을 확인해야 하는 유니크 인덱스는 체인지 버퍼를 사용할 수 없다.
* 상술한 바와 같이 **체인지 버퍼에 임시로 저장되는 인덱스 레코드 조각은 이후 적시에 `체인지 버퍼 머지 스레드`라고 하는 백그라운드 스레드에 의해 병합**된다.
* MySQL 5.5 버전까지는 INSERT 작업에 대해서만 버퍼링이 가능하였으며, 기본적으로 기능이 활성화된 상태였다.
  * 그 이후 버전에서는 점차 개선되어 UPDATE와 DELETE 문에 대해서도 버퍼링이 가능하며, 시스템 변수가 도입되어 작업 종류 별로 기능을 활성화할 수 있다.
  * 예를 들어, 임의의 DML 작업에서 체인지 버퍼가 비효율적이라면 해당 유형의 DML에 대해서만 체인지 버퍼를 비활성화할 수 있다.
* 체인지 버퍼는 기본적으로 버퍼 풀로 설정된 메모리의 25%까지 사용할 수 있으나, 필요시 50%까지 향상시킬 수 있다.
  * 체인지 버퍼에 너무 많은 공간을 할당하지 않고자 하거나, DML이 자주 실행되어 더 많은 체인지 버퍼 공간을 두고 싶은 경우 시스템 변수를 통해 설정한다.
  
### 많은 DBMS의 특징
* **모든 DBMS의 데이터 파일은 쓰기보다 읽기 성능을 고려하는 자료 구조**를 갖는다.
  * 이로 인해 데이터 파일 쓰기는 랜덤 디스크 IO를 사용할 수 밖에 없다.
  * 즉, **변경된 데이터를 데이터 파일에 기록하는 작업은 읽기 작업에 비해 상대적으로 큰 비용을 필요로 한다**.

### 리두 로그란?
```
> 리두 로그는 트랜잭션의 요소인 ACID 중 영속성을 의미하는 D와 밀접한 연관이 있다.
> 리두 로그는 여러 외부 요인으로 인해 MySQL 서버가 비정상 종료된 경우, 데이터 파일에 기록되지 않은 데이터가 유실되지 않도록 한다.
```
* **MySQL을 포함한 대부분의 데이터베이스 서버는 데이터 변경 내용을 우선 로그 형태로 기록**한다.
  * 그러나 상술한 바와 같이 쓰기 작업은 상대적으로 큰 비용을 필요로 하며, 자칫 로그 기록으로 인해 성능이 저하될 가능성이 존재한다.
  * 때문에 **쓰기 작업으로 인한 성능 저하를 막기 위해 DBMS는 리두 로그에 쓰기 비용이 낮은 형태의 자료 구조를 채택**한다.
* 또한, **데이터베이스 서버는 트랜잭션 ACID 만큼이나 성능도 중요하므로 데이터 파일 이외에도 리두 로그를 버퍼링할 수 있는 여러 자료 구조**를 갖는다.
  * 예를 들어, 리두 로그 자체를 버퍼링하는 InnoDB 버퍼 풀 또는 로그 버퍼와 같은 자료 구조를 갖는다.
* **외부 요인으로 인해 데이터베이스 서버가 비정상 종료된 경우, 서버는 리두 로그를 활용하여 데이터 파일을 서버 종료 직전 상태로 복구**한다.
* 데이터베이스 서버에서 리두 로그는 트랜잭션이 커밋되는 시점에 즉시 디스크로 기록되도록 시스템 변수를 설정하는 것이 권장된다.
  * 이는 서버가 비정상적으로 종료된 상황에도 장애 직전 시점까지 복구가 가능하도록 지원한다.
  * 반면, 트랜잭션이 커밋될 때마다 발생하는 디스크 IO 작업으로 부하가 심각한 경우 별도의 시스템 변수를 활용하여 리두 로그의 동기화 주기를 조절할 수 있다.

### 서버가 비정상 종료된 경우 데이터 파일의 상태
* MySQL 서버가 비정상 종료된 경우, InnoDB 스토리지 엔진의 데이터 파일은 다음과 같이 일관성이 깨진 데이터를 가질 수도 있다.
  1. 커밋되었으나, 데이터 파일에는 기록되지 않은 데이터
  2. 롤백되었으나, 데이터 파일에 이미 기록된 데이터
* **첫 번째 상황의 경우, 리두 로그에 저장된 데이터를 데이터 파일에 다시 복사하기만 하면 일관성을 복구**할 수 있다.
* 그러나 **두 번째 상황의 경우, 리두 로그만으로는 해결이 불가능**하다.
  * 이 경우, 변경되기 전 데이터를 갖는 언두 로그의 내용을 토대로 데이터 파일에 복사하여 일관성을 유지한다.
  * 반면 **해당 상황에서의 리두 로그는 변경 사항이 커밋되거나 롤백됐는지, 또는 트랜잭션 도중이었는지 확인하기 위해 사용**된다.

### 로그 버퍼
* **InnoDB 스토리지 엔진의 리두 로그 파일들의 전체 크기는 엔진이 갖는 버퍼 풀의 효율성을 결정하므로, 신중히 결정**되어야 한다.
  * 즉, 리두 로그 파일의 전체 크기가 InnoDB 엔진의 버퍼 풀 크기에 맞도록 적절히 선택되어야 한다.
  * **이를 통해 스토리지 엔진은 변경된 내용을 버퍼 풀에 우선 모아두었다가, 적시에 한 번에 디스크에 기록**할 수 있다.
* 반면, **변경 작업이 매우 많은 DBMS 서버의 경우 리두 로그의 기록 작업 자체가 문제가 될 가능성**이 높다. 
  * 이러한 **성능 저하를 보완하기 위해 최대한 ACID 수준을 보장하는 수준에서 추가로 버퍼링할 수 있으며, 이 때 사용되는 공간이 로그 버퍼에 해당**한다.
* **로그 버퍼는 리두 로그 버퍼링에 사용되는 공간으로, 기본값인 16MB 수준으로 설정을 유지하더라도 충분**하다.
  * 반면, BLOB 또는 TEXT 등 큰 데이터가 자주 변경되는 경우에는 더 큰 값을 설정하는 것이 바람직하다.

### 리두 로그 활성화 / 비활성화하기
* InnoDB의 **리두 로그는 여러 외부 요인으로 인해 MySQL 서버가 비정상 종료된 경우 데이터 파일에 기록되지 못한 트랜잭션을 복구하기 위해 사용**된다.
  * 이렇듯 리두 로그는 매우 중요한 역할을 수행하므로, 기본적으로 항상 활성화되어 있다.
  * **MySQL 서버에서 트랜잭션이 커밋되더라도 데이터 파일은 즉시 디스크로 동기화되지 않으나, 트랜잭션 로그인 리두 로그는 항상 디스크로 기록**된다.
* MySQL 8.0 버전 이전의 경우 수동으로 리두 로그 기능을 비활성화할 수 없었으나, 8.0 버전에 이르러서는 수동으로 리두 로그를 비활성화할 수 있게 되었다.
  * 이를 이용하여 **데이터를 복구하거나 대용량 데이터를 한 번에 적재하는 경우, 리두 로그를 비활성화하여 적재 시간을 단축**시킬 수 있다.
* **리두 로그를 비활성화하여 데이터 적재 작업을 수행한 경우, 적재가 완료된 후에는 반드시 리두 로그를 활성화하는 것이 바람직**하다.
  * 예를 들어, 리두 로그가 비활성화된 경우 MySQL 서버가 비정상 종료되면 마지막 체크포인트 이후의 데이터는 모두 복구할 수 없다.
  * 때문에 데이터가 중요하지 않더라도 서비스 도중에는 리두 로그를 활성화하여 비정상 종료에도 특정한 시점의 일관된 데이터로 복구할 수 있도록 해야 한다.
* **MySQL 서버가 비정상적으로 종료되었을 때 일부 데이터가 손실되도 무방한 경우에도 리두 로그를 비활성화하는 것은 권장되지 않는다**.
  * **대신 `innodb_flush_log_at_trx_commit` 시스템 변수를 1이 아닌 0 또는 2로 설정하여 사용하는 것이 권장**된다.

### MySQL 서버의 재시작
* **MySQL 서버는 재시작되는 경우에는 언제나 리두 로그를 통해 데이터 파일에 기록되지 못한 데이터가 존재하는지 검사**한다.
  * 그러나 **리두 로그가 비활성화된 경우, MySQL 서버가 재시작되더라도 리두 로그를 활용한 복구가 불가능하므로 MySQL 서버가 시작되지 못할 수도 있다**.
* 때문에 리두 로그를 비활성화하는 경우 의도치 않게 데이터가 손실될 수 있음에 항상 주의해야 한다.

## 2022-09-24 Sat
### 어댑티브 해시 인덱스
* **어댑티브 해시 인덱스는 사용자가 직접 생성하는 인덱스가 아니며, InnoDB 스토리지 엔진 차원에서 사용자가 자주 요청하는 데이터에 대해 자동으로 생성**한다.
* B-Tree 인덱스의 탐색은 데이터베이스 서버의 부하에 따라 매우 빠를 수도, 느릴 수도 있다.
  * **어댑티브 해시 인덱스는 이러한 B-Tree 인덱스의 탐색 시간을 줄이기 위해 도입된 기능**이다.
  * **InnoDB는 자주 조회되는 데이터 페이지의 키 값을 활용하여 해시 인덱스를 만들고, 필요시 이를 튱해 레코드가 저장된 데이터 페이지를 조회**한다.
  * 이로 인해 **B-Tree 인덱스의 루트 노드부터 리프 노드까지 검색하는 비용이 없어지며, CPU의 부하가 줄어들어 쿼리의 성능이 높아진다**.
* 해시 인덱스는 인덱스의 키와 해당 인덱스 키가 저장된 데이터 페이지의 주소 쌍으로 관리된다.
  * 이 때, **인덱스 키 값은 B-Tree 인덱스의 고유 번호와 B-Tree 인덱스의 실제 키 값의 조합으로 생성**된다.
* **InnoDB 스토리지 엔진에서 어댑티브 해시 인덱스는 단 하나만 존재하므로, 인덱스의 키 값에는 반드시 B-Tree 인덱스의 고유 번호가 포함**되어야 한다.
  * 이렇듯 모든 B-Tree 인덱스에 대한 어댑티브 해시 인덱스는 하나의 해시 인덱스에 저장되며, 특정한 키 값이 어떤 B-Tree 인덱스의 것인지 구분할 수도 있다.

### 버퍼 풀과 어댑티브 해시 인덱스의 관계
* 해시 인덱스에 저장되는 **데이터 페이지 주소는 실제 키 값이 저장되는 데이터 페이지의 주소이며, 이는 곧 InnoDB 버퍼 풀에 로딩된 페이지의 주소**이다.
  * 때문에 **어댑티브 해시 인덱스는 버퍼 풀에 올려진 데이터 페이지에 대해서만 관리**된다.
  * 또한, **버퍼 풀에서 해당 데이터 페이지가 없어지면 어댑티브 해시 인덱스에서도 해당 페이지의 정보가 사라진다**.

### 어댑티브 해시 인덱스의 이점
* **어댑티브 해시 인덱스를 활성화한 경우, 쿼리의 처리량은 크게 늘어나는 반면 CPU 사용률은 그와 반비례하여 감소**한다.
* 또한, **B-Tree 인덱스의 루트 노드로부터 검색이 줄어들게 되므로 InnoDB의 내부 잠금인 세마포어 횟수 역시 크게 감소**한다.

### 어댑티브 해시 인덱스 파티셔닝
* MySQL 8.0 버전 이전의 경우, 어댑티브 해시 인덱스는 단일 메모리 객체이므로 경합이 심한 축에 속했다.
  * 이로 인해 **MySQL 8.0 버전부터는 내부 잠금(세마포어) 경합을 줄이기 위해 어댑티브 해시 인덱스의 파티션 기능을 제공**한다.
  * 시스템 변수를 활용하여 파티션 수를 변경할 수 있으며, 해시 인덱스로 인한 성능 향상을 체감한 경우 파티션 개수를 더 많이 설정하는 것도 고려할 수 있다.

### 어댑티브 해시 인덱스가 도움이 되는 경우와 그렇지 않은 경우
* **어댑티브 해시 인덱스는 다음과 같은 경우에는 성능 향상에 크게 도움을 주지 못하므로 비활성화하는 것을 고려**할 수 있다.
  1. 디스크의 읽기가 많은 경우
  2. JOIN 또는 LIKE 검색과 같이 특정한 패턴의 쿼리가 많은 경우
  3. 매우 큰 데이터를 갖는 테이블의 레코드를 폭 넓게 읽어들이는 경우
* 반면, 다음과 같은 경우에서는 어댑티브 해시 인덱스를 활성화하는 것으로 큰 성능 향상을 기대할 수 있다.
  1. 디스크 읽기가 많지 않은 경우: 즉, 디스크의 데이터가 InnoDB 버퍼 풀의 크기와 비슷한 경우
  2. 동등 비교 또는 IN 연산자 등의 동등 조건 검색이 빈번한 경우
  3. 쿼리가 데이터 중 일부 데이터에만 집중되는 경우
* 그러나 어댑티브 해시 인덱스가 도움이 되는지 판단하는 것은 단순한 작업이 아니며, 다음과 같은 확실한 사실을 기반으로 의사 결정을 내리는 것이 중요하다.
  1. **어댑티브 해시 인덱스는 데이터 페이지를 메모리에 위치한 버퍼 풀로부터 접근하는 과정을 더 빠르게 만드는 기능**이다.
  2. 즉, 데이터 페이지를 인덱스가 아닌 디스크로부터 읽어들이는 경우가 빈번한 데이터베이스 서버에서는 아무런 도움이 되지 않는다.
  3. **어댑티브 해시 인덱스 역시 메모리 공간을 사용하며, 때에 따라 상당히 큰 공간을 점유할 수도** 있다.
  4. 어댑티브 해시 인덱스 역시 불필요한 경우 제거되는 등 관리될 필요가 있으며, 활성화된 경우 InnoDB는 키 값의 존재 유무와 관계 없이 이를 검색해야 한다.
     * 즉, **해시 인덱스에 키 값이 존재하지 않는 경우가 많아 효율이 떨어지는 상황에서도 InnoDB 엔진은 반드시 해시 인덱스를 사용하려 들 것**이다.

### 어댑티브 해시 인덱스와 테이블의 삭제
```
> 어댑티브 해시 인덱스는 테이블의 삭제 작업에도 큰 영향을 준다.
> 어댑티브 해시 인덱스의 도움을 크게 받을수록 테이블 삭제 또는 변경 작업은 더더욱 치명적일 수 있다.
```
* 예를 들어 임의의 테이블 인덱스가 어댑티브 해시 인덱스에 적재된 경우를 가정한다.
  * 이 경우, **테이블을 삭제하거나 변경하기 위해 InnoDB 엔진은 테이블이 갖는 모든 페이지의 내용을 어댑티브 해시 인덱스에서 제거**해야 한다.
  * **이로 인해 테이블이 삭제되거나 변경되는 경우 상당히 많은 CPU 자원을 사용하며, 당연히 데이터베이스 서버의 처리 성능 역시 저하**된다.

### 어댑티브 해시 인덱스의 효율성 판단하기
* **어댑티브 해시 인덱스는 경우에 따라 서비스에 도움을 줄 수도, 불필요한 오버헤드만 만들 수도 있으므로 이를 잘 판단할 수 있어야 한다**.
  * 예를 들어, `SHOW ENGINE INNODB STATUS` 명령어를 활용하여 `hash searched/s`와 `non-hash searches/s` 값을 확인할 수 있다.
* 어댑티브 해시 인덱스의 효율을 판단하는 경우, 반드시 다음과 같은 내용을 종합하여 판단하는 것이 바람직하다.
  1. 해시 인덱스 히트율
  2. 어댑티브 해시 인덱스가 사용하는 메모리 공간의 크기: `performance_schema` 데이터베이스의 테이블을 적절히 사용하요 확인할 수 있다.
  3. MySQL 서버의 CPU 사용량
* 예를 들어, 30% 정도의 히트율을 보이는 어댑티브 해시 인덱스는 MySQL 서버의 CPU 사용량에 따라 다른 의미를 가질 수 있다.
  1. CPU 사용량이 100%에 근접하는 경우, 30% 정도의 히트율을 보이는 어댑티브 해시 인덱스는 효율적이라고 볼 수 있다.
  2. CPU 사용량이 높지 않은 경우, 차라리 어댑티브 해시 인덱스를 비활성화하여 InnoDB 버퍼 풀이 더 많은 메모리를 사용하도록 유도하는 것이 좋을 수 있다.