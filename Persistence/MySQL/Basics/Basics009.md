...
* **MySQL 서버의 옵티마이저가 실행 계획을 수립하는 경우에는 통계 정보와 옵티마이저 옵션을 결합하여 최적의 실행 계획을 수립**한다.
  * 이 때, **옵티마이저 옵션은 크게 조인과 관련된 옵티마이저 옵션과 옵티마이저 스위치로 구분**된다.
...
* 이 때, 드라이빙 테이블에서 드리븐 테이블의 조인 조건이 인덱스를 사용할 수 없다면 반복적으로 풀 테이블을 스캔해야 할 수도 있다.
...
* **어떠한 방식으로도 드리븐 테이블의 풀 테이블 스캔 또는 인덱스 풀 스캔을 피할 수 없는 경우, 옵티마이저는 조인 버퍼를 활용하도록 동작**한다.
  * 이 때, **옵티마이저는 드라이빙 테이블에서 읽어들인 레코드를 우선 메모리의 조인 버퍼에 캐시한 후에 드리븐 테이블과 해당 메모리 캐시를 조인**한다.
...
## 2022-10-04 Tue
### 쿼리 힌트
* MySQL 버전이 업그레이드됨에 따라 통계 정보와 옵티마이저의 최적화 방법은 더 다양해지고, 이에 따라 쿼리의 실행 계획 최적화 역시 성숙해지고 있다.
* 그러나 **여전히 MySQL 서버는 비즈니스를 100% 이해하지는 못하므로, 개발자 또는 DBA보다 부족한 실행 계획을 수립하는 경우가 발생**할 수 있다.
  * 이러한 경우에는 옵티마이저에게 쿼리 실행 계획을 어떻게 수립해야할지 알려주기 위한 방법이 필요하다.
* **일반적인 RDBMS는 옵티마이저에게 실행 계획 수립 방향성을 알려주기 위한 힌트 기능을 제공하며, MySQL 역시 다양한 옵티마이저 힌트를 제공**한다.
  * 이 때, MySQL 서버에서 사용 가능한 쿼리 힌트는 크게 인덱스 힌트와 옵티마이저 힌트로 분류된다.
  * **인덱스 힌트는 예전 버전의 MySQL 서버에서 사용되어온 힌트인 반면, 옵티마이저 힌트는 MySQL 5.6 버전부터 새롭게 추가되기 시작한 힌트들을**지칭한다.

### 인덱스 힌트
```
> 인덱스 힌트는 SELECT와 UPDATE 쿼리에서만 사용이 가능하다.
```
* 여러 **인덱스 힌트들은 MySQL 서버에 옵티마이저 힌트가 도입되기 전부터 사용되어왔으며, 이로 인해 ANSI SQL 표준을 준수하지 못하는 단점이 존재**한다.
  * 반면, MySQL 5.6 버전부터 추가되기 시작한 옵티마이저 힌트들은 다른 RDBMS에서는 주석으로 해석하므로 표준을 준수할 수 있다.
  * 때문에 **가능하다면 인덱스 힌트보다는 ANSI SQL 표준을 준수하는 옵티마이저 힌트를 사용하는 것이 바람직**하다.

### STRAIGHT_JOIN 힌트
```
> STRAIGHT_JOIN 키워드는 옵티마이저 힌트이면서도 조인 키워드이기도 하다.
```
* **STRAIGHT_JOIN은 SELECT나 DELETE, UPDATE 쿼리에서 여러 테이블이 조인되는 경우에 조인 순서를 고정하는 역할을 수행**한다.
  * 일반적으로 여러 테이블을 조인하는 쿼리는 어느 테이블이 드라이빙 테이블 또는 드리븐 테이블이 될지 미리 알 수 없다.
  * 대신 **옵티마이저는 그 때마다 각 테이블의 통계 정보와 쿼리 조건을 기반으로 가장 최적이라고 판단되는 순서에 따라 조인하도록 실행 계획을 수립**한다.
* 일반적으로 **조인 순서는 다음과 같은 조건에 따라 드라이빙 테이블과 함께 결정**된다.
  1. 조인을 하기 위한 컬럼들의 인덱스 여부
     * **조인 대상 테이블 중 한 쪽에만 조인 대상 컬럼에 인덱스가 존재하는 경우, 인덱스가 존재하지 않는 테이블을 드라이빙 테이블로 선택**한다. 
  2. 인덱스에 문제가 없으며 WHERE 절이 존재하는 경우, WHERE 조건을 만족하는 레코드가 적은 테이블
  3. 인덱스에 문제가 없으며 WHERE 절이 존재하지 않는 경우, 레코드 수가 적은 테이블
* 반면, 쿼리의 조인 순서를 강제로 변경하고자 하는 경우에는 STRAIGHT_JOIN 힌트를 사용할 수 있다.
  * 이 때, 인덱스 힌트는 명시해야하는 위치가 정해져 있음에 유의해야 한다.
* STRAIGHT_JOIN 힌트를 사용할 경우, 옵티마이저는 FROM 절에 명시된 테이블의 순서대로 조인을 수행하도록 유도한다.
* **일반적으로 다음과 같은 기준에 맞추어 조인 순서가 결정되지 않는 경우에만 STRAIGHT_JOIN 힌트로 조인 순서를 조정하는 것이 바람직**하다.
  1. 임시 테이블과 일반 테이블의 조인: **일반적으로 임시 테이블이 드라이빙 테이블로 선정되는 것이 바람직**하다.
     * 대부분의 경우 옵티마이저가 적절한 조인 순서를 설정하므로, 항상 STRAIGHT_JOIN 힌트를 명시할 필요는 없다.
  2. 임시 테이블 간의 조인: **임시 테이블 또는 서브쿼리로 파생된 테이블은 항상 인덱스가 없으므로 어떤 것을 드라이빙 테이블로 선정하여도 무방**하다.
     * 때문에 **크기가 더 작은 테이블을 드라이빙 테이블로 선정하는 것이 바람직**하다.
  3. 일반 테이블 간의 조인: 양 테이블 모두 조인 컬럼에 인덱스가 존재하거나, 존재하지 않는 경우 모두 레코드 수가 적은 테이블을 드라이빙 테이블로 선정한다.
     * **그 이외의 경우, 조인 컬럼에 인덱스가 없는 테이블을 드라이빙 테이블로 선정하는 것이 바람직**하다.
* 이 때, 레코드 건수는 인덱스를 사용할 수 있는 WHERE 절을 포함하여 그 조건을 만족하는 레코드 개수를 의미한다.
  * 즉, **무조건 테이블 전체의 레코드 개수를 의미하는 것이 아니다**.
* STRAIGHT_JOIN 힌트와 동일한 역할을 수행하는 옵티마이저 힌트로 JOIN_FIXED_ORDER 힌트가 존재한다.
  * 이러한 옵티마이저 힌트는 FROM 절에 명시된 모든 테이블의 조인 순서를 결정한다.
  * 반면, JOIN_ORDER와 JOIN_PREFIX 및 JOIN_SUFFIX는 일부 테이블의 조인 순서를 제안하기 위해 사용하는 옵티마이저 힌트에 해당한다.

### 인덱스 힌트
```
> 조인의 순서를 변경하는 것 다음으로 자주 사용되는 것이 인덱스 힌트이다.
```
* 이 때, **인덱스 힌트는 STRAIGHT_JOIN과는 달리 사용하려는 인덱스를 갖는 테이블 뒤에 힌트를 명시**한다.
* 일반적으로 MySQL 서버의 옵티마이저는 사용할 인덱스를 잘 선택하는 편이다.
  * 그러나 **3 ~ 4개 이상의 컬럼을 포함하는 유사한 형태의 인덱스가 여럿 존재하는 경우, 옵티마이저가 잘못 된 인덱스를 선택할 가능성이 존재**한다.
  * 이러한 경우에는 강제로 임의의 인덱스를 사용하도록 힌트를 추가하는 것이 바람직하다.
* 인덱스 힌트는 크게 다음과 같이 분류되며, 모두 키워드 뒤에 사용할 인덱스의 이름을 괄호로 묶어 사용한다.
  1. USE INDEX: **가장 자주 사용되는 인덱스 힌트이며, 옵티마이저에게 특정 테이블의 인덱스를 사용하도록 권장하는 힌트로 사용**된다.
     * 어디까지나 권장하는 것으로, 대부분의 경우에 옵티마이저는 권장 사항에 따르지만 반드시 해당 인덱스를 사용하는 것은 아니다.
  2. FORCE INDEX: USE INDEX와 유사하지만, 옵티마이저에게 더 강한 영향을 주는 힌트에 해당한다.
     * 그러나 **USE INDEX 만으로도 옵티마이저에 대해 큰 영향을 줄 수 있으므로, 해당 인덱스 힌트는 거의 사용할 필요가 없다**.
     * **일반적으로 USE INDEX를 사용했을 때 권장 사항을 따르지 않는 경우, FORCE INDEX를 사용하더라도 해당 인덱스를 사용하지 않는 경우가 많다**.
  3. IGNORE INDEX: **상술한 인덱스 힌트와는 반대로 임의의 인덱스를 사용하지 못하도록 하며, 주로 풀 테이블 스캔을 유도하고자 하는 경우에 사용**된다.
* **PK의 경우 PRIMARY를 명시하여 사용할 수 있으며, 존재하지 않는 인덱스를 사용하거나 빈 괄호를 적용한 경우애는 문법 오류로 처리**된다.
* 또한, **옵티마이저는 일반적인 B-Tree 인덱스보다 PK 또는 전문 검색 인덱스에 가중치를 두고 실행 계획을 수립**한다.

### 인덱스 힌트의 용도 명시하기
* 상술한 **모든 인덱스 힌트에는 선택적으로 다음과 같은 용도를 명시할 수 있으며, 용도를 명시하지 않는 경우 인덱스를 세 용도로 사용**한다.
  1. USE INDEX FOR JOIN: 테이블 간의 조인 뿐만 아니라 레코드를 검색하기 위한 용도로 사용된다.
     * 일반적으로, **MySQL 서버에서는 하나의 테이블로부터 데이터를 검색하는 작업까지도 JOIN이라는 용어로 표현**한다.
  2. USE INDEX FOR ORDER BY: 명시된 인덱스를 ORDER BY 용도로만 사용하도록 제한한다.
  3. USE INDEX FOR GROUP BY: 명시된 인덱스를 GROUP BY 용도로만 사용하도록 제한한다.
* 그러나 **용도는 옵티마이저가 대부분의 경우에 최적으로 선택하므로 크게 신경쓰지 않아도 무방**하다.

### 인덱스 힌트 지양하기
* **인덱스의 사용법이나 좋은 실행 계획을 판단하기 어려운 경우, 힌트를 사용하여 옵티마이저에 영향을 주는 것 자체를 지양하는 것이 바람직**하다.
  * 애초에 현재의 MySQL 옵티마이저는 간단한 최적화는 눈 깜짝할 사이에 처리할 수 있다.
* **최적의 실행 계획은 데이터의 성격에 따라 변화해 나가므로, 지금 PK를 사용하는 것이 좋은 계획이었다고 해서 미래에도 항상 좋을 것을 보장할 수 없다**.
  * 때문에 **가능하다면 옵티마이저가 그 당시의 통계 정보를 통해 인덱스를 자율적으로 선택하도록 하는 것이 바람직**하다.
* **가장 좋은 최적화는 해당 쿼리를 서비스에서 제거하거나 튜닝할 필요가 없도록 데이터를 최소화하는 것**이다.
  * **이 자체가 어려운 상황에서는 우선 데이터 모델을 단순화하여 쿼리를 간결하게 만들거나, 힌트 자체가 필요하지 않은 상황을 유도할 수 있어야 한다**.
  * 그러나 어쩔 수 없는 상황도 있는 반면, 이상적인 방법은 일반적으로 많은 시간과 능력을 필요로 하기에 실무에서는 힌트에 의존하는 경우가 많다.

## 2022-10-05 Wed
### SQL_CALC_FOUND_ROWS 힌트
* **MySQL은 LIMIT 절을 명시하는 경우, 조건을 만족하는 레코드를 LIMIT에 명시된 만큼 검색하는 데에 성공했다면 즉시 검색을 중지**한다.
* 반면, **SQL_CALC_FOUND_ROWS 힌트가 포함된 쿼리의 경우에는 LIMIT 이상의 레코드를 찾았다고 하더라도 끝까지 검색을 진행**한다.
  * 최종적으로는 LIMIT에 명시된 만큼만 반환되며, 이후 FOUND_ROWS() 함수를 활용하여 LIMIT을 제외한 조건을 만족하는 레코드의 개수를 알 수 있다.
* **언뜻 해당 힌트는 페이징에 유용하게 사용될 수 있는 것처럼 보일 수 있으나, 실제로는 다음과 같은 이유에서 사용하지 않는 것이 바람직**하다.
  1. 쿼리를 2번 실행해야 하며, LIMIT 절에 명시된 수치와 관계 없이 WHERE 절의 조건을 만족하는 모든 레코드 개수만큼 디스크 IO가 발생한다.
  2. **애초에 해당 힌트는 성능 향상을 위한 것이 아닌, 개발자의 편의를 위해 만들어진 힌트에 해당**한다.
* 물론 COUNT(*) 등의 쿼리가 적절히 튜닝되지 않았거나 WHERE 조건을 처리할 수 있는 인덱스가 존재하지 않는 경우, 해당 힌트를 사용하는 것이 빠를 수도 있다.
  * **그렇다고 해서 SQL_CALC_FOUND_ROWS 힌트를 사용하는 것보다는 쿼리 또는 인덱스를 튜닝하는 것이 훨씬 바람직한 결과**를 낳는다.
* 예외적인 경우가 아니라면, **일반적으로 SQL_CALC_FOUND_ROWS 힌트를 사용하기보다는 레코드 카운터 쿼리와 데이터 조회 쿼리를 분리하는 것이 효율적**이다.

### 옵티마이저 힌트의 종류
```
> MySQL 8.0 버전에서 사용 가능한 힌트의 종류는 매우 많고, 각각의 옵티마이저 힌트가 미치는 영향 범위도 다양하다.
```
* 옵티마이저 힌트는 각각 영향을 미치는 범위에 따라 다음과 같이 분류할 수 있다.
  1. 인덱스: 특정한 인덱스의 이름을 사용할 수 있는 옵티마이저 힌트이다.
  2. 테이블: 특정한 테이블의 이름을 사용할 수 있는 옵티마이저 힌트이다.
  3. 쿼리 블록: 특정한 쿼리 블록에 대해 사용할 수 있는 옵티마이저 힌트이다.
     * 이 때, 특정 쿼리 블록의 이름을 명시하는 것이 아닌 힌트가 명시된 쿼리 블록에 대해서만 영향을 미치는 종류의 옵티마이저 힌트에 해당한다.
  4. 글로벌(또는 쿼리 전체): 전체 쿼리에 대해 영향을 미치는 힌트이다.
* 그러나 이러한 구분으로 인해 힌트를 명시할 위치가 달라지는 것은 아니다.
* 또한 힌트에 인덱스 또는 테이블, 아니면 둘 다 명시될 수 있는 경우가 각각 존재하며, 이를 다시 다음과 같이 분류할 수 있다.
  1. 힌트에 인덱스 이름이 명시될 수 있는 경우: 인덱스 수준의 힌트이다.
  2. 힌트에 테이블 이름이 명시될 수 있는 경우: 테이블 수준의 힌트이다.
  3. 힌트에 인덱스와 테이블 이름이 모두 명시될 수 있는 경우: **인덱스와 테이블 수준의 힌트**이다.

### 옵티마이저 힌트의 주의 사항
* **모든 인덱스 수준의 힌트는 반드시 `/*+ INDEX(employees, idx_firstname) */ *` 형태로 테이블 명을 선행하여 명시**해야 한다.
  * 이 때, 명시되는 테이블은 반드시 후행 인자로 전달된 인덱스를 가져야만 한다.
* 옵티마이저 힌트가 문법에 맞지 않게 사용된 경우, 실행 계획 수립 시 2개의 경고 메시지가 출력된다.
  * 일반적으로 실행 계획은 항상 1개의 경고 메시지를 출력하므로, 경고 메시지의 수로 옵티마이저 힌트의 문법 오류를 알 수 있다.
  * 즉, **익숙하지 않은 옵티마이저 힌트를 사용하려는 경우 반드시 실행 계획을 우선 수립하여 문법상 오류를 체크하는 것이 바람직**하다.

### 쿼리 블록
* **단일 SQL 문장에서 SELECT 절은 여러 번 사용될 수 있으며, 각각의 SELECT 키워드로 시작하는 서브쿼리 영역이 쿼리 블록**이 된다.
* 이 때, 쿼리 블록에 영향을 주는 옵티마이저 힌트는 해당 쿼리 블록 내부 또는 외부로부터 사용될 수 있다.
  * 이렇듯 특정한 쿼리 블록을 외부 쿼리 블록에서 사용하고자 하는 경우, `QB_NAME()` 힌트를 사용하여 해당 쿼리 블록에 이름을 부여할 필요가 있다.

### MAX_EXECUTION_TIME
* 옵티마이저 힌트 중 유일하게 쿼리의 실행 계획에 영향을 주지 않는 힌트이며, 단순히 쿼리의 최대 실행 시간을 설정한다.
  * 밀리초 단위의 시간을 설정하는 것으로 쿼리가 지정된 시간을 초과하는 경우에 쿼리가 실패하도록 만들 수 있다.

### SET_VAR
* 옵티마이저 힌트 뿐만 아니라 MySQL 서버의 시스템 변수 역시 쿼리의 실행 계획 수립에 큰 영향을 줄 수 있다.
  * 대표적으로 조인 버퍼의 크기를 설정하는 `join_buffer_size` 시스템 변수가 있다.
  * 또한, 옵티마이저 힌트로는 부족한 경우 `optimizer_switch` 시스템 변수를 제어해야할 수도 있다.
* SET_VAR 힌트는 실행 계획을 바꿀 수도 있으며, 나아가 조인 버퍼나 소트 버퍼의 크기를 일시적으로 증가시켜 대용량 처리 쿼리의 성능을 향상시킬 수도 있다.
  * 그러나 **모든 시스템 변수를 SET_VAR 힌트로 관리할 수 있는 것은 아니다**.

### SEMIJOIN과 NO_SEMIJOIN
* 세미 조인 최적화는 여러 세부 전략이 존재하며, SEMIJOIN 힌트를 통해 사용할 세부 전략을 선택할 수 있다.
  * 반면, Table pull-out 전략은 일반적으로 다른 전략에 비해 항상 더 나은 성능을 보장하므로 별도의 힌트가 존재하지 않는다.
* 이 때, **세미 조인 최적화 힌트는 외부 쿼리가 아닌 서브쿼리에 명시**해야 한다.
  * 또는 서브쿼리에 쿼리 블록 이름을 정의한 후, 실제 세미 조인 힌트를 외부 쿼리 블록에 명시할 수도 있다.
* 거꾸로 특정한 세미 조인 최적화 전략을 사용하지 않고자 하는 경우에는 NO_SEMIJOIN 힌트를 명시하여 가이드할 수 있다.

### SUBQUERY
* 서브쿼리 최적화는 세미 조인 최적화를 사용하지 못하는 경우에 적용할 수 있는 최적화 방법이다.
  * 세미 조인 최적화는 주로 IN(서브쿼리) 형태의 쿼리에서 사용되는 반면, 안티 세미 조인의 최적화에는 사용이 불가능하다.
  * 때문에 주로 안티 세미 조인 최적화에는 IN-to-EXISTS와 Materialization의 2가지 최적화 기법이 사용된다.
* 서브쿼리 최적화는 사용할 만한 상황이 자주 발생하지 않는, 필요한 상황이 드문 최적화 기법에 해당한다.

### BNL과 NO_BNL 및 HASHJOIN과 NO_HASHJOIN
* MySQL 8.0.19 버전까지는 블록 네스티드 루프 조인 알고리즘을 사용했으나, 8.0.20버전부터는 해시 조인 알고리즘으로 대체되었다.
  * 때문에 8.0.20 버전 이후로는 블록 네스티드 루프 조인이 더 이상 MySQL 서버 상에서 사용되지 않는다.
* **8.0.20 버전 이후의 경우, BNL 힌트는 해시 조인을 유도하는 힌트로 사용**된다.
  * 때문에 8.0.20 버전 이후로는 해시 조인을 유도하거나 유도하지 않고자 하는 경우에 BNL과 NO_BNL 힌트를 사용해야 한다.
  * 반면, HASHJOIN과 NO_HASHJOIN은 8.0.18 버전까지만 유효하여 그 이후로는 효력이 없다.

### 해시 조인 유도하기
* **MySQL 서버에서, 실제로는 조인 조건이 되는 컬럼에 인덱스가 적절히 준비되어 있는 경우에 해시 조인이 거의 사용되지 않는다**.
  * 힌트를 사용하더라도 실제 실행 계획에서는 네스티드 루프 조인이 사용되는 경우가 많다.
  * **해시 조인을 사용하도록 유도하려면 조인 조건이 되는 컬럼의 인덱스를 드라이빙 및 드리븐 테이블로부터 제거하거나 사용할 수 없도록 해야 한다**.

## 2022-10-06 Thu
### 조인 순서 결정 힌트
* STRAIGHT_JOIN 힌트는 전통적으로 조인의 순서를 결정하기 위해 사용되어 왔으나, 다음과 같은 문제가 존재한다.
  1. 쿼리의 FROM 절에 명시된 테이블의 순서를 반드시 원하는 조인 순서에 맞추어야 한다.
  2. **한 번 STRAIGHT_JOIN 힌트를 적용한 경우, FROM 절에 명시된 모든 테이블의 순서로 조인되므로 일부의 순서만을 결정할 수는 없다**.
* 이러한 단점을 보완하기 위해 옵티마이저 힌트는 다음과 같은 힌트를 추가로 제공한다.
  1. JOIN_FIXED_ORDER: STRAIGHT_JOIN과 동일한 기능을 갖는다.
  2. JOIN_ORDER: FROM 절에서 명시된 테이블의 순서가 아닌, 힌트에 명시된 테이블의 순서대로 조인한다.
  3. JOIN_PREFIX: 조인 드라이빙 테이블만을 강제한다.
  4. JOIN_SUFFIX: 조인 드리븐 테이블만을 강제한다.

### MERGE와 NO_MERGE
* **MySQL의 이전 버전에서는 FROM 절에 사용되는 서브쿼리를 항상 내부 임시 테이블인 파생 테이블로 생성**하였다.
  * 이러한 임시 테이블은 불필요한 자원 소모를 발생시키므로, 5.7 버전 이후로는 가능한 한 임시 테이블을 사용하지 않는다.
  * 예를 들어, **옵티마이저는 가능한 경우에 FROM 절에 명시된 서브쿼리를 외부 쿼리와 병합하여 최적화**한다. 
* 그러나 이러한 쿼리 병합이 항상 좋은 것은 아니며, 때로는 내부 임시 테이블을 생성하는 것이 더 바람직할 수 있다.
  * **옵티마이저가 항상 올바른 판단을 내리는 것은 아니므로, 이러한 경우에 해당 옵티마이저 힌트를 통해 외부 쿼리와의 병합을 유도하거나 제한**할 수 있다.

### INDEX_MERGE와 NO_INDEX_MERGE
* **MySQL 서버는 가능한 한 테이블 당 하나의 인덱스만을 사용하여 쿼리를 처리하려고 하는 경향**이 있다.
  * 그러나 하나의 인덱스만으로는 검색 대상 범위를 충분히 좁힐 수 없을 수 있으며, 이 경우 옵티마이저는 다른 인덱스를 함께 사용한다.
  * 예를 들어, 여러 인덱스로부터 검색된 레코드의 교집합 또는 합집합을 반환하여 활용한다.
* 이렇듯 단일 테이블에 대해 여러 인덱스를 동시에 사용하는 것을 인덱스 병합이라고 한다.
* 그러나 인덱스 병합이 항상 성능 향상에 도움을 주는 것은 아니므로, 해당 옵티마이저 힌트를 통해 인덱스 병합 실행 계획의 사용 여부를 제어할 수 있다.

### NO_ICP
* **기본적으로 인덱스 컨디션 푸시다운 최적화는 항상 도움이 되므로, 옵티마이저는 되도록 해당 기능을 사용하고자 하는 경향**이 있다.
  * 때문에 옵티마이저는 ICP 힌트를 제공하지 않는다.
* 그러나 인덱스 컨디션 푸시다운으로 인해 여러 실행 계획의 비용 계산이 잘못 처리되는 경우, 결과적으로 잘못 된 실행 계획을 수립할 가능성 역시 존재한다.
  * 이 경우, **임의의 인덱스를 선호하거나 사용하지 않도록 유도하는 것보다는 인덱스 컨디션 푸시다운 최적화만 비활성화하는 것이 바람직**하다.

### SKIP_SCAN과 NO_SKIP_SCAN
* **인덱스 스킵 스캔은 WHERE 절에 인덱스의 선행 컬럼과 관련된 조건이 없더라도 옵티마이저가 해당 인덱스를 활용할 수 있도록 하는 훌륭한 최적화 기능**이다.
* 그러나 **조건이 누락되었던 선행 컬럼이 갖는 유니크한 값의 개수가 많은 경우, 인덱스 스킵 스캔으로 인해 성능은 오히려 저하**될 수 있다.
  * 때문에 옵티마이저가 유니크한 값의 개수를 적절히 분석하지 못하였거나 잘못된 경로로 인해 비효율적인 인덱스 스킵 스캔을 적용할 수도 있다.
  * 이 경우, NO_SKIP_SCAN 옵티마이저 힌트를 통해 비효율적인 인덱스 스킵 스캔을 제한할 수 있다.

### INDEX와 NO_INDEX
* 해당 옵티마이저 힌트는 예전 버전의 MySQL 서버에서 사용되던 인덱스 힌트를 대체하는 용도로 제공하며, 각각 다음의 인덱스 힌트를 대체한다.
  1. USE INDEX: INDEX
  2. USE INDEX FOR GROUP BY: GROUP_INDEX
  3. USE INDEX FOR ORDER BY: ORDER_INDEX
  4. IGNORE INDEX: NO_INDEX
  5. IGNORE INDEX FOR GROUP BY: NO_GROUP_INDEX
  6. IGNORE INDEX FOR ORDER BY: NO_ORDER_INDEX
* 인덱스 힌트는 임의의 테이블 뒤에 명시하였으므로 별도로 테이블명을 명시하지 않고 인덱스명만 나열해도 무방하다.
  * 반면, 옵티마이저 힌트는 반드시 `INDEX(테이블명 인덱스명)` 형식으로 테이블명과 인덱스명을 함께 명시해야 한다.