...
### 컴포넌트 기반의 서비스
* 서비스에도 클린 아키텍쳐의 이점을 적용할 수 있으며, 예를 들어 SOLID 원칙을 적용하거나 컴포넌트 구조를 갖추도록 할 수 있다.
...
### 서비스와 아키텍쳐 경계 - 결론
```
> 서비스는 시스템의 확장성과 개발 가능성 측면에서 유용하지만, 그 자체적으로 아키텍쳐가 되지는 않으며 경계 역시 서비스 사이에 위치하지 않는다.
> 시스템 아키텍쳐는 오히려 시스템 내부에 그어진 경계와, 이를 넘나드는 의존성에 의해 정의된다. 
```
* 서비스 내부는 DIP를 준수하는 컴포넌트 아키텍쳐로 설계되는 반면, 경계는 서비스 내부의 컴포넌트들에 의해 정의되므로 서비스는 하나 이상의 컴포넌트로 구성된다.
  * 즉, **시스템의 구성 요소가 통신하고 실행되는 물리적인 방식에 의해 아키텍쳐가 정의되는 것은 아니다**.
