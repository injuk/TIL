...
```
* 엔티티는 가장 고수준의 정책으로서 최심부에 위치하며, 그 바깥에는 유즈케이스와 인터페이스 어댑터 및 프레임워크가 위치한다.
  * 이 때, **인터페이스 어댑터에는 컨트롤러 등 표현 계층이 포함되는 반면 프레임워크에는 UI나 웹 DB 등 외부 인터페이스가 포함**된다.
...
* **엔티티는 전사적인 핵심 업무 규칙을 캡슐화하므로, 일련의 메소드와 데이터 구조를 갖는 객체**일 수 있다.
  * 엔티티 계층은 클린 아키텍쳐의 최심부에 위치하므로, 운영 측면에서의 시스템 변경을 비롯한 모든 외부 변경 사항으로부터 영향받지 않아야 한다.
...
* 인터페이스 어댑터 계층은 일련의 어댑터로 구성되며, 해당 계층에 속한 어떠한 코드도 데이터베이스 등 외부의 세부사항에 대해 알 필요가 없어야 한다.
  * 이 때, **어댑터는 유즈케이스 또는 엔티티와 데이터베이스 등 외부의 세부사항 사이에 위치하여 임의의 데이터를 서로가 사용하는 방식으로 변환**한다.
* 또한 프리젠터와 뷰 및 컨트롤러는 모두 해당 계층에 속하며, 모델은 데이터 구조에 불과한 등 해당 계층은 GUI 상의 MVC 아키텍쳐를 모두 포괄한다.

### 네 번째 동심원 - 프레임워크와 드라이버 계층
* **해당 동심원은 가장 바깥에 위치하며, 데이터베이스 또는 웹 프레임워크와 같은 모든 선택 가능한 세부사항들로 구성**된다.
...
* 유즈케이스와 데이터베이스의 사이에는 다형적 인터페이스인 데이터베이스 게이트웨이가 존재하며, 이는 시스템이 데이터베이스에 수행하는 모든 메소드를 포함한다.
  * 이 때, 게이트웨이 인터페이스의 구현체는 데이터베이스 계층에 위치하며 이 역시 전형적인 험블 객체에 해당한다.
  * 반면, **유즈케이스 역시 시스템에 특화된 업무 규칙을 캡슐화하는 전형적인 험블 객체에 해당**한다.
  * 나아가 ORM, 즉 **데이터 매퍼 역시 데이터베이스 계층에 위치하며 일종의 험블 객체로서 게이트웨이 인터페이스와 데이터베이스 사이에 경계를 형성**한다.
