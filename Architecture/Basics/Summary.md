...
* **`깨지기 쉬운 테스트`는 시스템에 강하게 결합되며, 시스템이 변경될 때마다 수많은 테스트가 깨지므로 변경이 유발**될 수 밖에 없다.
  * 만약 **시스템의 간단한 변경이 많은 테스트 실패를 유발한다면, 개발자는 변경을 하지 않으려 들게 되어 시스템의 유연성이 떨어지는 부작용**을 낳게 된다.
* 상술한 **`깨지기 쉬운 테스트` 문제를 해결하기 위해서는 반드시 아키텍쳐 설계시 테스트가 고려**되어야 한다.
  * 예를 들어 **테스트는 변동성이 큰 UI 등의 구성 요소에 의존하지 않고도 핵심 업무 규칙만을 테스트**할 수 있어야 한다.
...
### 세부사항 첫 번째 - 데이터베이스
```
> 데이터는 중요하지만 이를 다루기 위한 데이터베이스는 대표적인 세부사항이며, 아키텍쳐의 구성 요소 수준으로 끌어올려질 수 없다.
```
* 데이터베이스는 데이터 구조가 아닌 데이터 자체에 접근할 방법을 제공하는 유틸리티에 불과하며, 곧 저수준의 세부사항으로 아키텍쳐 자체와는 관계가 없다.

### 세부사항 두 번째 - 웹
* 웹은 GUI이고 GUI 역시 대표적인 세부사항이므로 웹 또한 세부사항이며, 때문에 반드시 핵심 업무 규칙으로부터 분리되어 경계의 바깥에 위치해야 한다.
...
* 시스템을 개발하는 '우리'는 프레임워크에 크게 헌신해야 하지만, 프레임워크는 '우리'에게 어떠한 헌신도 제공하지 않는다.
  * 나아가 이러한 프레임워크의 아키텍쳐는 깔끔하지 않은 경우가 많고, 심지어는 의존성 규칙을 위배하는 경향을 갖기도 한다.
