...
```
* **핵심 업무 규칙이란, 임의의 사업 자체에 대해 핵심적이면서도 자동화된 시스템이 없더라도 업무 규칙이 그대로 존재하는 규칙**을 의미한다.
...
* **엔티티는 핵심 업무 데이터를 포함하거나 이에 쉽게 접근할 수 있는 상태를 유지하며, 이를 토대로 동작하는 핵심 업무 규칙을 구현하는 함수들로 구성**된다.
  ...
  * **극단적으로 엔티티는 반드시 클래스로 표현될 필요조차 없으며, 그저 핵심 업무 데이터와 핵심 업무 규칙을 하나로 묶은 모듈인 것으로도 충분**하다.
...
### 요청 모델과 응답 모델
```
> 유즈케이스는 입력에 대한 출력을 생성하지만, 이러한 동작을 데이터를 사용자 또는 다른 컴포넌트와 주고 받는 방식과는 무관해야 한다.
```
...
* 엔티티와 요청 및 응답 데이터 모델은 그 특성상 많은 데이터를 공유하지만, 그럼에도 **요청과 응답 모델이 엔티티의 참조를 갖는 것은 지양**해야 한다.
...
> 아키텍쳐는 시스템 자체를 이야기하는 반면, 시스템에 적용된 프레임워크에 대해서 이야기할 필요는 없다.
```
